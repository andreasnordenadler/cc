...
export async function handleCustomQuestCreateRequest(request: Request, dependencies: CustomQuestCreateDependencies): Promise<Response> {
  ...
  const hasRequestedId = Object.prototype.hasOwnProperty.call(input, "id");
  if (hasRequestedId && (typeof input.id !== "string" || !/^custom-[a-z0-9-]+$/i.test(input.id))) return Response.json({ apiVersion: 1, authenticated: true, ok: false, message: "Unknown custom Side Quest." }, { status: 400 });
  ...
  try {
    ...
    const requestedId = hasRequestedId ? input.id as string : null;
    const previous = requestedId ? existing.find(item => item.id === requestedId) : undefined;
...
function compact(quest: CustomSideQuest, preserveValidConfig = false): CustomSideQuest { const parsed = parseCustomRuleConfig(quest.config); return { id: quest.id, title: cleanText(quest.title, 80) || "Custom Side Quest", summary: cleanText(quest.summary, 220) || (quest.lifecycle === "draft" ? "Draft Side Quest" : "Custom Side Quest"), config: parsed ? preserveValidConfig ? quest.config : JSON.stringify(parsed) : quest.config.slice(0, 1200), visibility: quest.visibility === "public" ? "public" : "private", lifecycle: quest.lifecycle === "draft" || quest.lifecycle === "archived" ? quest.lifecycle : "published", createdAt: typeof quest.createdAt === "string" ? quest.createdAt : new Date().toISOString(), updatedAt: typeof quest.updatedAt === "string" ? quest.updatedAt : new Date().toISOString(), badgeImageUrl: typeof quest.badgeImageUrl === "string" ? quest.badgeImageUrl.slice(0, 160) : null }; }
function getRawRuleBlockCount(config: string) { try { const parsed = JSON.parse(config) as { blocks?: unknown }; return Array.isArray(parsed?.blocks) ? parsed.blocks.length : 0; } catch { return 0; } }
