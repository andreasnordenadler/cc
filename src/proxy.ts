...
export const config = {
  matcher: ["/api/account", "/api/mobile/(.*)", "/api/analytics", "/api/community-likes", "/api/support", "/api/reports/(.*)", "/api/blocks/(.*)", "/api/groupquests/(.*)", "/api/groupquests", "/((?!_next|.*\\..*|_vercel|api|trpc).*)"],
};
