...
import { getCommunitySoloDetailHref, getCommunitySoloPickState } from "@/lib/mobile-web-parity-actions";
...
export default function CommunitySoloPickControl({
  ...
  const state = getCommunitySoloPickState({ questId, signedIn, activeQuestId, detailHref });
  ...
  async function pick() {
    ...
    try {
      ...
      window.location.href = getCommunitySoloDetailHref(questId, detailHref);
    } catch (caught) {
