...
export default function DesktopHomeMenu({ items, activeItemId, fullDocumentNavigation = false }: { items: readonly DesktopMenuItem[]; activeItemId: string; fullDocumentNavigation?: boolean }) {
      ...
      <nav aria-label="Desktop main menu">
        {groups.map((group) => (
          <section key={group.label} className="sqc-desktop-menu-group" aria-label={group.label}>
            ...
            {group.items.map((item) => fullDocumentNavigation || item.href.startsWith("/create-multiplayer-side-quest") ? (
              <FullDocumentLink
                key={item.id}
                href={item.href}
