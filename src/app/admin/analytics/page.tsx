...
export default async function AdminAnalyticsPage() {
  ...
  const client = await clerkClient();
  const firstPage = await client.users.getUserList({ limit: 100, offset: 0, orderBy: "-created_at" });
  const users = [...firstPage.data];
  for (let offset = users.length; ; offset += 100) {
    const page = await client.users.getUserList({ limit: 100, offset, orderBy: "-created_at" });
    users.push(...page.data);
    if (page.data.length < 100) break;
  }
  const rows: AnalyticsUserRow[] = users.map((user) => {
  ...
  const multiplayerQuestRows = users
  ...
  const inReportingWindow = (value?: string) => {
    const timestamp = value ? Date.parse(value) : Number.NaN;
    return Number.isFinite(timestamp) && timestamp >= windowStart.getTime() && timestamp <= now.getTime();
  };
  const newUsersInWindow = rows.filter((row) => inReportingWindow(row.createdAt));
  const activeUsersInWindow = activeRows.filter((row) => inReportingWindow(row.lastSeenAt));
  const retainedEventsInWindow = activeRows.flatMap((row) => row.recentEvents).filter((event) => inReportingWindow(event.at));
  const retainedEventCount = (type: SQCAnalyticsEvent["type"]) => retainedEventsInWindow.filter((event) => event.type === type).length;
      ...
      <div className="content-wrap">
        ...
        <section className="mission-card">
          <div className="section-head">
            <div>
              <span className="eyebrow">Last three months</span>
              <h2>{formatDateOnly(windowStart.toISOString())} → {formatDateOnly(now.toISOString())}</h2>
              <p>New accounts and last-seen activity are exact. Event counts are minimums because each user retains only their latest 12 detailed events.</p>
            </div>
          </div>
          <div className="grid lean-status-grid" aria-label="Three month analytics summary">
            <Fact label="New accounts" value={String(newUsersInWindow.length)} />
            <Fact label="Tracked active users" value={String(activeUsersInWindow.length)} />
            <Fact label="Retained page views (min.)" value={String(retainedEventCount("page_view"))} />
            <Fact label="Quest starts (min.)" value={String(retainedEventCount("quest_started"))} />
            <Fact label="Quest completions (min.)" value={String(retainedEventCount("quest_completed"))} />
            <Fact label="Profile saves (min.)" value={String(retainedEventCount("profile_saved"))} />
          </div>
        </section>
