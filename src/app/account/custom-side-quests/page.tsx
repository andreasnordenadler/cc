        ...
        <section className="mission-card" aria-label="My Custom Side Quest library">
          <div className="section-head">
            <div>
              ...
              <p>
                Create a starter rule here, publish it to Community Solo when ready, archive old ideas, or restore an archived recipe as a private draft. Raw custom configs stay hidden.
              </p>
            </div>
        ...
        <section className="mission-card" id="custom-side-quest-builder" aria-label="Create Custom Side Quest on website">
          <div className="section-head">
            <div>
              ...
              <h2>Create a Custom Solo Side Quest.</h2>
              <p>
                Match the mobile app&apos;s safest starter recipes: one clear proof condition, private draft by default, and optional public publishing for Community Solo discovery.
              </p>
            </div>
          ...
          <form action={saveCustomSideQuestFromWeb} className="profile-form">
            <label>
              Side Quest title
              <input name="title" maxLength={80} placeholder="Win after moving every knight" required />
            </label>
            <label>
              Tavern-wall summary
              <textarea name="summary" maxLength={220} rows={3} placeholder="Tell runners what strange chess habit this quest rewards." required />
            </label>
            <div className="grid side-quest-mode-grid">
              <label>
                Proof condition
                <select name="conditionType" defaultValue="gameResult">
              ...
              <label>
                ...
                <input name="sequence" placeholder="e4 e5 Nf3 or O-O" />
              </label>
              <label>
                Piece rule
                <select name="piece" defaultValue="queen">
              ...
              <label>
                Piece condition
                <select name="pieceCondition" defaultValue="moved">
              ...
              <label>
                ...
                <input name="targetSquare" maxLength={2} placeholder="e4" />
              </label>
              <label>
                Save state
            ...
            <div className="button-row">
              <button className="button primary" type="submit">Save Custom Side Quest</button>
              <Link className="button secondary" href="/challenges/community">See public examples</Link>
            </div>
          </form>
        </section>
      </div>
...
async function saveCustomSideQuestFromWeb(formData: FormData) {
  ...
  const config = buildWebCustomRuleConfig(formData);
  ...
  const quest: CustomSideQuest = compactCustomSideQuest({
    ...
    createdAt: now,
    updatedAt: now,
    badgeImageUrl: chooseCustomSideQuestBadge(),
  });

  await saveCustomQuestStore(client, authUser.id, [quest, ...existing.map(compactCustomSideQuest)].slice(0, 8));
  revalidatePath("/account/custom-side-quests");
  revalidatePath("/challenges/community");
  redirect("/account/custom-side-quests?saved=1#custom-side-quest-builder");
}
