...
import { completeMultiplayerCreateTransition, consumeMultiplayerCreateIntent, initialMultiplayerCreateIntent, requestMultiplayerCreateIntent, scheduleMultiplayerCreateOpen } from "./src/multiplayer/createIntent";
...
const MULTIPLAYER_CREATE_LIST_PAGE_SIZE = 8;
...
const CUSTOM_QUEST_TEMPLATES = [
  { title: "Win the game", helper: "Default friendly template: complete the Side Quest by winning your next public game.", condition: "game result" as const, result: "win" as const, piece: "queen" as const },
  { title: "Draw the game", helper: "For stubborn escape artists: hold the game to a draw.", condition: "game result" as const, result: "draw" as const, piece: "queen" as const },
  { title: "Queen adventure", helper: "Build a piece-story challenge around your queen moving, vanishing, or landing somewhere weird.", condition: "moved" as const, result: "win" as const, piece: "queen" as const },
  { title: "Knight dare", helper: "Start from a horse-crime idea and tune the exact condition after selecting it.", condition: "moved" as const, result: "win" as const, piece: "knight" as const },
] as const;
...
type CustomLibraryQuest = {
  ...
  likeSummary?: MobileCustomSideQuest["likeSummary"];
};
...
function getCheckActionMessage(receipt?: MobileAccountState["latestReceipt"] | null) {
  if (!receipt) return "No qualifying game found yet. Play a fresh public game on your connected chess account, then check again.";
  if (receipt.status === "passed" || receipt.headline?.toLowerCase().includes("passed")) return "Quest completed. Your proof is ready.";
  if (isFailedReceipt(receipt)) return `${getReceiptFailureText(receipt) ?? "That game did not match this Side Quest goal."} Try another fresh public game, then check again.`;
  return "No qualifying game found yet. Make sure the game is public, finished, on your connected username, and played after you picked this Side Quest.";
}
...
function MobileShell({ authBridge }: { authBridge: MobileAuthBridge }) {
  ...
  const pendingMultiplayerCreateQuestId = multiplayerCreateIntent.pendingQuestId;
      ...
      <View style={compactStyles.activeMultiplayerSection}>
        <Pressable accessibilityRole="button" accessibilityLabel="Open active Multiplayer Side Quest details" style={compactStyles.activeMultiplayerSummary} onPress={() => activeMultiplayer[0] ? setJoinedMultiplayerId(activeMultiplayer[0].id) : onSelectTab("multiplayerSideQuests")}>
          <View style={compactStyles.multiplayerHeroMarker}>
            ...
            <Image source={SQC_MULTIPLAYER_SEAL_ASSET} style={compactStyles.multiplayerHeroSeal} resizeMode="contain" />
          </View>
          <View style={compactStyles.activeSoloPill}>
        ...
        <View style={compactStyles.activeMultiplayerList}>
          {visibleActiveMultiplayer.length ? visibleActiveMultiplayer.map((quest) => (
            ...
            <AppRow title="No active Multiplayer Side Quests" meta="Join or host shared challenges with friends." status="Explore" imageSource={SQC_MULTIPLAYER_SEAL_ASSET} onPress={() => onSelectTab("multiplayerSideQuests")} />
          )}
          {activeMultiplayer.length > 5 ? (
            <AppRow title={showAllActiveMultiplayer ? "Show fewer Multiplayer Side Quests" : "Show all active Multiplayer Side Quests"} meta={showAllActiveMultiplayer ? "Collapse this list back to the top five." : `${activeMultiplayer.length - 5} more active Multiplayer Side Quest${activeMultiplayer.length - 5 === 1 ? "" : "s"}.`} status={showAllActiveMultiplayer ? "Collapse" : "Expand"} imageSource={SQC_MULTIPLAYER_SEAL_ASSET} onPress={() => setShowAllActiveMultiplayer((current) => !current)} />
          ) : null}
        </View>
...
function JoinedMultiplayerQuestModal({
  ...
  onToggleLike?: (quest: MobileGroupQuestSummary) => Promise<void> | void;
  ...
  async function handleToggleLike() {
    if (!quest || !onToggleLike || likeBusy) return;
    setLikeBusy(true);
    try {
      await onToggleLike(quest);
    } finally {
      setLikeBusy(false);
    }
  }
...
function getMultiplayerQuestCoatSource(_title: string): ImageSourcePropType {
  return SQC_MULTIPLAYER_SEAL_ASSET;
}
...
function getLikeSummaryLine(likeSummary?: { count: number; likedByViewer: boolean }) {
  ...
  return `${likeSummary?.likedByViewer ? "Liked by you · " : ""}${count} like${count === 1 ? "" : "s"}`;
}
...
function QuestBoardDashboard({
  ...
  const communityBrowseQuests = publicCommunityQuests
    ...
    .filter((quest) => {
      if (communityFilter === "popular") return getCustomQuestPopularity(quest.stats) + getLikeCount(quest.likeSummary) * 5 > 0;
    ...
    .sort((a, b) => {
      ...
      if (communitySort === "liked") return getLikeCount(b.likeSummary) - getLikeCount(a.likeSummary) || getCustomQuestUpdatedMs(b) - getCustomQuestUpdatedMs(a);
      const popularDelta = (getCustomQuestPopularity(b.stats) + getLikeCount(b.likeSummary) * 5) - (getCustomQuestPopularity(a.stats) + getLikeCount(a.likeSummary) * 5);
  ...
  function focusSelectedQuestCloseButton() {
    const nodeHandle = findNodeHandle(selectedQuestCloseButtonRef.current);
    if (nodeHandle !== null) AccessibilityInfo.setAccessibilityFocus(nodeHandle);
  }
  ...
  function applyCustomQuestTemplate(template: (typeof CUSTOM_QUEST_TEMPLATES)[number]) {
    ...
    setCustomQuestName(template.title);
    setCustomRuleCondition(template.condition);
    setCustomRuleResult(template.result);
  ...
  return (
    <View style={compactStyles.stack}>
      ...
      <CustomSideQuestDetailModal
        ...
        onToggleLike={customDetailDraft?.visibility === "public" ? toggleCommunitySoloLike : undefined}
          ...
          <ScrollHintedScrollView contentContainerStyle={[compactStyles.detailContent, compactStyles.detailContentWithBottomSafe]} showsVerticalScrollIndicator={false}>
            ...
            <View style={compactStyles.multiplayerNativeCard}>
              ...
              <View style={compactStyles.multiplayerOptionGrid}>
                {CUSTOM_QUEST_TEMPLATES.map((template) => (
                  <Pressable key={template.title} accessibilityRole="button" accessibilityLabel={`Use ${template.title} custom Side Quest template`} style={compactStyles.multiplayerOptionCard} onPress={() => applyCustomQuestTemplate(template)}>
                    <View style={compactStyles.multiplayerOptionCopy}>
                      <Text style={compactStyles.multiplayerOptionTitle}>{template.title}</Text>
                      <Text style={compactStyles.multiplayerOptionHelper}>{template.helper}</Text>
                    </View>
                  </Pressable>
                ))}
              </View>
      ...
      <Modal visible={Boolean(detailChallenge)} animationType="slide" presentationStyle="fullScreen" onShow={focusSelectedQuestCloseButton} onRequestClose={closeSelectedQuestDetail}>
        <SafeAreaView style={compactStyles.detailScreen} accessibilityViewIsModal onAccessibilityEscape={closeSelectedQuestDetail}>
          <GradientBackdrop challenge={detailChallenge} />
          <View style={compactStyles.detailTopBar}>
            <Pressable ref={selectedQuestCloseButtonRef} accessibilityRole="button" accessibilityLabel="Close Side Quest details" style={compactStyles.detailCloseButton} onPress={closeSelectedQuestDetail}>
              <MaterialCommunityIcons name="close" size={23} color={colors.paper} />
            </Pressable>
          </View>
          <ScrollHintedScrollView contentContainerStyle={[compactStyles.detailContent, compactStyles.detailContentWithBottomSafe]} showsVerticalScrollIndicator={false}>
...
function MultiplayerSideQuestsScreen({ bootstrap, account, authBridge, onSelectTab, pendingCreateOpenToken, pendingCreateQuestId, onConsumePendingCreateOpen, onAccountUpdated }: { bootstrap: MobileBootstrap; account: MobileAccountResponse | null; authBridge: MobileAuthBridge; onSelectTab: (tab: AppTab) => void; pendingCreateOpenToken?: number; pendingCreateQuestId?: string | null; onConsumePendingCreateOpen?: () => void; onAccountUpdated: AccountUpdatedCallback }) {
  ...
  const officialCreateQuestChoiceCount = createQuestChoices.filter((choice) => choice.source === "official").length;
  const communityCreateQuestChoiceCount = createQuestChoices.filter((choice) => choice.source === "community").length;
  ...
  useEffect(() => {
    ...
    return scheduleMultiplayerCreateOpen({
      pendingToken: pendingCreateOpenToken ?? 0,
      handledToken: lastHandledPendingCreateTokenRef.current,
      delayMs: Platform.OS === "android" ? 50 : 0,
      onOpen: (token) => {
        lastHandledPendingCreateTokenRef.current = token;
        setCreateOpen(true);
      },
    });
  ...
  async function createGroupQuest() {
    ...
    try {
      ...
      await completeMultiplayerCreateTransition({
        createdGroupQuestId,
        message: result.message,
        closeCreate: () => setCreateOpen(false),
        consumeIntent: () => onConsumePendingCreateOpen?.(),
        resetDraft: resetCreateDraft,
        refreshAccount: onAccountUpdated,
        waitForRefresh: () => waitMs(450),
        publishSuccess: (questId, message) => setGroupQuestActionState({ busy: false, questId, message, error: null }),
        openCreatedQuest: (questId) => {
          if (!result.groupQuestId) return;
          setJoinedMultiplayerId(questId);
          setOfficialMultiplayerId(null);
  ...
  const availableGroupQuests = allCommunityMultiplayerQuests
    ...
    .sort((a, b) => {
      ...
      if (multiplayerCommunitySort === "liked") return getLikeCount(b.likeSummary) - getLikeCount(a.likeSummary) || Date.parse(b.startAt ?? b.endAt ?? "") - Date.parse(a.startAt ?? a.endAt ?? "");
  ...
  return (
    <View style={styles.screenStack}>
      ...
      {isSignedOutBrowse ? null : <View style={styles.groupquestsActionCard} accessibilityLabel="Join private Multiplayer Side Quest">
      ...
      <Modal visible={createOpen} animationType="slide" presentationStyle="fullScreen" onShow={createBuilderModalAccessibility.focusInitial} onRequestClose={createBuilderModalAccessibility.dismiss}>
        <SafeAreaView style={compactStyles.detailScreen} accessibilityViewIsModal onAccessibilityEscape={createBuilderModalAccessibility.dismiss}>
          ...
          <ScrollHintedScrollView contentContainerStyle={[compactStyles.detailContent, compactStyles.detailContentWithBottomSafe, compactStyles.detailContentWithCreateFooter]} showsVerticalScrollIndicator={false}>
            <View style={compactStyles.multiplayerDetailHero}>
              <Image source={SQC_GENERIC_COAT_GLOW_ASSET} style={compactStyles.multiplayerDetailSealGlow} resizeMode="contain" />
              <Image source={SQC_MULTIPLAYER_SEAL_ASSET} style={compactStyles.multiplayerDetailSeal} resizeMode="contain" />
            ...
            <View style={compactStyles.multiplayerNativeCard}>
              ...
              <View style={compactStyles.createSelectedTray}>
                {selectedCreateQuestChoices.length ? selectedCreateQuestChoices.map((choice, index) => (
                  <Pressable key={choice.id} accessibilityRole="button" accessibilityLabel={`Remove ${choice.title} from Multiplayer Side Quest`} style={compactStyles.createSelectedRow} onPress={() => toggleCreateQuestId(choice.id)}>
                    <Text style={compactStyles.createSelectedIndex}>{index + 1}</Text>
                    <View style={compactStyles.createSelectedCopy}>
                      <Text style={compactStyles.createSelectedTitle} numberOfLines={1}>{choice.title}</Text>
                      <Text style={compactStyles.createSelectedMeta} numberOfLines={1}>{choice.sourceBadge ?? choice.status}</Text>
                    </View>
                    <View style={compactStyles.createSelectedRemoveIcon}>
                      <MaterialCommunityIcons name="close" size={14} color="rgba(255,247,232,.72)" />
                    </View>
                  </Pressable>
                )) : (
                  <View style={compactStyles.createSelectionEmptyPanel}>
                    <Text style={compactStyles.createSelectionEmptyTitle}>No Side Quests selected yet.</Text>
                    <Text style={compactStyles.createSelectionEmpty}>Search or browse below, then tap rows to add them here.</Text>
                  </View>
                )}
              </View>
            ...
            <View style={compactStyles.multiplayerNativeCard}>
              <Text style={compactStyles.multiplayerCardEyebrow}>Add from catalog</Text>
              <Text style={compactStyles.multiplayerCardTitle}>Browse like Community Side Quests.</Text>
              ...
              <View style={compactStyles.sideQuestBrandTabs} accessibilityRole="tablist" accessibilityLabel="Multiplayer creator Side Quest sources">
                ...
                <Pressable
                  ...
                  hitSlop={10}
                  style={compactStyles.sideQuestBrandTabSwitchButton}
                  onPress={() => setCreateQuestSourceTab((current) => current === "official" ? "community" : "official")}
                >
                  <MaterialCommunityIcons name="swap-horizontal-bold" size={19} color="rgba(255,247,232,.82)" />
                </Pressable>
                <Pressable
                ...
                >
                  <Text style={[compactStyles.sideQuestBrandTabText, createQuestSourceTab === "community" && compactStyles.sideQuestBrandTabCommunityTextActive]} numberOfLines={2}>Community ({communityCreateQuestChoiceCount})</Text>
                </Pressable>
              </View>
                    ...
                    {visibleCreateQuestChoicePage.map((choice) => {
                      ...
                      return (
                        <AppRow
                          ...
                          title={choice.title}
                          meta={choice.meta}
                          status={included ? "Remove" : maxed ? "Max 4" : "Add"}
          ...
          <View style={compactStyles.createFooterBar}>
            <View style={compactStyles.createFooterCopy}>
...
function CustomSideQuestDetailModal({
  ...
  onToggleLike?: (quest: CustomLibraryQuest) => Promise<void> | void;
...
const compactStyles = StyleSheet.create({
  ...
  likePill: { flexShrink: 0, flexDirection: "row", alignItems: "center", gap: 2, paddingHorizontal: 2, paddingVertical: 1, opacity: .72 },
  likePillLiked: { opacity: 1 },
  ...
  createSelectedRow: { minHeight: 46, flexDirection: "row", alignItems: "center", gap: 8, paddingVertical: 7, paddingHorizontal: 9, borderRadius: 14, backgroundColor: "rgba(245,200,106,.12)", borderWidth: 1, borderColor: "rgba(245,200,106,.24)" },
  ...
  createFooterBar: { position: "absolute", left: 0, right: 0, bottom: 0, minHeight: 92, flexDirection: "row", alignItems: "center", gap: 10, paddingHorizontal: 16, paddingTop: 12, paddingBottom: 18, backgroundColor: "rgba(18,12,13,.96)", borderTopWidth: 1, borderTopColor: "rgba(245,200,106,.22)", zIndex: 40, elevation: 12 },
  createFooterCopy: { flex: 1, minWidth: 0, gap: 2 },
  createFooterTitle: { color: colors.green, fontSize: 12, lineHeight: 15, fontWeight: "900", textTransform: "uppercase", letterSpacing: .65 },
  createFooterMeta: { color: colors.muted, fontSize: 12, lineHeight: 16, fontWeight: "800" },
  createFooterButton: { minWidth: 108, minHeight: 46, alignItems: "center", justifyContent: "center", paddingHorizontal: 16, borderRadius: 999, backgroundColor: colors.gold },
  createFooterButtonText: { color: "#111", fontSize: 14, lineHeight: 18, fontWeight: "900" },
