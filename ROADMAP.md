...
- [ ] Prepare SQC for polished launch readiness without adding more beta-tester functionality.
  ...
  - 2026-05-04 07:58 Europe/Stockholm progress: made `/path` directly activate the next starter quest for signed-in runners and each unfinished starter-path card, while signed-out visitors get a clear connect-first path; proof doc `docs/SQC_STARTER_PATH_DIRECT_ACTIVATION_LIVE_DEPLOY_2026-05-04.md`. Verification so far: `pnpm install --frozen-lockfile`, `pnpm lint`, and `pnpm build` passed; live deploy/smoke pending.
