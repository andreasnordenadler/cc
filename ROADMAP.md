...
- [ ] Prepare SQC for polished friends/private beta before any public launch push.
  ...
  - 2026-05-03 21:00 Europe/Stockholm progress: polished first-run visible language so the top nav, homepage, quest hub, today page, path, share kit, invite copy, and verifier fallback consistently frame the product as quests instead of mixed challenge/dare language while preserving existing `/challenges` and `/dare` routes; proof doc `docs/SQC_QUEST_LANGUAGE_POLISH_LIVE_DEPLOY_2026-05-03.md`. Verification: `pnpm lint`, `pnpm build`, production deploy to `https://cc-olqz1vx3k-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`, live smoke for deploy/canonical `/`, canonical `/challenges`, `/today`, and `/share-kit`. Branch pushed as `origin/sqc-quest-language` because the main checkout has unrelated dirty files.
  - Acceptance:
