...
Last updated: 2026-06-11 10:05 Europe/Stockholm
Owner: Sam  
...
- [ ] 24h SQC website UX parity review: make newly added website functions feel app-quality from a user perspective.
  ...
  - 2026-06-11 Community Solo coat reassignment slice: Andreas requested replacing the old repeated coat on existing community-created Side Quests with randomized coats from the newly created pool and confirming pool quality. QA reviewed the generated random pool, kept 12 approved no-text/generated coat assets, excluded the weak checker-background candidate `custom-coat-11.png` plus old generic/custom variants from future assignment, and applied Clerk metadata updates so all existing public/published community quests use approved random-pool coats. Proof: random-pool contact sheet QA pass except excluded `custom-coat-11.png`; dry-run found 47 quests across 35 users; apply updated 47 quests; verification scan found 47 public/published community quests, 0 invalid/old coat paths, and distribution across all 12 approved coats; follow-up smoke caught stale rendered seed/fallback paths, so the public Community Solo catalog was made no-store and all remaining custom fallback surfaces were moved to the approved random-pool fallback; added a narrow Community Solo render normalizer so any stale/missing/out-of-pool coat path is displayed as a deterministic approved-pool coat; `pnpm lint`; `pnpm lint -- src/lib/community-side-quests.ts`; `pnpm build`; `pnpm deploy:prod` released `https://cc-ama3kx0ad-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke confirmed `/challenges/community` renders all 12 approved random-pool coat paths, 0 old fallback crest paths, 0 excluded `custom-coat-11` paths, and detail pages for `seed-opening-hipster-24-1` / `seed-opening-hipster-32-1` render approved random coats only.
