...
1. **P0 — Finish structured verifier diagnostics.** Several official verifiers already return final-position FEN and last-move SAN/UCI, but the result contract is inconsistent and does not reliably identify the first failing ply/condition. Define one diagnostic payload, adopt it across verifiers, and render it consistently on web/mobile proof boards.
2. **P1 — Complete paired logged-in app/web QA.** The July 9 parity sprint shipped many verified slices, but current Android capture was unstable and several slices relied on source or older app evidence. Re-establish a stable signed-in emulator/device capture path, then close the remaining Home and account-state comparisons with paired screenshots.
3. **P1 — Define creator post-finish operations.** Deadline locking, final ranking, podium rewards, result links, and participant final states exist; explicit host handling for ties/edge cases and verifier-event review remains undefined.
4. **P2 — External/product-decision blockers.** Facebook sign-in UI exists but needs Clerk/Facebook production configuration and end-to-end smoke. Multilingual architecture remains intentionally blocked until Andreas reactivates it.
...
| Locked time-control proof enforcement | **proven complete** | `multiplayer-proof-rules.ts` normalizes Lichess/Chess.com metadata and rejects time-control, rated-state, player-color, provider, variant, window, and result mismatches. `groupquest-proof.ts` binds that decision to the verified replay, and executable web/mobile refresh-route tests prove mismatch rejection before writes. Verified on 2026-09-16 by 124 targeted tests and the 2,268-test suite. |
...
- [x] Remove visible transparency holes from the Back Rank Goblin and Pawn Only Picnic coat-of-arms assets.
  ...
  - proof: patched `public/badges/v7/coming-soon-clean/back-rank-goblin-badge.png`, `public/badges/v7/coming-soon-clean/pawn-only-picnic-badge.png`, and matching `apps/mobile/assets` copies; visual QA passed with no visible internal transparency holes and no rectangular/angular backing artifacts; `pnpm --filter @sidequestchess/mobile typecheck` passed; `pnpm lint` passed with existing 3 warnings only; `pnpm build` passed; `pnpm mobile:release:github` published Android `mobile-v287` with SHA256 `91c2419382ecee2220f80f28bc2b431a34070f6e09151616045147bf3d204940`; commit `b51a0f0` pushed to `main`; guarded `pnpm deploy:prod` deployed `https://cc-2wnch0jl5-andreas-nordenadlers-projects.vercel.app` and aliased it to `https://sidequestchess.com`; production byte checks confirmed both live PNGs exactly match local SHA256s (`d50bc2aba2243d33877a31775a3dd565eafd8ded5858a1a227811f15a044e8c1`, `36bb8792b47858ebe5a21475ca2703940fa98be41ae4d0b97ac28688c774139f`); live `/challenges`, `/challenges/back-rank-goblin`, and `/challenges/pawn-only-picnic` returned 200; Vercel production error-log scan over 30m returned 0 lines.
...
- [x] Add Community Solo + Multiplayer like/unlike parity to the mobile app.
  ...
  - proof: implemented mobile authenticated `/api/mobile/community-likes`, added `likeSummary` to mobile account Solo + Multiplayer payloads, wired like/unlike buttons/counts into Community Solo and Multiplayer detail surfaces, added like counts to lists, added Most liked sort cycles for both surfaces, and published APK `artifacts/mobile-releases/sqc-mobile-android-v263-2026-06-15.apk` (`0.1.263` / versionCode `263` / sha256 `cb8897d55f1af255ecfed68c5fa1dc9a95771d56bf575e7dbf855e26a3a7befe`) at `https://github.com/andreasnordenadler/cc/releases/tag/mobile-v263`. Verification passed: `pnpm --filter @sidequestchess/mobile typecheck`; `pnpm lint` (existing 3 warnings only); `pnpm build`; `pnpm mobile:release`; `pnpm mobile:release:github`; installed APK smoke `pnpm mobile:smoke:hamburger -- --apk artifacts/mobile-releases/sqc-mobile-android-v263-2026-06-15.apk`; production deploy guard and Vercel deploy passed; live smoke returned 200 for `/challenges/community?sort=liked` and `/groupquests/public?sort=liked`, and signed-out `/api/mobile/community-likes` returned 401 as expected.
...
- [x] Add signed-in like/unlike ratings for Community Solo Side Quests and public Community Multiplayer Side Quests.
  - added_at: 2026-06-15 08:03 Europe/Stockholm
  - source: Andreas requested a simple rating system: like button is OK, signed-in users only, one like per Side Quest, unlike allowed, usable for sorting.
...
- [x] Release SQC Mobile v259 fixing disappearing Discover/My Custom segmented-button labels.
  ...
  - proof: shipped in commits `8dcb5e4` and `bb2154d`, pushed to `main`, and published GitHub release `mobile-v259` (`https://github.com/andreasnordenadler/cc/releases/tag/mobile-v259`). APK `sqc-mobile-android-v259-2026-06-14.apk` has versionName `0.1.259`, versionCode `259`, SHA256 `75644aece40695b7118afa7b34aeaf4387ecf184f5d316c9a24c7b6d019616aa`. Verification passed: `pnpm --filter @sidequestchess/mobile typecheck`; `pnpm lint` (existing 3 warnings only); `pnpm build`; `pnpm mobile:release`; installed APK smoke `pnpm mobile:smoke:hamburger -- --apk artifacts/mobile-releases/sqc-mobile-android-v259-2026-06-14.apk`, including Discover/My Custom subtab label assertions.
...
- [x] 24h SQC website UX parity review: make newly added website functions feel app-quality from a user perspective.
  ...
  - 2026-06-10 Any Game Counts default-starter slice: made `finish-any-game` / **Any Game Counts** the default pre-selected active Solo Side Quest for fresh signed-in users on web and mobile account bootstrap paths. The shared helper only applies when there is no existing active quest, no completed quest history, and no saved proof attempts, so established users are not overwritten. Proof: `pnpm lint -- src/lib/user-metadata.ts src/app/page.tsx src/app/account/page.tsx src/app/challenges/page.tsx 'src/app/challenges/[id]/page.tsx' src/app/api/mobile/account/route.ts`; `pnpm build`; `pnpm quest:release-gate`; committed/pushed `5341509`; production deploy `https://cc-mku0r6fse-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for `/`, `/challenges`, `/challenges/finish-any-game`, and deploy URL `/challenges/finish-any-game` with expected SQC/Any Game Counts content; proof doc `docs/SQC_ANY_GAME_COUNTS_DEFAULT_STARTER_2026-06-10.md`.
  - 2026-06-10 Custom Solo builder UX polish slice: started the UX parity review with Andreas's flagged Custom Solo builder. Replaced internal “website” language in `/account/custom-side-quests` with product-facing Custom Solo/library copy, renamed the builder to `Custom Solo builder`, added a three-step creation guide, and changed the six-condition rule editor from one overwhelming grid into SQC-styled condition cards where optional slots stay collapsed unless populated and piece-state details are grouped clearly. Proof: `pnpm lint -- src/app/account/custom-side-quests/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `61f30fd`; production deploy `https://cc-ccu5j9zmi-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned expected signed-out `307` for `/account/custom-side-quests?uxBuilderSmoke=20260610` and 200 Community Solo content on production/deploy URLs; proof doc `docs/SQC_WEB_CUSTOM_SOLO_BUILDER_UX_POLISH_2026-06-10.md`.
  - 2026-06-10 Community Solo product-language polish slice: continued Andreas's product-language cleanup by removing remaining visible internal web-vs-app phrasing from Community Solo browse/detail and Custom Solo management actions. Replaced `website or app` / `Website-first` / `App-first` style copy with player-facing SQC account, roomy web view, compact mobile flow, and `Edit recipe` language while preserving established cards and verifier behavior. Proof: no flagged phrase-family matches remain in `src/app`, `src/components`, `src/lib`; `pnpm lint -- src/app/account/custom-side-quests/page.tsx src/app/challenges/community/page.tsx 'src/app/challenges/community/[id]/page.tsx'`; `pnpm build`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-bjv8gf54h-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 Community Solo browse/detail content and signed-out account sign-in content; proof doc `docs/SQC_WEB_COMMUNITY_SOLO_PRODUCT_LANGUAGE_POLISH_2026-06-10.md`.
  - 2026-06-10 product-language sweep slice: continued visible UX polish by removing remaining player-facing web-vs-mobile/internal phrasing across Community Solo, Custom Solo empty states, Official Solo share, Solo discovery, profile/account readiness, Multiplayer invite/results, home Trophy Cabinet, badges, and support. Reframed Community Solo `Creator context` as a public `Player shelf`, changed `More by` to `More from`, and kept technical Open Graph/API `website` enum values unchanged because they are not product copy. Proof: `pnpm lint -- src/app/challenges/community/page.tsx 'src/app/challenges/community/[id]/page.tsx' 'src/app/challenges/[id]/page.tsx' src/app/challenges/page.tsx src/app/profile/page.tsx src/app/account/page.tsx src/app/groupquests/page.tsx src/app/page.tsx src/app/badges/page.tsx src/app/support/page.tsx src/components/support-contact-form.tsx`; `pnpm build`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-6gxhyqcx5-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned expected Community Solo `Player shelf`/public-player content, Support details copy, and signed-out Custom Solo sign-in/SQC content; proof doc `docs/SQC_WEB_PRODUCT_LANGUAGE_SWEEP_2026-06-10.md`.
  - 2026-06-10 surface language cleanup slice: continued the UX parity sprint language cleanup by removing remaining player-visible web-vs-mobile comparison wording from Community Solo browse/detail, Official Leaderboards, and the mobile profile save message. Community Solo now reads as one SQC account path with `Recipe review`/`Account handoff` actions instead of `Roomy web view`/`Compact mobile flow`; Scoreboard presents the page as the SQC leaderboard hall instead of positioning it against the mobile app. Proof: `grep -RInE "web view|mobile flow|mobile app|website sync|website creator|Website creator|website-first|Website-first|web-first|website or app|switch to mobile" src/app src/components src/lib` now only finds an admin-only support label; `pnpm lint -- src/app/challenges/community/page.tsx 'src/app/challenges/community/[id]/page.tsx' src/app/scoreboard/page.tsx src/app/api/mobile/profile/route.ts`; `pnpm build`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-7sr44mn6m-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for Community Solo browse/detail and Scoreboard with new SQC account/leaderboard hall copy and no old `Roomy web view`/`Compact mobile flow`/`mobile app` strings; proof doc `docs/SQC_WEB_SURFACE_LANGUAGE_CLEANUP_2026-06-10.md`.
  - 2026-06-11 Custom Solo rule-focus polish slice: continued Andreas's flagged builder UX cleanup by reducing the six-condition editor's visual clutter. Condition cards now show only the rule controls matching the selected proof shape, optional `No condition` slots get a calm resting state, and helper copy explains a focused pick-rule-then-fill flow while preserving existing SQC styling and verifier behavior. Proof: `pnpm lint -- src/app/account/custom-side-quests/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `1e74b51`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-oskdhldra-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned expected signed-out `307` for `/account/custom-side-quests?ruleFocusSmoke=20260611`, 200 Community Solo content, and 200 Public Multiplayer content; proof doc `docs/SQC_WEB_CUSTOM_SOLO_RULE_FOCUS_POLISH_2026-06-11.md`.
  - 2026-06-11 Custom Solo library action polish slice: continued the flagged Custom Solo UX cleanup by replacing the saved-library card action pile with an SQC-styled action panel. Cards now show a plain status line, group public/start/Multiplayer actions under `Play`, reveal proof tools only when active/completed, and tuck duplicate/edit/archive/delete behind `Manage recipe`, with `Edit recipe` copy replacing generic edit wording. Proof: `pnpm lint -- src/app/account/custom-side-quests/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `37a6674`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-mp0t6thk6-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned signed-out sign-in content for production/deploy `/account/custom-side-quests?actionPanelSmoke=20260611`, 200 Community Solo content, and 200 Public Multiplayer content; proof doc `docs/SQC_WEB_CUSTOM_SOLO_LIBRARY_ACTION_POLISH_2026-06-11.md`.
  - 2026-06-11 Custom Solo builder copy/checklist polish slice: continued the flagged builder UX cleanup by replacing harsher shelf/library wording with player-facing Custom Solo recipe language, renaming save-state choices to `Draft (private)`, `Playable privately`, and `Publish to Community Solo`, and adding an SQC-styled quality checklist so runners see the “clear promise / one strong rule” expectation before filling proof conditions. Proof: `pnpm lint -- src/app/account/custom-side-quests/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `8e71b2a`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-llqw7rsqp-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned signed-out sign-in content for production/deploy `/account/custom-side-quests?copyChecklistSmoke=20260611`, 200 Community Solo content, and 200 Public Multiplayer content; proof doc `docs/SQC_WEB_CUSTOM_SOLO_BUILDER_COPY_CHECKLIST_POLISH_2026-06-11.md`.
  - 2026-06-11 Custom Solo recipe starter polish slice: continued the flagged builder UX cleanup by adding an SQC-styled `Recipe starters` panel before the form, translating the rule controls into three player-facing patterns (`Clean finish`, `Opening dare`, `Piece story`) so runners can choose the quest vibe before handling verifier fields. The panel preserves the established tavern-card styling, keeps mobile stacking tidy, and does not change saved rule behavior or verifier paths. Proof: `pnpm lint -- src/app/account/custom-side-quests/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `1d2719c`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-l8isvb1j9-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned signed-out `307` to `/sign-in` for `/account/custom-side-quests?recipeStarterSmoke=20260611`, deploy sign-in content, 200 Community Solo content, and 200 Public Multiplayer content; proof doc `docs/SQC_WEB_CUSTOM_SOLO_RECIPE_STARTERS_POLISH_2026-06-11.md`.
  - 2026-06-11 Community Solo inline polish slice: continued the visible UX parity review by upgrading Community Solo browse usability. Discovery controls now sit in an SQC-styled `Find your next run` panel with clearer search/filter copy and quick chips; public quest cards show inline `Rule preview` details before click-through; and card CTAs are grouped under a `Next step` panel so inspect/start/report/player-shelf actions feel deliberate instead of piled together. Proof: `pnpm lint -- src/app/challenges/community/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `358412b`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-j7zbysi5y-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/challenges/community?inlinePolishSmoke=20260611b` with `Find your next run`/`Rule preview`/`Next step`, and 200 Public Multiplayer content; proof doc `docs/SQC_WEB_COMMUNITY_SOLO_INLINE_POLISH_2026-06-11.md`.
  - 2026-06-11 Community Solo detail polish slice: continued the visible UX parity review by upgrading Community Solo detail pages. Detail heroes now show quest promise, crest, quick facts, and update context; a hero-level `Rule preview` and `Next step` panel make start/Multiplayer/share/report choices deliberate; and the old repetitive rule-summary block is replaced by a compact `Run checklist` in player-facing SQC account language. Proof: `pnpm lint -- 'src/app/challenges/community/[id]/page.tsx' src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `f548197`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-2qeks7rec-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy Community Solo detail with `Rule preview`/`Run checklist`/`Next step`, 200 Community Solo browse content, and 200 Public Multiplayer content; proof doc `docs/SQC_WEB_COMMUNITY_SOLO_DETAIL_POLISH_2026-06-11.md`.
  - 2026-06-11 official Solo proof-check choices polish slice: continued the proof submission/check UX parity review by turning the active official Solo checker area into two SQC-styled proof-choice cards: `Fastest check` for the newest public game and `Specific game` for a known Lichess ID or Chess.com URL, while keeping the existing verifier actions unchanged. Also removed remaining visible `mobile-style` wording from private Multiplayer host-code helper copy. Proof: `pnpm lint -- 'src/app/challenges/[id]/page.tsx' 'src/app/groupquests/[id]/page.tsx' src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `901be2b`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-b2yc3epmp-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy official Solo detail, seeded Multiplayer detail, and Public Multiplayer browse, with old `mobile-style joining` copy absent; proof doc `docs/SQC_WEB_OFFICIAL_PROOF_CHECK_CHOICES_POLISH_2026-06-11.md`.
  - 2026-06-11 Multiplayer proof-choice polish slice: continued proof submission/check UX parity review by turning the joined Multiplayer proof area from a single refresh control into SQC-styled proof-choice cards. The live verifier action now sits in a `Fastest check` / `Judge my latest table game` card, a companion `What SQC checks` card explains objective/provider/window/receipt behavior, and onboarding copy no longer promises a paste-game-link path this surface does not support. Proof: `pnpm lint -- 'src/app/groupquests/[id]/page.tsx' src/components/group-quest-proof-controls.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `c74aaed`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-jjw3yf1xs-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy seeded Multiplayer detail with `Run the latest-game check`, 200 Public Multiplayer content, and 200 Community Solo content; proof doc `docs/SQC_WEB_MULTIPLAYER_PROOF_CHOICE_POLISH_2026-06-11.md`.
  - 2026-06-11 Multiplayer create-flow polish slice: continued Multiplayer create/join/proof UX parity review by making the signed-in `/groupquests/create` host builder feel more guided and app-quality. The create flow now opens with an SQC-styled three-step host guide, clarifies the side-quest picker as a `Quest stack`, labels choices as `Official Solo` / `Custom Solo`, replaces the loose save footer with a `Ready to host?` action panel, and removes visible `mobile shortcut` comparison wording while preserving verifier/release behavior. Proof: `pnpm lint -- src/components/group-quest-draft-builder.tsx src/app/groupquests/create/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; first deploy attempt was safely blocked until local `HEAD` was pushed; committed/pushed `1a75831`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-4fuqz95vx-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned expected signed-out `307` for production/deploy `/groupquests/create?createFlowSmoke=20260611`, 200 Public Multiplayer content, and 200 seeded Multiplayer invite/proof-check content; proof doc `docs/SQC_WEB_MULTIPLAYER_CREATE_FLOW_POLISH_2026-06-11.md`.
  - 2026-06-11 remaining product-language polish slice: continued Andreas's product-language cleanup after noticing newly reintroduced visible `recipe`/`account handoff` wording in Custom Solo, Community Solo detail, and Multiplayer create copy. Custom Solo now consistently talks about Custom Solo Side Quests/quests/rules, the builder starter panel is `Quest starters`, Community Solo checklist copy uses normal account start/report language, and Multiplayer create references launch-ready Custom Solo Side Quests. Proof: targeted phrase sweep now only finds an admin-only support label; `pnpm lint -- src/app/account/custom-side-quests/page.tsx 'src/app/challenges/community/[id]/page.tsx' src/components/group-quest-draft-builder.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `61515e2`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-eegoim823-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy Community Solo detail with new account-start/quest-report language and no forbidden phrase matches, 200 Public Multiplayer content, and signed-out sign-in content for Custom Solo/Multiplayer create routes; proof doc `docs/SQC_WEB_REMAINING_PRODUCT_LANGUAGE_POLISH_2026-06-11.md`.
  - 2026-06-11 account readiness polish slice: continued the account-readiness UX parity review by replacing support-centric readiness copy on `/account` with player-facing run setup language and adding an SQC-styled `Ready to run` checklist for identity, chess account, and next Solo quest while preserving existing progress/Trophy Cabinet behavior. Proof: `pnpm lint -- src/app/account/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `9081c50`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-h6vn6ugpi-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned signed-out sign-in content for production/deploy `/account?readinessSmoke=20260611`, 200 profile sign-in content, and 200 Solo discovery content; proof doc `docs/SQC_WEB_ACCOUNT_READINESS_POLISH_2026-06-11.md`.
  - 2026-06-11 Trophy Cabinet receipt polish slice: continued account/Trophy Cabinet proof-receipt parity by adding a compact Trophy Cabinet summary strip and turning completed Solo/Custom trophy cards into clearer receipt cards with provider/game context, safe proof summary, and grouped open/share actions while preserving existing public proof links and account behavior. Proof: `pnpm lint -- src/app/account/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `d449b41`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-39tj0sco3-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned expected signed-out `307` for production/deploy `/account?trophyReceiptSmoke=20260611`, 200 Solo discovery content, and 200 public proof receipt/proof-board content; proof doc `docs/SQC_WEB_TROPHY_CABINET_RECEIPT_POLISH_2026-06-11.md`.
  - 2026-06-11 saved-quest language polish slice: continued Andreas's product-language cleanup by removing remaining visible shelf/library/host-context wording from Custom Solo management, Community Solo share/empty states, and Multiplayer host discovery. Player-facing copy now uses saved Custom Solo Side Quests, saved-quest/account language, and Host details while preserving existing verifier, lifecycle, Community Solo, and Multiplayer behavior. Proof: targeted phrase sweep now only leaves code/class/API-mobile/admin internals; `pnpm lint -- src/app/account/custom-side-quests/page.tsx src/app/challenges/community/page.tsx 'src/app/challenges/community/[id]/page.tsx' src/app/groupquests/public/page.tsx 'src/app/groupquests/[id]/page.tsx' src/components/group-quest-draft-builder.tsx`; `pnpm build`; committed/pushed `ea0ac3f`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-asddjmlj3-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned expected signed-out `307` for production/deploy `/account/custom-side-quests?savedQuestLanguageSmoke=20260611`, 200 Public Multiplayer with `view more from a host` and no `host shelf`, 200 seeded Multiplayer detail with `Host details` and no old `host context`/`host shelf`, and Community Solo browse with old `Custom Side Quest library` phrase absent; proof doc `docs/SQC_WEB_SAVED_QUEST_LANGUAGE_POLISH_2026-06-11.md`.
  - 2026-06-11 support/report polish slice: continued support/report UX parity by making `/support` feel like an SQC-quality reporting flow instead of a generic contact page. Added a safe-report checklist for proof/account/community reports, SQC-styled detail chips, and a clear report-context note when Community Solo or Multiplayer report links prefill quest/host details, while preserving signed-in account threads and signed-out mail fallback. Proof: `pnpm lint -- src/app/support/page.tsx src/components/support-contact-form.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `208dc88`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-5wu39oex5-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/support` with `safe report`/`Quest or table`, 200 report-context support URL with seeded quest/host details, and 200 Community Solo report links intact; proof doc `docs/SQC_WEB_SUPPORT_REPORT_POLISH_2026-06-11.md`.
  - 2026-06-11 Multiplayer edit-flow polish slice: continued Multiplayer lifecycle/host UX parity by making the signed-in host edit flow feel deliberate instead of a loose settings form. Added a three-step host edit guide, clearer `Quest stack and proof settings` wording, Official Solo / Custom Solo labels in the stack, richer edit preview facts for proof window/provider/share URL, and a `Ready to update this table?` save panel. Also removed remaining visible mobile-comparison helper copy from Multiplayer edit and Custom Solo exact-proof controls. Proof: `pnpm lint -- src/components/group-quest-edit-form.tsx src/app/account/custom-side-quests/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `2e4e90d`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-8bnqyhe77-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 seeded Multiplayer content, expected signed-out `307`/sign-in for host-only edit and Custom Solo account routes, 200 Public Multiplayer content on the deploy URL, and no old visible phrase-family matches in smoke bodies; proof doc `docs/SQC_WEB_MULTIPLAYER_EDIT_FLOW_POLISH_2026-06-11.md`.
  - 2026-06-11 Public Multiplayer discovery polish slice: continued Multiplayer create/join/proof/leaderboard UX parity by upgrading `/groupquests/public` browse cards. Public discovery now uses an SQC-styled `Find a table` panel, each listing exposes a visible `Rule preview` for provider/clock/window before click-through, and inspect/join/host-browse actions sit in a deliberate `Next step` panel while preserving privacy and join behavior. Proof: `pnpm lint -- src/app/groupquests/public/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `f92df82`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-pu9c9qape-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy Public Multiplayer with `Find a table`/`Rule preview`/`Next step`, 200 seeded Multiplayer detail/proof-check content, and no old visible phrase-family matches; proof doc `docs/SQC_WEB_PUBLIC_MULTIPLAYER_DISCOVERY_POLISH_2026-06-11.md`.
  - 2026-06-11 Multiplayer leaderboard receipt polish slice: continued Multiplayer create/join/proof/leaderboard UX parity by polishing the signed-in accepted-participant leaderboard area. The leaderboard is now framed as `Leaderboard and proof receipts`, summary cards expose current leader/your run/full clears at a glance, expanded rows open into clearer player receipt trails, and the victory scroll now uses the actual Multiplayer Side Quest name instead of a hard-coded starter table name. Proof: `pnpm lint -- 'src/app/groupquests/[id]/page.tsx' src/components/group-quest-leaderboard.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `088d91a`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-hdzk810yg-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy seeded Multiplayer detail, 200 Public Multiplayer content, and 200 Community Solo content; proof doc `docs/SQC_WEB_MULTIPLAYER_LEADERBOARD_RECEIPT_POLISH_2026-06-11.md`.

  - 2026-06-11 Community Solo coat reassignment slice: Andreas requested replacing the old repeated coat on existing community-created Side Quests with randomized coats from the newly created pool and confirming pool quality. QA reviewed the generated random pool, kept 12 approved no-text/generated coat assets, excluded the weak checker-background candidate `custom-coat-11.png` plus old generic/custom variants from future assignment, and applied Clerk metadata updates so all existing public/published community quests use approved random-pool coats. Follow-up smokes caught stale rendered seed/fallback paths and then live `/badges/custom/custom-side-quest-crest.png` refs, so display sanitization now maps legacy/stale custom badge metadata and saved Multiplayer snapshots to deterministic approved random-pool coats, and custom public proof links use sanitized coat art. Proof: random-pool contact sheet QA pass except excluded `custom-coat-11.png`; dry-run found 47 quests across 35 users; apply updated 47 quests; verification scan found 47 public/published community quests, 0 invalid/old coat paths, and distribution across all 12 approved coats; `pnpm lint`; `pnpm lint -- src/lib/community-side-quests.ts`; `pnpm build`; `pnpm lint -- src/lib/custom-side-quests.ts 'src/app/groupquests/[id]/page.tsx' src/lib/proof-share.ts`; `pnpm build`; committed/pushed `d61d162`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-ch7zqr00n-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; final live smoke returned 200 for Community Solo browse/detail, seeded Multiplayer detail, and Public Multiplayer, with no old generic `/badges/custom/custom-side-quest-crest.png`, old `/badges/custom/custom-coat-*.png`, or excluded random `custom-coat-11.png` refs; proof doc `docs/SQC_WEB_COMMUNITY_COAT_REASSIGNMENT_DEPLOY_2026-06-11.md`.
  - 2026-06-11 official Solo lifecycle next-step polish slice: continued lifecycle-controls UX parity by replacing the bare official Solo detail hero action row with an SQC-styled `Next step` panel. Signed-out, username-missing, active-run, and ready-to-start states now explain what happens next; start/check/deactivate controls stay on the same verifier/lifecycle paths; and run setup facts summarize proof source, verifier path, and reward. Proof: `pnpm lint -- 'src/app/challenges/[id]/page.tsx' src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `581cb27`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-6soxpnktv-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/challenges/finish-any-game?lifecycleNextStepSmoke=20260611c` with `Next step`/`Proof source`, 200 Solo discovery, and 200 Public Multiplayer content; proof doc `docs/SQC_WEB_OFFICIAL_SOLO_LIFECYCLE_NEXT_STEP_POLISH_2026-06-11.md`.
  - 2026-06-11 Custom Solo proof-choice polish slice: continued proof submission/check UX parity by moving active Custom Solo checks out of the generic saved-quest action row and into SQC-styled proof-choice cards. Active Custom Solo Side Quests now present `Fastest check` for the latest public Lichess/Chess.com game and `Specific game` for a known game link/ID, while pause/reset lifecycle controls sit in a separate calm run-control row. Proof: `pnpm lint -- src/app/account/custom-side-quests/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `e5fef6e`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-7meimrf3m-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned signed-out sign-in content for production/deploy `/account/custom-side-quests?customProofChoiceSmoke=20260611`, 200 Community Solo content, and 200 Public Multiplayer content; proof doc `docs/SQC_WEB_CUSTOM_SOLO_PROOF_CHOICE_POLISH_2026-06-11.md`.
  - 2026-06-11 home active-run panel polish slice: continued mobile-vs-web standalone parity by turning the signed-in home Active Solo Side Quest area from paragraph guidance into SQC-styled next-step cards. The home card now separates `Fastest check` from `Run details`, adapts copy for no quest / missing chess username / official Solo / Custom Solo states, and keeps mobile stacking tidy while preserving existing verifier and lifecycle paths. Proof: `pnpm lint -- src/app/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `52cb05f`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-qtwvsso27-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy home, 200 Solo discovery, and 200 Public Multiplayer content; proof doc `docs/SQC_WEB_HOME_ACTIVE_RUN_PANEL_POLISH_2026-06-11.md`.
  - 2026-06-11 official Solo discovery polish slice: continued Solo discovery UX parity by upgrading `/challenges` from a plain filter/card grid into a more guided official-run finder. The finder now has player-facing intent copy, quick chips for good first runs / active run / big point targets, visible result counts, and official quest cards with compact `Run preview` receipt hints plus clear next-step labels while preserving existing official quest routes and verifier behavior. Proof: `pnpm lint -- src/components/challenge-deck-browser.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `2c8200e`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-at2ng1fsv-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/challenges` with `Official Solo finder`/`Run preview`/`Inspect quest`, 200 official Solo detail content, and 200 Public Multiplayer content; proof doc `docs/SQC_WEB_OFFICIAL_SOLO_DISCOVERY_POLISH_2026-06-11.md`.
  - 2026-06-11 badges Coat Room polish slice: continued Trophy Cabinet/proof-receipt UX parity by making `/badges` feel like a standalone SQC Coat of Arms room instead of a plain heraldry index. Added a `Coat room guide` for choose/play/receipt flow, upgraded each coat card with earned/ready status plus difficulty/points/category run preview and a clear next-step cue, and preserved existing quest/proof routes. Proof: `pnpm lint -- src/app/badges/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `aab8cd2`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-eksxkcwgp-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/badges` with `Coat room guide`/`Ready to earn`/`Browse Solo Side Quests`, 200 Solo discovery content, and 200 Public Multiplayer content; proof doc `docs/SQC_WEB_BADGES_COAT_ROOM_POLISH_2026-06-11.md`.
  - 2026-06-11 community Custom coat pool polish slice: continued visible polish for Custom Solo / Community Solo by replacing the small clean placeholder coat pool with 48 dedicated community coat-of-arms assets. New Custom Solo Side Quests now draw from `/badges/custom/community/community-coat-01.png` through `community-coat-48.png`, existing/missing custom badge metadata still maps deterministically to approved art, and the community reroll script uses the same pool. Proof: contact-sheet QA found no visible text/letters/numbers or broken renders; local PNG inspection confirmed 48 `512x512 RGBA` assets with alpha; `pnpm lint -- src/lib/custom-side-quests.ts scripts/randomize-community-coats.mjs`; `pnpm build`; `pnpm quest:release-gate`; committed/pushed `d0aa1a8`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-gjezm8gn8-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 Community Solo/account content and 200 image/png for representative community coat assets; proof doc `docs/SQC_WEB_COMMUNITY_CUSTOM_COAT_POOL_POLISH_2026-06-11.md`.
  - 2026-06-11 Multiplayer landing choice polish slice: continued Multiplayer create/join/proof UX parity by making `/groupquests` a clearer SQC table-entry hub. Signed-out copy now uses polished host/join/proof/leaderboard language, signed-in users get a `Choose your table` panel for public tables, private host-code tables, and hosting their own Multiplayer Side Quest, and the command-center copy now frames active tables/public runs/finished proof receipts as deliberate next steps while preserving existing create/join/private-code/public-discovery routes. Proof: `pnpm lint -- src/app/groupquests/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `4d8580e`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-mbufr0dx7-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/groupquests?landingChoiceSmoke=20260611` with `Choose your table`/`Chess dares for a shared table`, plus 200 Public Multiplayer and seeded Multiplayer detail content; proof doc `docs/SQC_WEB_MULTIPLAYER_LANDING_CHOICE_POLISH_2026-06-11.md`.
  - 2026-06-11 profile readiness polish slice: continued account-readiness/mobile-standalone parity by turning `/profile` from a plain edit form into an SQC runner-profile proof setup room. The page now uses product-facing runner/profile language, a `Ready to run` checklist for public runner card/proof source/next run, and clearer signed-out steps for sign in → add Lichess or Chess.com → browse Solo Side Quests while preserving existing profile save behavior. Proof: `pnpm lint -- src/app/profile/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `c683b8b`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-7uerv8l7o-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/profile?profileReadinessSmoke=20260611` with `Runner profile`/`Before your first run`, 200 signed-out account sign-in content, and 200 Solo discovery content; proof doc `docs/SQC_WEB_PROFILE_READINESS_POLISH_2026-06-11.md`.
  - 2026-06-11 public proof receipt polish slice: continued Trophy Cabinet/proof-receipt UX parity by making `/proof/[token]` feel like a finished SQC receipt instead of a plain share page. Public proof links now show runner/proof source/reward/completed facts in the hero, explain the verifier-backed proof board, and group sharing plus follow-on play into a `Next step` panel with Official/Community Solo and Multiplayer paths while preserving proof token/image behavior. Proof: `pnpm lint -- 'src/app/proof/[token]/page.tsx' src/app/globals.css` (CSS ignored-file warning only); first `pnpm build` caught a provider literal mismatch and the second passed; committed/pushed `5f27788`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-okpoe5724-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/proof/preview-finish-any-game?publicProofReceiptSmoke=20260611` with receipt/next-step copy, 200 image/png for the public proof image, and 200 Public Multiplayer content; proof doc `docs/SQC_WEB_PUBLIC_PROOF_RECEIPT_POLISH_2026-06-11.md`.
  - 2026-06-11 result receipt language polish slice: continued proof-receipt UX parity by cleaning internal/product-review phrasing from the signed-in `/result` receipt surface. Passed, pending, and failed receipt panels now use player-facing next-run language, and the old duplicate `Browse quests` buttons are split into clear Solo and Multiplayer next steps while preserving result/proof/share behavior. Proof: `pnpm lint -- src/app/result/page.tsx`; `pnpm build`; committed/pushed `d7a656f`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-51jwsv5ed-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/result?resultReceiptSmoke=20260611` with receipt guide/Solo/Multiplayer copy and no old internal/duplicate-copy phrases, plus 200 Public Multiplayer content; proof doc `docs/SQC_WEB_RESULT_RECEIPT_LANGUAGE_POLISH_2026-06-11.md`.
  - 2026-06-11 official Scoreboard receipt polish slice: continued Multiplayer leaderboard/proof-receipt UX parity by making `/scoreboard` feel like an SQC leaderboard hall instead of a plain archive. Added a `How the hall works` guide for join/check/receipt flow, and upgraded official leaderboard rows with visible run facts, podium context, and deliberate open/final-receipt CTAs while preserving existing official Multiplayer table links and verifier behavior. Proof: `pnpm lint -- src/app/scoreboard/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `fe5fd6c`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-ftl33vzn4-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/scoreboard?scoreboardReceiptSmoke=20260611` with `How the hall works`/`Pick a table, prove games, keep the receipt`, 200 scoreboard with `View final receipt`, and 200 Public Multiplayer content; proof doc `docs/SQC_WEB_OFFICIAL_SCOREBOARD_RECEIPT_POLISH_2026-06-11.md`.
  - 2026-06-11 proof guide/verifier board polish slice: continued proof-submission/readiness UX parity by making `/rules`, `/verifiers`, and `/path` more player-facing. The proof guide now explains `Fastest check`, `Specific game`, and `Honest receipt` choices instead of roadmap/adapter/product-review wording; the verifier board now has a `How to use this board` guide, `What SQC checks` receipt cards, and a non-duplicated next step; and the quest-picks page no longer exposes internal product phrasing. Proof: `pnpm lint -- src/app/rules/page.tsx src/app/verifiers/page.tsx src/app/path/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; `pnpm lint -- src/app/rules/page.tsx src/app/verifiers/page.tsx src/app/path/page.tsx`; committed/pushed `8947c6b`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-87tykfut7-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy verifier board with `How to use this board`/`What SQC checks`, 200 rules with `Proof choices`, and 200 quest picks with internal product phrasing absent; proof doc `docs/SQC_WEB_PROOF_GUIDE_VERIFIER_POLISH_2026-06-11.md`.
  - 2026-06-11 invite/share language polish slice: continued public standalone/mobile-parity polish by cleaning internal/product-review language from invite surfaces. `/share-kit` now describes the player action of picking a Side Quest, copying the invite, and sending the same rules/reward/coat/proof path; `/dare/[id]` removes page-review phrasing and labels the account CTA as `Open My Side Quests`; verifier metadata/shared pending labels use player-facing checker/rule-promise language instead of `next adapter`, `product-ready`, or `product contract`. Proof: `pnpm lint -- src/app/share-kit/page.tsx 'src/app/dare/[id]/page.tsx' src/app/verifiers/page.tsx src/lib/verifier-status.ts`; `pnpm build`; committed/pushed `78ad7c3`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-7xiwebs4k-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy Share Kit, 200 friend-quest dare, and 200 verifier board with old internal phrase families absent; proof doc `docs/SQC_WEB_INVITE_LANGUAGE_POLISH_2026-06-11.md`.
  - 2026-06-11 connect proof-source polish slice: continued account-readiness/proof-submission parity by making `/connect` feel like an SQC proof setup room instead of a plain username form. The page now uses `Proof source`/`Ready to run` language, shows saved Lichess/Chess.com sources in status chips, and guides runners through choose source → start a run → check proof while preserving existing username save behavior and verifier paths. Proof: `pnpm lint -- src/app/connect/page.tsx`; `pnpm build`; committed/pushed `07c6048`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-3l8l5u9i4-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/connect?connectProofSourceSmoke=20260611` with `Proof source` setup copy, 200 Solo discovery content, and 200 Public Multiplayer content; proof doc `docs/SQC_WEB_CONNECT_PROOF_SOURCE_POLISH_2026-06-11.md`.
  - 2026-06-11 random chooser polish slice: continued standalone/mobile-parity visible UX polish by upgrading `/random` from a loose roulette action row into a clearer SQC run chooser. The random hero now has a `Next step` panel for spin/start/share choices, friend-quest sharing uses cleaner player-facing labels, and the selected quest exposes an inline `Rule preview` before sharing while preserving official quest routes and verifier behavior. Proof: `pnpm lint -- src/components/challenge-roulette.tsx src/app/random/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `3463f6a`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-6k22pjsfj-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/random?randomChooserSmoke=20260611` with `Next step`/`Rule preview`/`Start this Solo quest`, plus 200 Solo discovery and Public Multiplayer content; proof doc `docs/SQC_WEB_RANDOM_CHOOSER_POLISH_2026-06-11.md`.
  - 2026-06-11 auth account setup polish slice: continued account-readiness/mobile-standalone parity by upgrading signed-out `/sign-in` and `/sign-up` from plain auth copy into SQC-styled account setup surfaces. Sign-in now shows an `SQC account` run guide for opening a run, checking public games, and keeping receipts; sign-up now shows a new-runner setup guide plus readiness chips for public username/proof checks/receipts, while preserving Clerk auth behavior and redirects. Proof: `pnpm lint -- 'src/app/sign-in/[[...sign-in]]/page.tsx' 'src/app/sign-up/[[...sign-up]]/page.tsx' src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `592ffe2`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-4eo5bh7yi-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy auth routes with `SQC account`/`Start your SQC account`/`Proof checks from public games`, plus 200 Solo discovery and Public Multiplayer content; proof doc `docs/SQC_WEB_AUTH_ACCOUNT_SETUP_POLISH_2026-06-11.md`.
  - 2026-06-11 Multiplayer sample room polish slice: continued Multiplayer create/join/proof UX parity by upgrading the legacy `/groupquests/gq_demo_no_castle_01` sample room from a stale prototype-feeling page into a clearer SQC sample table. The page now labels itself as a sample, adds a `Next step` panel to live public tables, replaces unsupported fake proof buttons with real live-table/proof-rules links, expands proof requirements with player-facing explanations, and explains latest-game checks plus leaderboard receipt trails while preserving the route. Proof: `pnpm lint -- src/app/groupquests/gq_demo_no_castle_01/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `d61cc8a`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-e3pw5hkku-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy sample room with `Use the live table list`/`Proof choices`/`Join a live table`, and 200 Public Multiplayer content; proof doc `docs/SQC_WEB_MULTIPLAYER_SAMPLE_ROOM_POLISH_2026-06-11.md`.
  - 2026-06-11 missing-page next-step polish slice: continued standalone/mobile-parity visible UX polish by replacing the generic root missing-page experience with an SQC-styled recovery route. Old, mistyped, private, or cleaned-up links now keep a correct 404 status while showing `Lost Side Quest`, clear tavern-wall copy, and a `Next step` panel back to Solo Side Quests, Public Multiplayer, or Support. Proof: `pnpm lint -- src/app/not-found.tsx`; `pnpm build`; committed/pushed `da056cd`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-q9f27mmra-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned expected 404 for production/deploy missing routes with `Lost Side Quest`/`Pick up the run from a safe room`, plus 200 Solo discovery and Public Multiplayer content; proof doc `docs/SQC_WEB_MISSING_PAGE_NEXT_STEP_POLISH_2026-06-11.md`.
  - 2026-06-11 terms/support guardrail polish slice: continued standalone/account-readiness polish by making `/terms` less like a plain legal wall and more like an SQC support-adjacent guardrail page. Added a `Before you run` quick guide for proof source, public receipt visibility, and support review expectations, then reworked the final help card into a clearer Support / Verifier Board next step while preserving the terms content. Proof: `pnpm lint -- src/app/terms/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; committed/pushed `b763253`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-b185u6urn-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/terms?termsSupportSmoke=20260611` with `Keep the quest fair`/`Need a human review`, plus 200 Support and Verifier Board content; proof doc `docs/SQC_WEB_TERMS_SUPPORT_POLISH_2026-06-11.md`.

  - 2026-06-12 Challenges page visual correction slice: Andreas reported that `/challenges` looked terrible outside the official Side Quest list/filter and that the Community Solo listing implementation looked especially bad. Simplified `/challenges` so the official Solo finder/list remains the main surface, removed the extra redundant recommendation/streamer/quick-proof card sections, replaced the top mode cards with a compact hero/action strip, and rebuilt `/challenges/community` from bulky nested cards into a cleaner public-board row layout with simpler filters, clearer actions, and less visual clutter. Proof: `pnpm lint`; `pnpm build`; local Playwright/Chrome smoke returned 200 for `/challenges` and `/challenges/community` with screenshots captured in `artifacts/challenges-redesign-2026-06-12/`; committed/pushed `f224460`; `pnpm deploy:prod` including release gate and deploy guard; production deploy `https://cc-clmw2q51k-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production and deploy URLs for `/challenges` and `/challenges/community` with expected new copy/classes; best-effort Vercel logs sample showed no 5xx/error lines.
  - 2026-06-12 Community Solo official-card correction slice: Andreas rejected the flat Community Solo listing and explicitly reminded that Community Solo must use the high-quality community coat-of-arms pool already created. Changed `/challenges/community` from row cards to official-style `challenge-card` cards with large uncropped community coats, rule preview, and inspect/start actions; fixed `listPublicCommunitySideQuests` so coats stay stable per quest id from the community-only pool instead of being reassigned by list index/sort; local rendered Playwright QA showed 47 `.community-side-quest-official-card` cards, sampled optimized image URLs all from `/badges/custom/community/community-coat-XX.png`, rendered coats about 142x142, and zero old `custom-coat-knight-gold` fallbacks. Proof: `pnpm lint -- src/app/challenges/community/page.tsx src/lib/community-side-quests.ts src/app/globals.css` (CSS ignored warning only); `pnpm build`; local screenshots in `artifacts/community-redesign-2026-06-12/`; committed/pushed `9fbbf10`; `pnpm deploy:prod` including quest release gate and production deploy guard; production deploy `https://cc-plryau875-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production and deploy `/challenges/community`, confirmed `community-side-quest-official-card` present, 188 `community-coat-XX.png` refs in HTML, and `custom-coat-knight-gold` absent; best-effort Vercel error sample was empty.
  - 2026-06-13 accepted UX review fixes slice: implemented Andreas-approved SQC review fixes without adding mobile bottom nav and without removing Google/Facebook auth. Mobile now has a top tracker nav plus signed-out Browse Solo/Browse Multiplayer CTAs, offline fallback mode always shows an explicit offline banner, website primary nav exposes Multiplayer and Leaderboard, stale/expired official Multiplayer pages are read-only archives instead of join/proof surfaces, Scoreboard empty-current copy clarifies archived official weeks, and public proof OG images are wide/social-ready 1200x630 while preserving the proof page receipt flow. Proof before deploy: `pnpm exec eslint apps/mobile/App.tsx src/components/site-nav.tsx 'src/app/groupquests/[id]/page.tsx' src/app/scoreboard/page.tsx 'src/app/proof/[token]/page.tsx' 'src/app/api/og/proof/[token]/route.tsx'`; `pnpm --dir apps/mobile typecheck`; `pnpm lint`; `pnpm build`.
...
- [x] 24h SQC website parity sprint: close missing website functionality compared to mobile app while preserving current website look and feel.
  ...
  - 2026-06-09 custom coats slice: Andreas approved simple random assignment for custom Side Quest coats. Added 13 generated no-text custom coat assets, combined them with existing custom coat variants for a 26-item random pool, and changed custom quest creation to save a random coat instead of always using the fallback crest. Mobile synced custom quests now preserve non-fallback custom badge URLs instead of collapsing them to the crest. Proof: `pnpm lint -- src/lib/custom-side-quests.ts apps/mobile/App.tsx`; `pnpm --dir apps/mobile typecheck`; `pnpm build`.
  - 2026-06-09 badge quality slice: refreshed `Any Game Counts` (`finish-any-game`) coat of arms because Andreas flagged the current starter quest badge as too simplistic for most players' first Side Quest. Replaced web and mobile mirrored `proof-loop-test-badge.png` assets with a premium no-text/no-letter starter crest. Proof doc: `docs/SQC_ANY_GAME_COUNTS_COAT_REFRESH_2026-06-09.md`.
  - 2026-06-10 badge transparency/quality correction slice: Andreas flagged the refreshed `Any Game Counts` coat as still lower quality than the other coats and not transparent. Replaced web and mobile mirrored `proof-loop-test-badge.png` with a higher-quality SQC-style no-text/no-letter crest, verified real RGBA transparency outside the coat before redeploy, and deployed production `https://cc-iltevfrl2-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`. Proof: `pnpm build`; `pnpm quest:release-gate`; live smoke 200 for `/challenges/finish-any-game`, `/badges`, and the badge asset; live PNG verified `1024x1024 RGBA`, alpha extrema `(0, 255)`, transparent corners `[0, 0, 0, 0, 0, 0]`; proof doc `docs/SQC_ANY_GAME_COUNTS_TRANSPARENT_QUALITY_FIX_2026-06-10.md`.
  - 2026-06-10 ornate Coffee House-level correction slice: Andreas rejected the prior fix because the Coffee House / `Knights Before Coffee` coat is much more detailed and decorated. Replaced the web/mobile mirrored `proof-loop-test-badge.png` again with a denser ornate heraldic crest using Coffee House as the quality bar: richer baroque filigree, jewels, scrollwork, shield subdivisions, ribbons, and completed-game/check motif while keeping no text/letters/numbers and real transparency. Proof: final strict visual QC approved against Coffee House; local PNG verified `1024x1024 RGBA`, alpha extrema `(0, 255)`, transparent corners `[0, 0, 0, 0, 0, 0]`, `49.27%` transparent pixels; `pnpm build`; `pnpm quest:release-gate`; proof doc `docs/SQC_ANY_GAME_COUNTS_ORNATE_COFFEEHOUSE_FIX_2026-06-10.md`.
  - 2026-06-09 slice: shipped website Custom Solo creation, reversible lifecycle management, explicit proof controls on `/account/custom-side-quests`, account Trophy Cabinet proof receipt/share actions, and joined Multiplayer proof controls on `/groupquests/[id]` using existing site cards/forms: starter proof conditions for result/opening/move/piece-state rules, private draft/private published/public publishing states, archive/restore controls, start/check/deactivate/reset proof controls for published custom recipes, safe proof receipts/rule summaries, canonical completed-official proof links/social-copy/download actions, Multiplayer `Check latest game` refresh controls, per-quest Multiplayer receipt cards, and Community Solo revalidation. Also removed the duplicate legacy root `middleware.ts` so the existing Clerk-aware `src/proxy.ts` remains the single backslash normalizer/request boundary for signed-in account routes and Next 16/Vercel proxy artifacts. Proof: `pnpm lint -- src/proxy.ts src/app/account/custom-side-quests/page.tsx`; `pnpm lint -- 'src/app/groupquests/[id]/page.tsx' src/components/group-quest-proof-controls.tsx`; `pnpm build`; production deploy `https://cc-ifdqd08tq-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live signed-out smoke for `/account/custom-side-quests`, `/support%5C`, and `/challenges/community`; proof-controls deploy `https://cc-4b39mjkit-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com` with live smoke for `/account/custom-side-quests` and `/challenges/community`; proof-receipt deploy `https://cc-kjdlo6tl5-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com` with live smoke for `/account` and `/proof/preview-finish-any-game`; Multiplayer proof-controls deploy `https://cc-dnum340g1-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com` with live smoke for `/groupquests` and `/groupquests/public`.
  - 2026-06-09 custom edit slice: added safe website edit parity for saved Custom Solo Side Quests on `/account/custom-side-quests`: each library card can reopen the website builder with title/summary/state/rule defaults, saves keep the same custom ID, badge, proof history, and public URL, edits can move recipes between draft/private/public states, and multi-condition mobile-created recipes preserve their existing rule stack while allowing metadata/state edits instead of collapsing raw config. Proof: `pnpm lint -- src/app/account/custom-side-quests/page.tsx`; `pnpm build`; production deploy `https://cc-p6xayf5d0-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned `307` to `/sign-in` for signed-out `/account/custom-side-quests?customEditSmoke=20260609`, 200 for `/groupquests?customEditSmoke=20260609`, and Community Solo browse still exposed report/start account affordances.
  - 2026-06-09 submitted proof slice: active official Side Quest detail pages now match mobile's specific-game submitted-proof path with a website form for Lichess game IDs or Chess.com game URLs, kept beside the existing latest-game refresh/receipt panel and limited to official quests because Custom Solo specific-game proof remains backend-blocked in favor of latest-game checks. Proof: `pnpm lint -- 'src/app/challenges/[id]/page.tsx'`; `pnpm build`; first `pnpm deploy:prod` release gates passed and production guard correctly blocked deploy until the focused code/doc commit was made; committed/pushed `3dc09a5`; production deploy `https://cc-cqxaxoi3g-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for `/challenges/finish-any-game?submittedProofSmoke=20260609` and `/challenges?submittedProofSmoke=20260609` with SQC challenge content.
  - 2026-06-09 private Multiplayer invite slice: closed the website gap with mobile private-code joining by adding a signed-in `Join by host code` lane on `/groupquests`, a private invite lookup API, invite-code handoff into `/groupquests/[id]`, and server enforcement that `private-key` Multiplayer joins include the matching code while public/unlisted joins keep their existing paths. Proof: `pnpm lint -- src/app/groupquests/page.tsx 'src/app/groupquests/[id]/page.tsx' src/components/group-quest-accept-modal.tsx src/components/group-quest-invite-key-join.tsx 'src/app/api/groupquests/[id]/join/route.ts' src/app/api/groupquests/invite/lookup/route.ts`; `pnpm build`; committed/pushed `ce75303`; production deploy `https://cc-1pmk9ftzx-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for `/groupquests?privateInviteSmoke=20260609` with the private invite form and 401 `sign_in_required` for signed-out `/api/groupquests/invite/lookup`.
  - 2026-06-09 Community Multiplayer discovery slice: closed the website gap with mobile public Multiplayer search/status/host context by adding `/groupquests/public` search, open/all/finished filters, host-shelf filtering, host labels, `More by host` links, and non-official detail/report/support host-context cards while keeping private invite-only tables, participant emails, and private account metadata hidden. Proof: `pnpm lint -- src/app/groupquests/public/page.tsx 'src/app/groupquests/[id]/page.tsx'`; `pnpm build`; committed/pushed `cbcd26a`; production deploy `https://cc-4t555wdxj-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for `/groupquests/public?multiDiscoverySmoke=20260609` and `/groupquests/seed-public-sqcseed11-11?multiDiscoverySmoke=20260609` with discovery filters, host context, `More by host`, and `Report Side Quest`.
  - 2026-06-09 private Multiplayer host-code creation slice: closed the website gap with mobile private-table creation/management by adding `Private host code` invite mode to `/groupquests/create` and `/groupquests/[id]/edit`, sanitized host-code editing, private invite-link previews, and host-only private detail share links that include the invite code while preserving public/unlisted behavior and existing join enforcement. Proof: `pnpm lint -- src/components/group-quest-draft-builder.tsx src/components/group-quest-edit-form.tsx 'src/app/groupquests/[id]/page.tsx'`; `pnpm build`; committed/pushed `7ef0788` plus proof-doc commit `facc5a1`; production deploy `https://cc-gxklv0lmw-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for `/groupquests?hostCodeCreateSmoke=20260609b`, signed-out `/groupquests/create?hostCodeCreateSmoke=20260609b` resolved to sign-in, and 200 for `/groupquests/public?hostCodeCreateSmoke=20260609b` plus seeded public detail with Multiplayer content.
  - 2026-06-09 Multiplayer host participant-management slice: closed the website gap with mobile host player removal by adding host-only remove-player controls inside `/groupquests/[id]` leaderboard rows and a matching web API route that reuses the existing server-side participant removal/storage path. Hosts cannot remove themselves from this control, non-hosts are rejected, and participant emails/private account metadata stay hidden. Proof: `pnpm lint -- 'src/app/groupquests/[id]/page.tsx' src/components/group-quest-leaderboard.tsx 'src/app/api/groupquests/[id]/remove-participant/route.ts'`; `pnpm build`; committed/pushed `1de39cb`; production deploy `https://cc-nsjnvvv8u-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for `/groupquests/seed-public-sqcseed11-11?hostRemoveSmoke=20260609`, 401 `sign_in_required` for signed-out `/api/groupquests/seed-public-sqcseed11-11/remove-participant`, and 200 for `/groupquests/public?hostRemoveSmoke=20260609`.
  - 2026-06-09 Community Solo analytics slice: closed the queued website observability gap by adding privacy-safe first-party analytics events for Community Solo browse views, detail views, creator-filter views/clicks, report/support clicks, and account handoff clicks. Payloads use event type, current path, optional public quest ID, status, source, and device type only; no raw custom quest config or private metadata is sent. Proof: `pnpm lint -- src/components/analytics/community-solo-analytics.tsx src/lib/analytics.ts src/app/challenges/community/page.tsx 'src/app/challenges/community/[id]/page.tsx'`; `pnpm build`; committed/pushed `ef2a013`; production deploy `https://cc-eaulu9w7l-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for `/challenges/community?analyticsSmoke=20260609`, 200 for `/challenges/community/seed-opening-hipster-32-1?analyticsSmoke=20260609`, and 200 `{\"ok\":true,\"stored\":false}` for signed-out `/api/analytics` with `community_solo_browse`.
  - 2026-06-09 account Multiplayer trophy slice: closed the website Trophy Cabinet data gap where the account page had a Multiplayer podium-scroll lane but never populated it. Finished related Multiplayer Side Quests now show top-three positive-score podium scroll cards with placement seals, verified-count context, dates, and links back to the table, matching the mobile account payload behavior without exposing participant emails/private metadata. Proof: `pnpm lint -- src/app/account/page.tsx`; `pnpm build`; committed/pushed `553ef6f`; production deploy `https://cc-g53hjmsu5-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 307 to `/sign-in` for signed-out `/account?trophySmoke=20260609`, 307 on the deploy URL for the same account route, and 200 for `/groupquests/public?trophySmoke=20260609` with Public Multiplayer/Join/Create content.
  - 2026-06-09 support/report parity slice: closed the website support gap versus mobile Help & Support by adding a signed-in account-attached support thread to `/support`, a Clerk-backed `/api/support` web endpoint, and report-link context prefill for Community Solo/Multiplayer while preserving the signed-out mailto fallback. Initial production smoke caught `/api/support` missing from the Clerk proxy matcher; fixed immediately so signed-out API calls return the intended 401 instead of a middleware 500. Proof: `pnpm lint -- src/proxy.ts src/app/support/page.tsx src/components/support-contact-form.tsx src/app/api/support/route.ts`; `pnpm build`; committed/pushed `d69ffed` and `74c4ca1`; production deploy `https://cc-lrhaqguy2-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for `/support?supportThreadSmoke=20260609&topic=community-multiplayer&quest=seed-public-sqcseed11-11&host=SQC%20Seed` with report context and 401 JSON for signed-out `POST /api/support`.
  - 2026-06-09 Custom Solo duplicate slice: added mobile-equivalent Duplicate management to website `/account/custom-side-quests` cards, preserving safe rule/config, badge, state, and summary while assigning a fresh custom ID/timestamps and staying under the existing 8-recipe metadata guardrail; no destructive delete was added. Proof: `pnpm lint -- src/app/account/custom-side-quests/page.tsx`; `pnpm build`; committed/pushed `1ca9ff3`; production deploy `https://cc-88d24eu02-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 307 to `/sign-in` for signed-out `/account/custom-side-quests?duplicateSmoke=20260609` and 200 for `/challenges/community?duplicateSmoke=20260609` with Community Solo/report/account-start affordances.
  - 2026-06-09 Custom Solo delete slice: added the mobile-supported Delete-from-library semantics to website `/account/custom-side-quests` behind an explicit disclosure/confirm step; deletion removes the signed-in user's custom recipe, clears it if active, revalidates account/community pages, and leaves existing Multiplayer lineups on saved safe snapshots. Proof: `pnpm lint -- src/app/account/custom-side-quests/page.tsx`; `pnpm build`; committed/pushed `4ef0f29`; production deploy `https://cc-c3zfvbf6r-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 307 to `/sign-in` for signed-out `/account/custom-side-quests?deleteSmoke=20260609` and 200 for `/challenges/community?deleteSmoke=20260609` with Community Solo/report/account-start affordances.
  - 2026-06-09 Multiplayer custom-stack slice: added website parity for mobile custom Multiplayer lineups: `/groupquests/create` and host `/groupquests/[id]/edit` now show the host's published launch-ready Custom Solo recipes alongside official quests, preserve saved custom snapshots during edits, and the web create/edit APIs validate ownership/lifecycle/rule config before saving snapshots. Proof: `pnpm lint -- src/app/groupquests/create/page.tsx src/components/group-quest-draft-builder.tsx 'src/app/groupquests/[id]/edit/page.tsx' src/components/group-quest-edit-form.tsx src/app/api/groupquests/route.ts 'src/app/api/groupquests/[id]/route.ts'`; `pnpm build`; committed/pushed `0de28e4`; production deploy `https://cc-ro7352hw0-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 307 to `/sign-in` for signed-out `/groupquests/create?customStackSmoke=20260609`, 200 for `/groupquests/public?customStackSmoke=20260609`, and 401 `sign_in_required` for signed-out `POST /api/groupquests`.
  - 2026-06-10 Custom Solo two-condition builder slice: expanded website Custom Solo creation from one starter condition to safe two-condition recipes with all/any logic across result/opening/move/piece-state blocks, while preserving mobile-created recipes with larger rule stacks during website metadata edits. Proof: `pnpm lint -- src/app/account/custom-side-quests/page.tsx`; `pnpm build`; committed/pushed `a031d53`; production deploy `https://cc-8yhzew92l-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 307 to `/sign-in` for signed-out `/account/custom-side-quests?twoConditionSmoke=20260610` and 200 for `/challenges/community?twoConditionSmoke=20260610` with Community Solo content.
  - 2026-06-10 Custom Solo six-condition builder slice: continued website creator parity by matching the mobile-v251 six-condition custom-rule cap on `/account/custom-side-quests`, including optional condition slots 2–6, all/any logic across the full stack, edit-prefill for up to six saved blocks, and safe preservation for larger/future rule stacks. Proof: `pnpm lint -- src/app/account/custom-side-quests/page.tsx`; `pnpm build`; committed/pushed `b8bba7d`; production deploy `https://cc-brlwcsx66-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 307 to `/sign-in` for signed-out `/account/custom-side-quests?sixConditionSmoke=20260610` and 200 for `/challenges/community?sixConditionSmoke=20260610` with Community Solo/creator/report content; proof doc `docs/SQC_WEB_CUSTOM_SIX_CONDITION_BUILDER_2026-06-10.md`.
  - 2026-06-10 Custom Solo detailed rule-controls slice: continued website creator parity by adding mobile-equivalent owner/timing/count/identity/negation controls to the six-condition Custom Solo builder, prefill for saved detailed rule fields, explicit `not moved` verifier support, and richer safe public rule summaries without exposing raw config. Proof: `pnpm lint -- src/app/account/custom-side-quests/page.tsx src/lib/custom-side-quests.ts src/lib/community-side-quests.ts`; `pnpm build`; committed/pushed `7c9fcaa`; production deploy `https://cc-3pvuuvb09-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 307 to `/sign-in` for signed-out `/account/custom-side-quests?ruleDetailSmoke=20260610`, 200 for `/challenges/community?ruleDetailSmoke=20260610`, and 200 for seeded Community Solo detail with creator/rule content; proof doc `docs/SQC_WEB_CUSTOM_RULE_DETAIL_CONTROLS_2026-06-10.md`.
  - 2026-06-10 Community Solo share-controls slice: closed the website gap where Community Solo detail copy promised share parity but the page and owner library cards had no explicit public-link/social controls. Public Community Solo detail pages now include a Share public link lane with canonical URL copy/social actions, and public Custom Solo cards on `/account/custom-side-quests` expose the same safe public quest share controls without leaking raw config/private shelf data. Proof: `pnpm lint -- src/components/share-proof-actions.tsx 'src/app/challenges/community/[id]/page.tsx' src/app/account/custom-side-quests/page.tsx`; `pnpm build`; committed/pushed `fd1b3b5`; production deploy `https://cc-155zz02sc-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for seeded Community Solo detail with share controls, 200 for `/challenges/community?shareControlsSmoke=20260610`, and signed-out `/account/custom-side-quests?shareControlsSmoke=20260610` resolved to sign-in; proof doc `docs/SQC_WEB_COMMUNITY_SOLO_SHARE_CONTROLS_2026-06-10.md`.
  - 2026-06-10 Multiplayer custom snapshot proof/detail slice: closed the website gap left after custom Multiplayer stack creation by resolving saved Custom Solo snapshots on `/groupquests/[id]` detail/proof views and passing those snapshots into the existing custom latest-game verifier during Multiplayer refresh. Custom lineup entries now keep badge/title/reward context, leaderboard proof metadata, and scoring without exposing raw custom configs. Proof: `pnpm lint -- 'src/app/groupquests/[id]/page.tsx' 'src/app/api/groupquests/[id]/refresh/route.ts'`; `pnpm build`; committed/pushed code `3beb693` plus proof docs `a791cc8`; production deploy `https://cc-bs2bkdk4i-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for seeded Multiplayer detail, 200 for public Multiplayer browse, and 401 `sign_in_required` for signed-out Multiplayer refresh API; proof doc `docs/SQC_WEB_MULTIPLAYER_CUSTOM_SNAPSHOT_PROOF_2026-06-10.md`.
  - 2026-06-10 proof-board receipts slice: closed the website proof-visual parity gap versus mobile by wiring verifier-saved chess positions into official Side Quest detail receipts and new public proof links. Completed official detail pages now show a verified final-position board when FEN/UCI evidence exists, active latest receipts show a referee board for checked positions, public proof tokens carry safe board evidence for new links, and older proof links keep the scroll fallback. Proof: `pnpm lint -- src/components/proof-position-board.tsx src/lib/proof-share.ts 'src/app/challenges/[id]/page.tsx' 'src/app/proof/[token]/page.tsx'`; `pnpm build`; committed/pushed `e530892`; production deploy `https://cc-qscm807mm-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for `/proof/preview-finish-any-game?proofBoardSmoke=20260610` with `Proof board` / `SQC proof board` / highlighted chess squares, 200 for `/challenges/finish-any-game?proofBoardSmoke=20260610`, and 200 for `/challenges?proofBoardSmoke=20260610`; proof doc `docs/SQC_WEB_PROOF_BOARD_RECEIPTS_2026-06-10.md`.
  - 2026-06-10 official Multiplayer archive slice: closed the website discovery/status gap where `/groupquests/public?status=finished` could never show finished public tables because the display guard removed ended events before filtering. Public Multiplayer finished/all filters now keep finished events eligible, Official SQC finished views use an `Official SQC Multiplayer archive` lane with final detail/leaderboard links, and private invite-only tables remain hidden. Proof: `pnpm lint -- src/app/groupquests/public/page.tsx`; `pnpm build`; committed/pushed `97a4b9f`; production deploy `https://cc-8xb4y2lnq-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for `/groupquests/public?status=finished&archiveSmoke=20260610` with `Official SQC Multiplayer archive`, 18 official finished rows, final leaderboard/detail links, and community finished rows; 200 for `/groupquests/public?status=all&archiveSmoke=20260610` with official + community all-status rows; 200 for `/groupquests/public?archiveSmoke=20260610`; proof doc `docs/SQC_WEB_OFFICIAL_MULTIPLAYER_ARCHIVE_2026-06-10.md`.
  - 2026-06-10 signed-in official Multiplayer weekly archive slice: continued mobile-v251 Official Leaderboards parity by adding `Previous official results` and `Official weekly archive` lanes to signed-in website `/groupquests`, grouping finished official SQC events by week with final detail/leaderboard links while preserving existing command-center styling and private-data boundaries. Proof: `pnpm lint -- src/app/groupquests/page.tsx`; `pnpm build`; committed/pushed `6b7a5f9`; production deploy `https://cc-eanev1k6c-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for `/groupquests?weeklyArchiveSmoke=20260610` with Multiplayer/flow content and 200 for `/groupquests/public?status=finished&weeklyArchiveSmoke=20260610` with the Official SQC archive; proof doc `docs/SQC_WEB_OFFICIAL_MULTIPLAYER_WEEKLY_ARCHIVE_2026-06-10.md`.
  - 2026-06-10 Custom Solo exact-proof control slice: continued website parity with mobile-v251 by adding a `Specific proof game` form to active Custom Solo cards on `/account/custom-side-quests`, submitting Lichess game IDs or Chess.com URLs through the same `action=submit` quest API path while preserving the existing backend safety gate for custom exact-game proof. Proof: `pnpm lint -- src/app/account/custom-side-quests/page.tsx`; `pnpm build`; committed/pushed `fb83215`; production deploy `https://cc-lfnosto6x-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 307 to `/sign-in` for signed-out `/account/custom-side-quests?customExactProofSmoke=20260610`, 200 for `/challenges/community?customExactProofSmoke=20260610` with Community Solo/creator content, and 200 for `/groupquests?customExactProofSmoke=20260610` with Multiplayer content; proof doc `docs/SQC_WEB_CUSTOM_EXACT_PROOF_CONTROL_2026-06-10.md`.
  - 2026-06-10 Multiplayer account proof-sync slice: closed the website gap where Multiplayer refresh updated table progress but did not merge passed proof into the runner's account proof ledger like mobile does. Website Multiplayer proof checks now append safe account attempts and update challenge progress for passed Multiplayer checks, so web-first Multiplayer completions can feed Trophy Cabinet/proof history without switching to mobile. Proof: `pnpm lint -- 'src/app/api/groupquests/[id]/refresh/route.ts'`; `pnpm build`; committed/pushed `5bd91ca`; production deploy `https://cc-ho9eo9vmg-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for seeded Multiplayer detail, 200 for public Multiplayer browse, and 401 `sign_in_required` for signed-out Multiplayer refresh API; proof doc `docs/SQC_WEB_MULTIPLAYER_ACCOUNT_PROOF_SYNC_2026-06-10.md`.
  - 2026-06-10 Solo discovery lanes slice: closed the website gap where mobile-v251 had backend-driven Solo discovery choices, a random Solo shortcut, and a quick-proof lane while `/challenges` only had the full deck plus older static sections. Website `/challenges` now includes the mobile-matching heroism choices, `Surprise me with a random Solo Side Quest`, the aligned recommended starting path, and `Quick proof loop` lane using existing cards/buttons. Proof: `pnpm lint -- src/app/challenges/page.tsx src/components/random-solo-quest-link.tsx`; `pnpm build`; committed/pushed `da66c1d`; production deploy `https://cc-axhcsoe0c-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for `/challenges?soloDiscoverySmoke=20260610` with the new heroism/random/quick-proof content; proof doc `docs/SQC_WEB_SOLO_DISCOVERY_LANES_2026-06-10.md`.
  - 2026-06-10 Multiplayer proof-board receipts slice: continued proof-board parity by carrying verifier FEN/last-move evidence through website Multiplayer refresh responses, rendering compact proof boards in joined table proof cards, and preserving the same evidence when passed Multiplayer checks sync into account proof history. Proof: `pnpm lint -- src/lib/groupquest-proof.ts 'src/app/api/groupquests/[id]/refresh/route.ts' src/components/group-quest-proof-controls.tsx`; `pnpm build`; committed/pushed `1488eaf`; production deploy `https://cc-laxh24u8i-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for seeded Multiplayer detail, 200 for public Multiplayer browse, and 401 `sign_in_required` for signed-out Multiplayer refresh API; proof doc `docs/SQC_WEB_MULTIPLAYER_PROOF_BOARD_RECEIPTS_2026-06-10.md`.
  - 2026-06-10 failure referee-board receipts slice: continued proof-board parity by rendering mobile-style failed Solo receipt boards on active official website Side Quests using verifier failure diagnostics (`fenAtBreak` plus breaker move), safe failure explanations, and an unavailable-board fallback when no usable FEN exists. Proof: `pnpm lint -- src/components/proof-position-board.tsx`; `pnpm build`; committed/pushed `429066c`; production deploy `https://cc-acgr5b089-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200/content for production and deploy `/challenges/finish-any-game?failureBoardSmoke=20260610` plus production `/proof/preview-finish-any-game?failureBoardSmoke=20260610`; proof doc `docs/SQC_WEB_FAILURE_REFEREE_BOARD_RECEIPTS_2026-06-10.md`.
  - 2026-06-10 official quest share-controls slice: closed the mobile-v251 official Solo detail share gap by adding an explicit public `Share link` card to website `/challenges/[id]` pages in every state and by letting shared social controls use public-quest titles/aria labels instead of proof-completion wording for Community/Custom quest links. Proof: `pnpm lint -- src/components/share-proof-actions.tsx 'src/app/challenges/[id]/page.tsx' 'src/app/challenges/community/[id]/page.tsx' src/app/account/custom-side-quests/page.tsx`; `pnpm build`; committed/pushed `b5ab737`; production deploy `https://cc-1q37rgqpp-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200/share content for production and deploy `/challenges/finish-any-game?officialShareSmoke=20260610` plus production Community detail; proof doc `docs/SQC_WEB_OFFICIAL_QUEST_SHARE_CONTROLS_2026-06-10.md`.
  - 2026-06-10 Multiplayer joined/hosted discovery filters slice: closed a small website discovery parity gap versus mobile-v251 by adding signed-in `Joined by me` and `Hosted by me` filters to `/groupquests/public`, plus safe row labels for tables the runner has joined or hosts, while keeping signed-out users on public open/all/finished filters and preserving private invite/account boundaries. Proof: `pnpm lint -- src/app/groupquests/public/page.tsx`; `pnpm build`; committed/pushed `7faf318`; production deploy `https://cc-s1uftytcr-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for signed-out production `/groupquests/public?status=joined&joinedHostedSmoke=20260610` and `/groupquests/public?status=hosted&joinedHostedSmoke=20260610` with public filters falling back safely, plus 200 for the deploy URL; proof doc `docs/SQC_WEB_MULTIPLAYER_JOINED_HOSTED_FILTERS_2026-06-10.md`.
  - 2026-06-10 support diagnostics copy slice: closed a small support/account-readiness parity gap versus mobile-v251 by adding signed-in `Copy support details` to website `/support`, producing a safe troubleshooting packet with profile-readiness flags, active Solo quest, completed/proof/support counts, page path, browser, and timestamp while excluding private emails, raw custom configs, invite codes, and destructive actions. Proof: `pnpm lint -- src/app/support/page.tsx src/components/support-contact-form.tsx`; `pnpm build`; committed/pushed `797f2fe`; production deploy `https://cc-iz70b5iyw-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production and deploy `/support?supportDiagnosticsSmoke=20260610` with Support/contact content, and signed-out `POST /api/support` returned 401 JSON; proof doc `docs/SQC_WEB_SUPPORT_DIAGNOSTICS_COPY_2026-06-10.md`.
  - 2026-06-10 account readiness panel slice: closed a small account-readiness parity gap versus mobile-v251 by adding an `Account readiness` panel to signed-in website `/account` with profile display-name/brag-line readiness, Lichess/Chess.com readiness, completed/proof counts, Custom Solo library state, and hosted/joined Multiplayer activity, using existing account styling and hiding private emails/raw custom configs/invite codes/participant metadata. Proof: `pnpm lint -- src/app/account/page.tsx`; `pnpm build`; committed/pushed `29d9f34`; production deploy `https://cc-n3oxzqu4d-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 307 to `/sign-in` for signed-out production and deploy `/account?accountReadinessSmoke=20260610`, plus 200 content for production `/challenges?accountReadinessSmoke=20260610` and `/groupquests?accountReadinessSmoke=20260610`; proof doc `docs/SQC_WEB_ACCOUNT_READINESS_PANEL_2026-06-10.md`.
  - 2026-06-10 support help-topics slice: closed a small Help & Support parity gap versus mobile-v251 by aligning website `/support` self-help topics with the mobile Help & Support modal and exposing `Copy support details` to signed-out visitors as well as signed-in account threads. Proof: `pnpm lint -- src/app/support/page.tsx src/components/support-contact-form.tsx`; `pnpm build`; committed/pushed `b183177`; production deploy `https://cc-3rolzafyo-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 content for `/support?supportHelpSmoke=20260610` with mobile-aligned help topics and `Copy support details`, and signed-out `POST /api/support` returned 401 JSON; proof doc `docs/SQC_WEB_SUPPORT_HELP_TOPICS_2026-06-10.md`.
  - 2026-06-10 Custom Solo proof-board receipts slice: continued proof-board parity by rendering the shared SQC proof/referee board on website Custom Solo library receipts when verifier attempts include safe FEN/last-move evidence, including failed-check diagnostics/fallbacks while preserving text-only receipts without board evidence. Proof: `pnpm lint -- src/components/proof-position-board.tsx src/app/account/custom-side-quests/page.tsx`; `pnpm build`; committed/pushed `172b812`; production deploy `https://cc-jupnaqu02-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned signed-out sign-in content for production and deploy `/account/custom-side-quests?customBoardSmoke=20260610b` and 200 Community Solo content for `/challenges/community?customBoardSmoke=20260610b`; proof doc `docs/SQC_WEB_CUSTOM_SOLO_PROOF_BOARD_RECEIPTS_2026-06-10.md`.
  - 2026-06-10 Custom Solo Trophy Cabinet slice: closed the website account Trophy Cabinet gap where mobile-v251 included completed Custom Solo entries in the completed quest payload while `/account` only rendered official Solo coats plus Multiplayer podium scrolls. Completed Custom Solo Side Quests now appear as existing-style trophy cards with their custom coat, safe proof/provider/game context, completion time, and a link back to the Custom Solo receipt/board controls without exposing raw configs or private metadata. Proof: `pnpm lint -- src/app/account/page.tsx`; `pnpm build`; committed/pushed `fe769e1`; production deploy `https://cc-rm2zpho07-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned sign-in content for signed-out production and deploy `/account?customTrophySmoke=20260610`, 200 for `/challenges?customTrophySmoke=20260610`, and sign-in content for `/account/custom-side-quests?customTrophySmoke=20260610`; proof doc `docs/SQC_WEB_CUSTOM_TROPHY_CABINET_2026-06-10.md`.
  - 2026-06-10 private Multiplayer host-code copy slice: closed a small website gap versus mobile-v251 by adding an explicit host-only `Copy host code` action to the existing Multiplayer share controls for private-code tables, while leaving public/unlisted tables unchanged and keeping private metadata hidden. Proof: `pnpm lint -- 'src/app/groupquests/[id]/page.tsx' src/components/group-quest-share-button.tsx`; `pnpm build`; committed/pushed `e4a5caa`; production deploy `https://cc-dxgh459u6-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for seeded Multiplayer detail on production and deploy plus 200 for `/groupquests/public?hostCodeCopySmoke=20260610`; proof doc `docs/SQC_WEB_PRIVATE_HOST_CODE_COPY_2026-06-10.md`.
  - 2026-06-10 Custom Solo Multiplayer preselect slice: closed a small website handoff gap versus mobile-v251 by letting website `Use in Multiplayer` links carry the selected Custom Solo ID into `/groupquests/create?quest=...`, where the builder preselects it only if it is available in the signed-in host's launch-ready picker list and otherwise falls back safely. Proof: `pnpm lint -- src/app/groupquests/create/page.tsx src/components/group-quest-draft-builder.tsx src/app/account/custom-side-quests/page.tsx 'src/app/challenges/community/[id]/page.tsx'`; `pnpm build`; committed/pushed `95a7e6f`; production deploy `https://cc-fh6wrm5xl-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned sign-in content for signed-out `/groupquests/create?quest=custom-preselect-smoke-20260610` and `/account/custom-side-quests?preselectSmoke=20260610`, 200 for seeded Community Solo detail with `Use in Multiplayer` and `/groupquests/create?quest=...`, and 200 for `/groupquests/public?preselectSmoke=20260610`; proof doc `docs/SQC_WEB_CUSTOM_MULTIPLAYER_PRESELECT_2026-06-10.md`.
  - 2026-06-10 Custom Solo library filters slice: closed a small website library-management gap versus mobile-v251 by adding All/Published/Drafts/Public/Archived filters and safe title/summary/rule-summary search to `/account/custom-side-quests`, while updating empty states so website-first users can create/manage without switching surfaces and raw configs/private metadata stay hidden. Proof: `pnpm lint -- src/app/account/custom-side-quests/page.tsx`; `pnpm build`; committed/pushed `b4bf1a5`; production deploy `https://cc-6l5v7ja1h-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned sign-in content for signed-out production/deploy Custom Solo filter/search URLs and 200 for `/challenges/community?libraryFilterSmoke=20260610`; proof doc `docs/SQC_WEB_CUSTOM_LIBRARY_FILTERS_2026-06-10.md`.
  - 2026-06-10 home active proof-control slice: closed a small website active-proof gap versus mobile-v251 by adding a direct `Check latest game` control and latest safe receipt summary to the signed-in home Active Solo Side Quest card, while keeping detail-page exact-game/deactivate/reset/receipt controls unchanged. Proof: `pnpm lint -- src/app/page.tsx`; `pnpm build`; committed/pushed `0d6a512`; production deploy `https://cc-73s7fl7e3-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/` and production `/challenges?homeProofSmoke=20260610`; proof doc `docs/SQC_WEB_HOME_ACTIVE_PROOF_CONTROL_2026-06-10.md`.
  - 2026-06-10 badges progress summary slice: closed a small Coat of Arms parity gap versus mobile-v251 by adding a `Trophy Cabinet status` progress panel to `/badges`, including earned live coats, passed proof receipt count, total SQC points, and a Trophy Cabinet path for signed-in users, with safe signed-out copy and unchanged roster/heraldry styling. Proof: `pnpm lint -- src/app/badges/page.tsx`; `pnpm build`; committed/pushed `d5aa51e`; production deploy `https://cc-92407xywk-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/badges?badgesProgressSmoke=20260610` with `Trophy Cabinet status`, `Earned coats`, and `Proof receipts`; proof doc `docs/SQC_WEB_BADGES_PROGRESS_SUMMARY_2026-06-10.md`.
  - 2026-06-10 home Trophy Cabinet lane slice: closed a small website home parity gap versus mobile-v251 by adding a signed-in Trophy Cabinet summary card to `/`, showing the latest five safe official Solo, completed Custom Solo, and Multiplayer podium-scroll items with direct receipt/account links while preserving existing home styling and hiding private metadata. Proof: `pnpm lint -- src/app/page.tsx`; `pnpm build`; committed/pushed `8173683`; production deploy `https://cc-6f1d1jj0c-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/` and 200 for `/badges?homeTrophySmoke=20260610`; proof doc `docs/SQC_WEB_HOME_TROPHY_CABINET_LANE_2026-06-10.md`.
  - 2026-06-10 home random Solo shortcut slice: closed a small website home discovery parity gap versus mobile-v251 by adding the existing `Surprise me with a random Solo Side Quest` action to the home `Where to begin` card, using the same active/completed-aware random pool as `/challenges` while preserving the fixed heroism choices and home styling. Proof: `pnpm lint -- src/app/page.tsx src/components/random-solo-quest-link.tsx`; `pnpm build`; committed/pushed `40a2609`; production deploy `https://cc-8qhkyt5bl-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/` with the random Solo shortcut and heroism content, and 200 for production `/challenges?homeRandomSmoke=20260610`; proof doc `docs/SQC_WEB_HOME_RANDOM_SOLO_SHORTCUT_2026-06-10.md`.
  - 2026-06-10 Community Solo discovery filters slice: closed a small website discovery parity gap versus mobile-v251 by adding safe title/creator/rule/goal search, All/Popular/New/Completed-by-me filters, Top/Newest/A–Z sorting, creator-context preservation, and public stats labels to `/challenges/community` while hiding private drafts/account metadata. Proof: `pnpm lint -- src/lib/community-side-quests.ts src/app/challenges/community/page.tsx`; `pnpm build`; committed/pushed `d146bd3`; production deploy `https://cc-h9xva0tu3-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy Community Solo filter/search URLs and confirmed discovery UI content; proof doc `docs/SQC_WEB_COMMUNITY_SOLO_DISCOVERY_FILTERS_2026-06-10.md`.
  - 2026-06-10 scoreboard Official Leaderboards slice: closed the website `/scoreboard` parity gap versus mobile-v251 by replacing the blurred coming-soon placeholder with a live official leaderboard hub: current official Multiplayer rows, latest final results, weekly archive groups, winner/player/proof context, and final detail links, using public official tables only and preserving private invite/email/account boundaries. Proof: `pnpm lint -- src/app/scoreboard/page.tsx`; `pnpm build`; committed/pushed `f56c80f`; production deploy `https://cc-qsichusqi-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/scoreboard?officialScoreboardSmoke=20260610b` with Official Leaderboards/Current week/Previous week/Archive content, plus 200 for production `/groupquests/public?status=finished&officialScoreboardSmoke=20260610b` with the Official SQC Multiplayer archive; proof doc `docs/SQC_WEB_SCOREBOARD_OFFICIAL_LEADERBOARDS_2026-06-10.md`.
  - 2026-06-10 scoreboard join-context slice: closed a small Official Leaderboards parity gap by adding viewer-aware joined/hosted/sign-in context to website `/scoreboard` rows, including per-runner verified quest counts for joined official tables while keeping existing row styling and preserving private invite/email/account boundaries. Proof: `pnpm lint -- src/app/scoreboard/page.tsx`; `pnpm build`; committed/pushed `0b533f3`; production deploy `https://cc-r5t7bghbq-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/scoreboard?joinContextSmoke=20260610` with Official Leaderboards/Current week and the signed-out join/compare context; proof doc `docs/SQC_WEB_SCOREBOARD_JOIN_CONTEXT_2026-06-10.md`.
  - 2026-06-10 scoreboard podium-context slice: continued Official Leaderboards parity by adding visible Gold/Silver/Bronze mini podium rows to website `/scoreboard` current/previous/archive official table rows when public participants have score or verified proof, including leaderboard name, points, and verified quest count while preserving existing styling and private-data boundaries. Proof: `pnpm lint -- src/app/scoreboard/page.tsx`; `pnpm build`; committed/pushed `9bc0335`; production deploy `https://cc-339ucu581-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/scoreboard?podiumContextSmoke=20260610` with Official Leaderboards/Gold/verified content and 200 for `/groupquests/public?podiumContextSmoke=20260610`; proof doc `docs/SQC_WEB_SCOREBOARD_PODIUM_CONTEXT_2026-06-10.md`.
  - 2026-06-10 home active Custom Solo control slice: closed a small website home parity gap where active Custom Solo runs were invisible to the home Active Solo card while mobile treats active official/custom Solo uniformly. Website `/` now resolves active custom IDs from the signed-in Custom Solo library, shows the custom coat/title, keeps latest receipt copy, runs `Check latest game` through the shared quest action path, and links to custom proof/lifecycle controls without exposing raw configs. Proof: `pnpm lint -- src/app/page.tsx`; `pnpm build`; committed/pushed `ef72e25`; production deploy `https://cc-qtwbsmgaz-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/` and sign-in content for signed-out `/account/custom-side-quests?homeCustomActiveSmoke=20260610`; proof doc `docs/SQC_WEB_HOME_ACTIVE_CUSTOM_SOLO_CONTROL_2026-06-10.md`.
  - 2026-06-10 home Trophy proof-receipt links slice: closed a small mobile-v251 home Trophy Cabinet parity gap by making signed-in website `/` official Solo Trophy rows open canonical public proof receipt links rather than only quest detail pages, while preserving existing home card styling and keeping Custom Solo/Multiplayer receipt destinations unchanged. Proof: `pnpm lint -- src/app/page.tsx`; `pnpm build`; committed/pushed `0b6e6af`; production deploy `https://cc-lz6yi4usg-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/`, 200 for public proof preview with proof/share-board content, and 200 for `/badges` with Trophy Cabinet content; proof doc `docs/SQC_WEB_HOME_TROPHY_PROOF_RECEIPT_LINKS_2026-06-10.md`.
  - 2026-06-10 Multiplayer quick duration chips slice: closed a small mobile-v251 create/manage parity gap by adding quick proof-window duration chips (24h, 3 days, 7 days, 14 days) to website `/groupquests/create` and host `/groupquests/[id]/edit`, preserving exact date editing, existing styling, invite/privacy behavior, and verifier semantics. Proof: `pnpm lint -- src/components/group-quest-draft-builder.tsx src/components/group-quest-edit-form.tsx src/app/globals.css` (passed with the CSS ignored-file warning only); `pnpm build`; committed/pushed `cc739c4`; production deploy `https://cc-2scuif1uc-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 307 sign-in protection for production/deploy `/groupquests/create?quickDurationSmoke=20260610`, 200 for `/groupquests/public?quickDurationSmoke=20260610`, and 200 for seeded Multiplayer detail; proof doc `docs/SQC_WEB_MULTIPLAYER_QUICK_DURATION_CHIPS_2026-06-10.md`.
  - 2026-06-10 Custom Solo public proof receipts slice: closed a completed-proof sharing gap by making completed website Custom Solo library cards generate canonical public `/proof/...` receipts from the latest passed attempt, with `Open proof receipt`, copy/social/image share controls, custom coat OG imagery, and public proof pages that accept signed Custom Solo payloads without exposing raw rule configs/private library state. Proof: `pnpm lint -- src/lib/proof-share.ts 'src/app/proof/[token]/page.tsx' 'src/app/api/og/proof/[token]/route.tsx' src/app/account/custom-side-quests/page.tsx`; `pnpm build`; committed/pushed `d7b2a11`; production deploy `https://cc-nax4fcll5-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned sign-in protection for `/account/custom-side-quests?customProofReceiptSmoke=20260610`, public proof preview content with proof/share controls, 200 `image/png` for the proof OG image, and matching deploy URL proof content; proof doc `docs/SQC_WEB_CUSTOM_SOLO_PUBLIC_PROOF_RECEIPTS_2026-06-10.md`.
  - 2026-06-10 Multiplayer command-center discovery links slice: closed a small signed-in Multiplayer parity gap versus mobile-v251 by showing the proof-ledger reminder on website `/groupquests` for signed-in users too, and adding direct command-center handoffs to Public Multiplayer discovery, Joined by me, and Hosted by me filters. Proof: `pnpm lint -- src/app/groupquests/page.tsx`; `pnpm build`; committed/pushed `30e5d56`; production deploy `https://cc-pt4lsgedh-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for `/groupquests?commandCenterLinksSmoke=20260610b` with Multiplayer/Public/Proof rule content and 200 for production/deploy `/groupquests/public?commandCenterLinksSmoke=20260610b`; proof doc `docs/SQC_WEB_MULTIPLAYER_COMMAND_CENTER_DISCOVERY_LINKS_2026-06-10.md`.
  - 2026-06-10 Public Multiplayer sort-controls slice: closed a small website discovery parity gap versus mobile-v251 by adding explicit Closing soon/Newest/Most players sort controls to `/groupquests/public` alongside search/status/host filters, with server-side ordering that keeps open tables prioritized by default and recent finals first in archive-style views while preserving private invite/account boundaries. Proof: `pnpm lint -- src/app/groupquests/public/page.tsx`; `pnpm build`; committed/pushed `1bae0a1`; production deploy `https://cc-h202vqmvj-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200/content for production `/groupquests/public?sort=players&publicSortSmoke=20260610` with sort/discovery content and deploy `/groupquests/public?sort=newest&publicSortSmoke=20260610`; proof doc `docs/SQC_WEB_PUBLIC_MULTIPLAYER_SORT_CONTROLS_2026-06-10.md`.
  - 2026-06-10 profile readiness chips slice: closed a small profile/account-readiness parity gap versus mobile-v251 by adding signed-in readiness chips to website `/profile` for Display name, Brag line, Lichess, and Chess.com using existing account readiness styling, while preserving the current profile form and hiding private emails/account metadata. Proof: `pnpm lint -- src/app/profile/page.tsx`; `pnpm build`; committed/pushed `566e468`; production deploy `https://cc-6r07401xq-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production/deploy `/profile?profileReadinessSmoke=20260610` with signed-out profile/sign-in content and 200 for production `/challenges?profileReadinessSmoke=20260610`; proof doc `docs/SQC_WEB_PROFILE_READINESS_CHIPS_2026-06-10.md`.
  - 2026-06-10 Custom/Community Quest language polish slice: continued Andreas's product-language cleanup by replacing remaining visible “recipe” wording across Custom Solo library/builder, Community Solo browse/detail/not-found, and Multiplayer create quick-start copy with product-facing Side Quest/rule language while preserving SQC voice and privacy boundaries. Proof: `grep -RIn "recipe" src/app/account/custom-side-quests/page.tsx src/app/challenges/community src/components/group-quest-draft-builder.tsx` returned no matches; `pnpm lint -- src/app/account/custom-side-quests/page.tsx src/app/challenges/community/page.tsx 'src/app/challenges/community/[id]/page.tsx' 'src/app/challenges/community/[id]/not-found.tsx' src/components/group-quest-draft-builder.tsx`; `pnpm build`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-58c273xfu-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200/no-`recipe` content for production Community Solo browse/detail and deploy browse, plus signed-out account sign-in content; proof doc `docs/SQC_WEB_CUSTOM_COMMUNITY_QUEST_LANGUAGE_POLISH_2026-06-10.md`.
  - 2026-06-10 public-player language polish slice: continued the visible UX cleanup by replacing awkward/internal-feeling public `player shelf`, `raw config`, `Safe rule summary`, and `Account handoff` copy across Community Solo browse/detail, Custom Solo library receipts, and Support diagnostics with player-facing `More from player`, `Public rule summary`, `Start from account`, and private rule-data wording. Proof: grep for the targeted phrase family now only finds an admin-only mobile support label; `pnpm lint -- src/app/account/custom-side-quests/page.tsx src/app/challenges/community/page.tsx 'src/app/challenges/community/[id]/page.tsx' src/components/support-contact-form.tsx`; `pnpm build`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-9ny9222ww-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned 200 for production Community Solo browse/detail with the new public-player wording and no old phrase-family matches, plus expected 307 sign-in protection for signed-out Custom Solo; proof doc `docs/SQC_WEB_PUBLIC_PLAYER_LANGUAGE_POLISH_2026-06-10.md`.
  - 2026-06-10 Community Solo card action polish slice: continued visible browse-page UX polish by simplifying Community Solo cards from a noisy four-button cluster plus duplicate player link into an app-quality action hierarchy: `Inspect quest` and `Start from account` as the primary card lane, quieter `More from {player}`/`Report quest` text actions, and SQC-styled rule-summary pills with mobile stacking. Proof: `pnpm lint -- src/app/challenges/community/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-p2aali53y-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned production/deploy Community Solo content with `community-quest-actions`, `Start from account`, and `Report quest`, and no old card `Start/check in account` copy; proof doc `docs/SQC_WEB_COMMUNITY_SOLO_CARD_ACTION_POLISH_2026-06-10.md`.
  - 2026-06-10 Community Solo detail action polish slice: continued visible detail-page UX polish by replacing the crowded five-action Community Solo hero row with an SQC-styled action panel: `Start from account` and `Use in Multiplayer` as primary actions, quieter `More from {player}`/`Share public link`/`Report quest` secondary links, and mobile stacking. Also removed remaining visible `Start/check in account`, `Report weird quest`, and `Report weird quests` wording from Community Solo browse/detail. Proof: `pnpm lint -- src/app/challenges/community/page.tsx 'src/app/challenges/community/[id]/page.tsx' src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-mjbyv37jy-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned production/deploy Community Solo detail content with `community-detail-action-panel`, `Start from account`, and `Report quest`, no old `Start/check in account`/`Report weird quest` strings, plus cleaned browse copy; proof doc `docs/SQC_WEB_COMMUNITY_SOLO_DETAIL_ACTION_POLISH_2026-06-10.md`.
  - 2026-06-10 Custom Solo builder rule-group polish slice: continued Andreas's flagged builder polish by replacing each condition card's flat wall of mixed controls with an in-card rule-shape guide and SQC-styled Result / Pattern / Piece-state / Advanced twist field groups. This keeps the six-condition verifier payload, saved-rule prefill, complex-rule preservation, and responsive behavior unchanged while making the builder clearer from a player perspective. Proof: `pnpm lint -- src/app/account/custom-side-quests/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-eju18j2w5-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned signed-out sign-in content for production/deploy Custom Solo builder URLs and 200 Community Solo content; proof doc `docs/SQC_WEB_CUSTOM_BUILDER_RULE_GROUP_POLISH_2026-06-10.md`.
  - 2026-06-10 Custom Solo active-rule highlight slice: continued Andreas's flagged builder polish by making the selected Result / Pattern / Piece-state section visually stand out inside each condition card, with clearer microcopy that dimmed sections are safe defaults. This preserves the six-condition payload, saved-rule prefill, complex-rule preservation, advanced twist controls, and responsive behavior while reducing the builder's wall-of-fields feel. Proof: `pnpm lint -- src/app/account/custom-side-quests/page.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; `pnpm deploy:prod` including `pnpm quest:release-gate`; production deploy `https://cc-5i48gqxdt-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`; live smoke returned signed-out sign-in content for production/deploy Custom Solo builder URLs, 200 Community Solo content, and deployed CSS containing `custom-condition-grid:has`; proof doc `docs/SQC_WEB_CUSTOM_BUILDER_ACTIVE_RULE_HIGHLIGHT_2026-06-10.md`.
...
- [x] Add Community Solo web analytics hooks.
  - added_at: 2026-06-04 17:16 Europe/Stockholm
  - completed_at: 2026-06-09 22:18 Europe/Stockholm
  - source: next website community hardening item after public browse/detail/trust/creator/handoff/empty states shipped.
  - scope: added lightweight privacy-safe first-party analytics events for Community Solo browse, detail, creator-filter, report-link, and account-handoff interactions using existing analytics conventions; no raw custom quest configs or private metadata in event payloads.
  - proof: commit `ef2a013` added the privacy-safe Community Solo analytics client helper and event allow-list entries. Verification passed: `pnpm lint -- src/components/analytics/community-solo-analytics.tsx src/lib/analytics.ts src/app/challenges/community/page.tsx 'src/app/challenges/community/[id]/page.tsx'`; `pnpm build`; production deploy `https://cc-eaulu9w7l-andreas-nordenadlers-projects.vercel.app` → `https://sidequestchess.com`; live smoke returned 200 for Community Solo browse/detail pages and 200 `{\"ok\":true,\"stored\":false}` for a signed-out privacy-safe analytics POST.
...
- [x] Fix active Solo `What to do` tense mismatch.
  ...
  - proof: changed active detail source priority from `proofCallout` first to `instruction`, then `objective`, then `proofCallout`; fallback copy now uses generic connected-account wording; `pnpm --filter @sidequestchess/mobile typecheck` passed; `pnpm lint` passed with existing 3 warnings only; `pnpm build` passed; `pnpm mobile:release` built v280; `pnpm mobile:release:github` published v281; installed APK smoke passed for `artifacts/mobile-releases/sqc-mobile-android-v281-2026-06-15.apk`; GitHub release `mobile-v281` is live at `https://github.com/andreasnordenadler/cc/releases/tag/mobile-v281` with SHA256 `99c678e0b4c8378188b83b158bae1327ebbe2e2514eef638b9d0e9e5c499ad22`.
...
- [x] Make active Solo post-check next-step copy provider-generic.
  ...
  - proof: replaced `Next step: play one new public game on {accountLabel}` with `Next step: play one new public game on your connected chess account`; `pnpm --filter @sidequestchess/mobile typecheck` passed; `pnpm lint` passed with existing 3 warnings only; `pnpm build` passed; `pnpm mobile:release` built v276; `pnpm mobile:release:github` published v277; installed APK smoke passed for `artifacts/mobile-releases/sqc-mobile-android-v277-2026-06-15.apk`; GitHub release `mobile-v277` is live at `https://github.com/andreasnordenadler/cc/releases/tag/mobile-v277` with SHA256 `b509db24037dbe6aac80d89147578db64df0b9629c30ba424a62b658f389a2c7`.

- [x] Clean mobile Community Side Quest detail player-facing copy and contrast.
  - added_at: 2026-06-15 14:40 Europe/Stockholm
  - source: Andreas screenshot feedback: the detail view looked too white and exposed creator/internal wording to end users.
  - scope: mobile custom/community Side Quest detail copy, related creator/host shelf copy, Multiplayer rule display copy, and proof detail card contrast.
  - proof: replaced raw `White`/`Black` rule option titles with `Play as White`/`Play as Black`; translated player-facing rules such as `White only` into `Play as White`; hid visibility/creator-management copy from public viewer mode; removed `raw custom quest config`, `creator context`, `local creator shelf`, and private-account implementation text; toned proof detail cards from bright cream to dark/gold; `pnpm --filter @sidequestchess/mobile typecheck` passed; `pnpm lint` passed with existing 3 warnings only; `pnpm build` passed; `pnpm mobile:release` built v274; `pnpm mobile:release:github` published v275; installed APK smoke passed for `artifacts/mobile-releases/sqc-mobile-android-v275-2026-06-15.apk`; GitHub release `mobile-v275` is live at `https://github.com/andreasnordenadler/cc/releases/tag/mobile-v275` with SHA256 `54057937efd1eb12673dc0de592c9f8def59b0dda8429717fdb464110cce3f5b`.

## SQC mobile coat-of-arms glow correction v293
- [x] Remove remaining mobile coat/seal solid-back visual cases after Andreas supplied counterexample screenshots.
  - added_at: 2026-06-15 23:35 Europe/Stockholm
  - source: Andreas screenshot correction after `mobile-v291`: several examples still read as solid backing/blob instead of glow behind coat-of-arms art.
  - scope: mobile coat/seal image source selection and Multiplayer compact/detail seal rendering.
  - fix: added `multiplayerHeroGlow`, changed challenge glow fallback to generic transparent glow, replaced Multiplayer baked seal/black seal usage with coat-of-arms artwork plus glow, removed circular detail/row seal radii, and blocked known mobile badge assets with baked backings (`pawn-storm-maniac`, `no-castle-club`, `knightmare-mode`) from rendering as mobile coat art until clean redraws exist.
  - proof: `pnpm --filter @sidequestchess/mobile typecheck`, `pnpm lint -- apps/mobile/App.tsx`, `pnpm build`, `pnpm audit --prod --audit-level high` (no high severity), `node scripts/mobile-release.mjs`, emulator install of `sqc-mobile-android-v293-2026-06-15.apk`, screenshot QA for `coat-glow-v293-pawn-storm-detail.png`, `coat-glow-v293-rookless-detail.png`, `coat-glow-v293-home-after-back.png`, and `coat-glow-v293-multiplayer-catalog.png`; GitHub release `mobile-v293` is live at `https://github.com/andreasnordenadler/cc/releases/tag/mobile-v293` with SHA256 `e8765a05c7d79c2973d0f4e93a030c541e86fa5f4d9243d6ac872a2ab8fe073d`.
