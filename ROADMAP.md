...
Last updated: 2026-04-30 03:52 Europe/Stockholm
Owner: Sam  
...
- [ ] Prepare SQC for polished friends/private beta before any public launch push.
  ...
  - 2026-05-03 00:30 Europe/Stockholm progress: removed the visible top-bar brand lockup while keeping primary nav/auth actions unchanged; proof doc `docs/SQC_REMOVE_TOP_BAR_WORDMARK_2026-05-03.md`. Verification: `pnpm lint`, `pnpm build`, production deploy to `https://cc-np9tvsppf-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`, live smoke confirmed deploy/canonical `/` return HTTP 200 and rendered top nav no longer contains `brand-text` or `<strong>Side Quest Chess</strong>`.
  - 2026-05-03 03:05 Europe/Stockholm progress: added a homepage private-beta trust strip that explains public chess-data-only verification, warns testers never to share chess-site passwords, and links to proof rules plus the beta support packet; proof doc `docs/SQC_HOMEPAGE_PRIVATE_BETA_TRUST_STRIP_LIVE_DEPLOY_2026-05-03.md`. Verification: `pnpm lint`, `pnpm build`, production deploy to `https://cc-6m7w694xx-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`, live smoke for deploy/canonical `/`, canonical `/rules`, and canonical `/beta`, content assertions for the trust/support strings, and Vercel error-log check with no logs found.
  - 2026-05-04 23:08 Europe/Stockholm progress: simplified the private-beta primary nav around core runner surfaces, added a dedicated `/support` page with copy/paste report fields, and normalized visible wording toward Side Quest Chess “quests” while preserving the full ten-quest dual-host verifier claim on `/beta`; proof doc `docs/SQC_NAV_SUPPORT_QUEST_LANGUAGE_LAUNCH_POLISH_2026-05-04.md`. Verification: clean worktree `pnpm install --frozen-lockfile`, `pnpm lint`, `pnpm build`, production deploy to `https://cc-mmtavvmwq-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`, live smoke for canonical `/support`, `/`, `/beta`, and preview `/support`.
  - 2026-05-05 00:50 Europe/Stockholm progress: connected the new support packet into the beta discovery path by routing the homepage trust CTA to `/support`, adding a `/support` hero CTA on `/beta`, and updating beta support copy to send confusing receipts/setup/badge/share-card reports through the packet; proof doc `docs/SQC_PRIVATE_BETA_SUPPORT_PACKET_DISCOVERY_2026-05-05.md`. Verification: `pnpm lint`, `pnpm build`, deployed code commit `22ac3b1`, pushed to `origin/main`, production deploy to `https://cc-bsjmzm040-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`, live smoke for canonical `/`, `/beta`, `/support`, and preview `/beta`.
  - 2026-05-05 01:55 Europe/Stockholm progress: tightened the receipt-to-support handoff so `/result` now sends confusing passed/failed/pending receipts directly to `/support`, and `/support` asks for latest-check headline plus fairness note with direct `/result` and `/account` actions; proof doc `docs/SQC_RECEIPT_SUPPORT_PACKET_HANDOFF_LIVE_DEPLOY_2026-05-05.md`. Verification: `pnpm lint`, `pnpm build`, production deploy to `https://cc-aoyh0ekzr-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`, live smoke for canonical `/support`, canonical `/result`, and preview `/support`.
  - 2026-05-05 03:10 Europe/Stockholm progress: simplified the homepage hero action cluster from eleven route-directory buttons to the four core first-run actions: continue/start path, quest hub, today’s quest, and connect/account preflight; proof doc `docs/SQC_HOMEPAGE_HERO_ACTION_SIMPLIFICATION_LIVE_DEPLOY_2026-05-05.md`. Verification: `pnpm lint`, `pnpm build`, production deploy to `https://cc-awvgpn347-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`, live smoke for canonical/preview `/` content plus canonical `/challenges`, `/path`, and `/today`.
  - 2026-05-05 03:54 Europe/Stockholm progress: added a dedicated `/today` proof-loop handoff so the daily quest now routes testers through identity preflight, the current daily dare, and the latest receipt/support path; proof doc `docs/SQC_TODAY_PROOF_LOOP_HANDOFF_LIVE_DEPLOY_2026-05-05.md`. Verification: `pnpm lint`, `pnpm build`, production deploy to `https://cc-haydujgc6-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`, live smoke for preview/canonical `/today` plus canonical `/connect` and `/result`, and bounded Vercel log watch with no runtime errors seen before timeout.
  - 2026-05-05 04:54 Europe/Stockholm progress: added a challenge-detail proof-loop handoff to every `/challenges/[id]` page so testers see start → play on Lichess/Chess.com → return to receipt/support at the exact quest-acceptance point; proof doc `docs/SQC_CHALLENGE_DETAIL_PROOF_LOOP_HANDOFF_LIVE_DEPLOY_2026-05-05.md`. Verification: `pnpm lint`, `pnpm build`, production deploy to `https://cc-r3a2yrewe-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`, live smoke for preview/canonical `knights-before-coffee`, canonical `no-castle-club`, `/result`, and `/support`, plus `vercel inspect --logs` ready-status proof.
  - 2026-05-05 05:47 Europe/Stockholm progress: tightened `/connect` after full starter-deck parity so Chess.com setup no longer implies partial support, and added direct setup handoff CTAs to `/path`, `/today`, `/result`, plus signed-out `/path` and `/support`; proof doc `docs/SQC_CONNECT_FULL_DECK_PROOF_HANDOFF_LIVE_DEPLOY_2026-05-05.md`. Verification: `pnpm lint`, `pnpm build`, commit `2d18a61`, pushed to `origin/main`, production deploy to `https://cc-j7paush9l-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`, live smoke for canonical/preview `/connect`, canonical `/path`, canonical `/support`, and `vercel inspect --logs` ready-status proof.
  - 2026-05-05 06:44 Europe/Stockholm progress: added a `/scoreboard` beta handoff so points and badge progress now route testers through setup, the recommended next quest, latest receipt review, and the support packet instead of ending as a passive status page; proof doc `docs/SQC_SCOREBOARD_BETA_HANDOFF_LIVE_DEPLOY_2026-05-05.md`. Verification: `pnpm install --frozen-lockfile`, `pnpm lint`, `pnpm build`, production deploy to `https://cc-49tkzc5lv-andreas-nordenadlers-projects.vercel.app` aliased to `https://sidequestchess.com`, live smoke for canonical/preview `/scoreboard`, canonical `/account`, canonical `/result`, canonical `/support`, and `vercel inspect --logs` ready-status proof.
  - Acceptance:
  ...
  - 2026-04-30 13:44 Europe/Stockholm progress: added a homepage `Private beta quickstart` block so testers can run the first loop from `/` without route hunting: `/account` identity preflight, `/challenges` starter route, then `/result` receipt/report. Proof doc `docs/SQC_HOMEPAGE_PRIVATE_BETA_QUICKSTART_LIVE_DEPLOY_2026-04-30.md`. Verification: clean isolated worktree, `pnpm install --frozen-lockfile`, `pnpm lint`, `pnpm build`, production deploy to `https://cc-3iptxmubl-andreas-nordenadlers-projects.vercel.app`, live smoke on deploy + canonical `/`, `/account`, `/challenges`, and `/result`, plus Vercel 500 scan with 0 recent errors.
  - 2026-05-01 20:58 Europe/Stockholm progress: added an `/account` first tester route so private-beta runners can make one of the three recommended starter dares active directly from account preflight instead of choosing from the full ten-quest deck cold; proof doc `docs/SQC_ACCOUNT_FIRST_TESTER_ROUTE_LIVE_DEPLOY_2026-05-01.md`. Verification: `pnpm lint`, `pnpm build`, production deploy to `https://cc-fbdl6bjw3-andreas-nordenadlers-projects.vercel.app`, live smoke on deploy/canonical `/account`, canonical `/challenges`, canonical `/result`, and Vercel error-log scan with no logs.
