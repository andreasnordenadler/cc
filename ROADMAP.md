...
Last updated: 2026-06-11 06:02 Europe/Stockholm
Owner: Sam  
...
- [ ] 24h SQC website UX parity review: make newly added website functions feel app-quality from a user perspective.
  ...
  - 2026-06-11 remaining product-language polish slice: continued Andreas's product-language cleanup after noticing newly reintroduced visible `recipe`/`account handoff` wording in Custom Solo, Community Solo detail, and Multiplayer create copy. Custom Solo now consistently talks about Custom Solo Side Quests/quests/rules, the builder starter panel is `Quest starters`, Community Solo checklist copy uses normal account start/report language, and Multiplayer create references launch-ready Custom Solo Side Quests. Proof so far: targeted phrase sweep now only finds an admin-only support label; `pnpm lint -- src/app/account/custom-side-quests/page.tsx 'src/app/challenges/community/[id]/page.tsx' src/components/group-quest-draft-builder.tsx src/app/globals.css` (CSS ignored-file warning only); `pnpm build`; proof doc `docs/SQC_WEB_REMAINING_PRODUCT_LANGUAGE_POLISH_2026-06-11.md`. Production deploy/smoke pending.
