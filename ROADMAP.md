...
- [ ] 24h SQC website parity sprint: close missing website functionality compared to mobile app while preserving current website look and feel.
  ...
  - 2026-06-10 badge transparency/quality correction slice: Andreas flagged the refreshed `Any Game Counts` coat as still lower quality than the other coats and not transparent. Scope: replace web and mobile mirrored `proof-loop-test-badge.png` with a higher-quality SQC-style no-text/no-letter crest and verify real RGBA transparency outside the coat before redeploy. Proof: pending.
