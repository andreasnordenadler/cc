...
test("native unavailable failure board exposes its visible title as a heading", async () => {
  const source = await readFile(new URL("../apps/mobile/App.tsx", import.meta.url), "utf8");
  const start = source.indexOf("function FailureDiagnosticBoard(");
  const end = source.indexOf("type VictoryProofBoardInput", start);
  assert.ok(start >= 0 && end > start, "the native failure diagnostic board must be present");

  const sourceFile = ts.createSourceFile(
    "FailureProofBoard.tsx",
  ...
  const inspect = (node: ts.Node) => {
    if (ts.isJsxElement(node) && node.openingElement.tagName.getText(sourceFile) === "Text") {
      ...
      if (visibleText === "Board position unavailable") matches.push(node);
    }
    ts.forEachChild(node, inspect);
  };
  inspect(sourceFile);

  assert.equal(matches.length, 1, "Expected the rendered unavailable-board title");
