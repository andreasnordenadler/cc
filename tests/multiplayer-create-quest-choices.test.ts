...
test("multiplayer create form gives authenticated users Android's name and intro guidance before submission", () => {
  ...
  assert.match(html, /aria-describedby="multiplayer-quest-name-help"[^>]*placeholder="Name this Multiplayer Side Quest"/);
  assert.match(html, /id="multiplayer-quest-name-help">Required\. Make it clear enough that players know what they are joining\.<\/small>/);
  assert.match(html, /aria-describedby="multiplayer-intro-text-help"[^>]*placeholder="Explain what players are joining\.\.\."/);
  assert.match(html, /id="multiplayer-intro-text-help">Shown to players before they join\.<\/small>/);
});
