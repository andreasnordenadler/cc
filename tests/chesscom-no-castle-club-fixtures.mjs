...
import {
  normalizeChessComNoCastleClubGame,
} from "../src/lib/chesscom.ts";
import { evaluateNoCastleClub } from "../src/lib/no-castle-club.ts";
...
test("Chess.com No Castle Club normalizer uses observed and72nor archive shape", () => {
  const game = normalizeChessComNoCastleClubGame(observedAnd72NorLatestGame, "and72nor");
  ...
  const verdict = evaluateNoCastleClub(game);
  assert.equal(verdict.status, "failed");
  assert.match(verdict.summary, /uncastled player still wins/);
});

test("Chess.com No Castle Club can verify a win-required quest from PGN without a pasted game URL", () => {
  const game = normalizeChessComNoCastleClubGame(syntheticAnd72NorNoCastleWin, "and72nor");
  ...
  assert.match(verdict.summary, /zero player castling/);
});
