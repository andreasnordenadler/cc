...
test("native Multiplayer host filter shelf exposes its visible title as a screen-reader heading", async () => {
  ...
  const inspect = (node: ts.Node) => {
    if (ts.isJsxElement(node) && node.openingElement.tagName.getText(sourceFile) === "Text") {
      ...
      if (text === "Host shelf: {multiplayerHostFilter}") titles.push(node);
    }
    ts.forEachChild(node, inspect);
  };
  inspect(sourceFile);

  assert.equal(titles.length, 1, "Expected the visible Multiplayer host filter shelf title");
