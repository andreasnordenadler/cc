...
import { normalizeChessComKnightsBeforeCoffeeGame } from "../src/lib/chesscom.ts";
import { evaluateKnightsBeforeCoffee } from "../src/lib/knights-before-coffee.ts";
...
const syntheticHorseFirstWin = {
  url: "https://www.chess.com/game/live/synthetic-horse-first-win",
  end_time: 1706476100,
  time_class: "blitz",
  rules: "chess",
...
1. Na3 e5 2. Nh3 Nc6 3. Nb1 d5 4. Ng1 Nf6 5. e3 Be7 6. d4 O-O 7. Nf3 exd4 8. exd4 Re8 9. Be2 Bb4+ 10. c3 Bd6 11. O-O 1-0`,
};

test("Chess.com Knights Before Coffee normalizer uses observed and72nor archive shape", () => {
  const game = normalizeChessComKnightsBeforeCoffeeGame(observedAnd72NorLatestGame, "and72nor");
  ...
  assert.deepEqual(game.firstFourPlayerMovePieces, ["pawn", "pawn", "bishop", "pawn"]);

  const verdict = evaluateKnightsBeforeCoffee(game);
  assert.equal(verdict.status, "failed");
  assert.match(verdict.summary, /horse-first player wins/);
});

test("Chess.com Knights Before Coffee can verify a win-required horse-first quest from PGN", () => {
  const game = normalizeChessComKnightsBeforeCoffeeGame(syntheticHorseFirstWin, "and72nor");

  assert.ok(game);
  assert.equal(game.playerColor, "white");
  assert.equal(game.winner, "white");
  assert.deepEqual(game.firstFourPlayerMovePieces, ["knight", "knight", "knight", "knight"]);

  const verdict = evaluateKnightsBeforeCoffee(game);
  assert.equal(verdict.status, "passed");
  assert.match(verdict.summary, /Horse-first opening confirmed/);
});
