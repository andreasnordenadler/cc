...
test("native empty official Multiplayer shelf exposes its visible title as a heading", async () => {
  ...
  const inspect = (node: ts.Node) => {
    if (ts.isJsxElement(node) && node.openingElement.tagName.getText(sourceFile) === "Text") {
      ...
      if (visibleText === "Official Multiplayer Side Quests") matches.push(node);
    }
    ts.forEachChild(node, inspect);
  };
  inspect(sourceFile);

  assert.equal(matches.length, 2, "Expected both the catalog and empty-state official shelf titles");
  const emptyStateTitle = matches.find((node) => node.openingElement.attributes.properties.some(
    (property) => ts.isJsxAttribute(property)
      && property.name.getText(sourceFile) === "style"
      && property.initializer?.getText(sourceFile).includes("communityEmptyTitle"),
  ));
  assert.ok(emptyStateTitle, "Expected the rendered empty-state official shelf title");
  const role = emptyStateTitle.openingElement.attributes.properties.find(
