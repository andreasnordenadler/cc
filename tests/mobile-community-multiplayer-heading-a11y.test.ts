...
test("native Community Multiplayer shelf exposes its visible section title as a heading", async () => {
  ...
  const inspect = (node: ts.Node) => {
    if (ts.isJsxElement(node) && node.openingElement.tagName.getText(sourceFile) === "Text") {
      ...
      if (visibleText === "Community Multiplayer Side Quests.") matches.push(node);
    }
    ts.forEachChild(node, inspect);
  };
  inspect(sourceFile);

  assert.equal(matches.length, 1, "Expected the rendered Community Multiplayer shelf title");
