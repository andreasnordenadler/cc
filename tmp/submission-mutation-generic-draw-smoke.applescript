on run argv
  ...
  tell application "Google Chrome"
    ...
    delay 1
    set workTab to make new tab at end of tabs of win with properties {URL:canonicalHost & "/challenges/draw-any-game"}
    delay 2
    ...
    repeat with hostPair in {{"canonical", canonicalHost}, {"active", activeHost}}
      ...
      set URL of workTab to hostBase & "/challenges/draw-any-game"
      delay 2
      set beforeText to execute workTab javascript "document.body ? document.body.innerText : ''"
      do shell script "cat > " & quoted form of (outDir & "/" & hostName & "__draw-any-game__before.txt") & " <<'EOF'\n" & beforeText & "\nEOF"
      ...
      do shell script "cat > " & quoted form of (outDir & "/" & hostName & "__draw-any-game__after.txt") & " <<'EOF'\n" & afterText & "\nEOF"
    end repeat
