on run argv
  ...
  set routeSlugs to {"finish-any-game", "finish-as-white", "finish-as-black", "lose-any-game", "lose-as-black", "lose-as-white", "draw-any-game", "draw-as-black", "draw-as-white", "win-as-white", "win-as-black"}
  ...
  tell application "Google Chrome"
    ...
    delay 1
    set workTab to make new tab at end of tabs of win with properties {URL:canonicalHost & "/challenges/finish-any-game"}
    delay 2
    ...
    repeat with hostPair in {{"canonical", canonicalHost}, {"active", activeHost}}
      ...
      repeat with slug in routeSlugs
        set routeSlug to slug as text
        set URL of workTab to hostBase & "/challenges/" & routeSlug
        delay 2
        set beforeText to execute workTab javascript "document.body ? document.body.innerText : ''"
        do shell script "cat > " & quoted form of (outDir & "/" & hostName & "__" & routeSlug & "__before.txt") & " <<'EOF'\n" & beforeText & "\nEOF"
        ...
        do shell script "cat > " & quoted form of (outDir & "/" & hostName & "__" & routeSlug & "__after.txt") & " <<'EOF'\n" & afterText & "\nEOF"
      end repeat
    end repeat
