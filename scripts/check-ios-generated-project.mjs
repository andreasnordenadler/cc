...
const parsePlist = (source) => {
  ...
  const parseValue = () => {
    ...
    if (token?.startsWith("<integer>")) {
      const value = Number(token.slice(9, -10).trim());
      if (!Number.isSafeInteger(value)) fail("malformed plist integer");
      return value;
    }
    if (token?.startsWith("<real>")) {
      const value = Number(token.slice(6, -7).trim());
      if (!Number.isFinite(value)) fail("malformed plist real");
      return value;
    }
    if (token?.startsWith("<date>")) return token.slice(6, -7).trim();
    if (token?.startsWith("<data>")) return token.slice(6, -7).replaceAll(/\s/g, "");
    if (token === "<array>") {
      const values = [];
...
try {
  ...
  const urlSchemes = Array.isArray(parsedInfo.CFBundleURLTypes)
    ...
    : [];
  const expectedUrlSchemes = [EXPECTED_SCHEME, EXPECTED_BUNDLE_ID];
  if (
    urlSchemes.length !== expectedUrlSchemes.length ||
    expectedUrlSchemes.some((scheme) => !urlSchemes.includes(scheme))
  ) {
    fail(`URL schemes must contain only ${expectedUrlSchemes.join(" and ")}`);
  }
  ...
  const entitlementKeys = Object.keys(parsedEntitlements);
  if (
    entitlementKeys.length !== 1 ||
    entitlementKeys[0] !== "com.apple.developer.applesignin"
  ) {
    fail(`unexpected entitlement keys: ${entitlementKeys.join(", ") || "none"}`);
  }
