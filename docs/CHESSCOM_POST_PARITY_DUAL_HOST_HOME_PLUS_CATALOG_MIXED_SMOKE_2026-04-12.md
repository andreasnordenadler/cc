# Chess.com Post-Parity Dual-Host Home-Plus-Catalog Mixed Smoke

Date: 2026-04-12
Proof window: 2026-04-11 22:22:00 UTC to 2026-04-11 22:22:03 UTC (same-run dual-host fetch)
...
Dual-host home-plus-catalog mixed parity passed.

All ten checks completed during the same proof window. Both `/` responses returned `200`, both `/challenges` responses returned `200` and exposed the full eleven shipped challenge routes, both representative and boundary detail routes returned `200`, and both signed-out `/account` checks returned the same visible protected `404` shape.

## Home and catalog integrity evidence
...
## Shared signals supporting the home-plus-catalog mixed verdict

The same-run responses on both hosts exposed these shared signals:

- `/` showed the same Chess.com-supported home-entry wording on both hosts during the same proof window.
...
The canonical host and active deployment host served the same home-plus-catalog mixed parity slice successfully in one proof window across `/`, `/challenges`, `/challenges/win-as-white`, `/challenges/lose-as-black`, and signed-out `/account`. Both hosts exposed the same Chess.com-supported home wording, both `/challenges` surfaces exposed the full eleven-route catalog, both detail routes matched with the same visible wording, and the protected route matched with the same signed-out `404` shape. This is a clean dual-host home-plus-catalog mixed parity pass.

## Verification

Verified live on 2026-04-12 by fetching all ten exact URLs in one Python run and confirming that both `/challenges` responses exposed the same eleven-route catalog during the same proof window.