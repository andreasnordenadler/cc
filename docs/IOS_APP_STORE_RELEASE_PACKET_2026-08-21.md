...
- Current local native receipt: Xcode 26.6 with the iOS 26.5 SDK selected; CocoaPods 1.17.0 `pod install` PASS with 85 dependencies/84 pods; generated workspace `SideQuestChess.xcworkspace`; scheme `SideQuestChess`; Debug and Release Simulator build PASS at exact iPhone 17 Pro / iOS 26.5 destination. Generated Release identity readback: Side Quest Chess, `com.sidequestchess.app`, `0.1.349 (1)`. The same Release `.app` installed successfully; iPhone 17 Pro and iPad Pro 13-inch (M5), both iOS 26.5, launched it as a running process. OCR of settled captures found public “Side Quest Chess” and signed-out sign-in copy; captures were not visually reviewed and are not App Store screenshots. A signed archive was not produced.
