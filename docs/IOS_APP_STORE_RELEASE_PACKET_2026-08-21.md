...
- Source bundle ID: `com.sidequestchess.app`; Expo scheme: `sidequestchess`; native callback: `sidequestchess://sso-callback`. Apple-side App ID ownership/availability, App Store Connect app record, signing association, and duplicate status are unverified.
...
- Production signed-out Support readback on 2026-08-25 exposed Crowdler AB, `sam@crowdler.com`, and the legal address. This closes the prior deployment/readback gap, but owner/legal must still confirm whether local law requires a telephone number. If Crowdler AB declares EU trader status, Apple's DSA workflow separately requires public address, phone, and email display information.
...
- The iOS 26.5 Simulator runtime (`23F77`) was registered and available for the historical build described below. That build was performed on mobile release inputs unchanged by the current upstream reconciliation, but its generated project, build log, app hash, screenshot and `.xcresult` were not retained. Treat it as an unretained local observation, not independently auditable current-candidate evidence.
...
- No Apple Developer/App Store Connect readback has verified the Crowdler AB team, dedicated operator, App ID, App Store Connect app record, signing identity, provisioning, TestFlight build, or highest prior build number. The signing identity is unverified.
...
- A historical unsigned generic Simulator build under Xcode 26.6 began compiling all 92 targets and reached app Swift/storyboard work, but failed while compiling `SplashScreen.storyboard` after the CoreSimulator service connection was interrupted. Its retry predated successful runtime registration. It remains a concrete historical failure receipt, not a pass or evidence for current commit `3cf61fb1d5233a8899d5dfcd3a6caea4c2a8dc4a`.
- **Unretained local observation (not independently auditable evidence):** the superseding historical receipt used generated workspace `SideQuestChess.xcworkspace`, scheme `SideQuestChess`, Debug configuration, iOS Simulator SDK 26.5, and an iPhone 17 Pro / iOS 26.5 destination. With `CODE_SIGNING_ALLOWED=NO` and a worktree-bounded DerivedData path, Xcode reported `** BUILD SUCCEEDED **`; the generated app reported bundle `com.sidequestchess.app`, short version `0.1.349`, build `1`, then installed, launched and rendered through Metro. The generated project, bounded DerivedData, logs and non-fixture screenshot were discarded. This is not a signed IPA, TestFlight build, physical-device smoke, screenshot asset, or release-readiness receipt.

### Retained cross-worktree cached Simulator build receipt

On 2026-08-25, a build was invoked from the clean release-input commit `4f62e560ce2d14f82943c9fa0e2acd0f7669b470` after fresh unsigned native generation and CocoaPods integration. It used Xcode 26.6, Debug, `CODE_SIGNING_ALLOWED=NO`, and a bounded DerivedData cache retained from an older worktree whose mobile release inputs are byte-identical. The result bundle is retained locally as `ios-build-4f62e560.xcresult`; `xcresulttool get build-results` reports `succeeded`, 0 errors, and 484 dependency/toolchain warnings. The warnings reference the current generation worktree, the dependency-providing packet worktree, and the reused DerivedData worktree; the built app is byte-identical to the prior cached Debug app. This proves a successful cached rebuild invocation with unchanged mobile inputs, not a self-contained exact-current or pristine release build.

- The built app reports `Side Quest Chess`, bundle `com.sidequestchess.app`, version/build `0.1.349` / `1`, minimum iOS `15.1`, device family `1,2`, SDK iOS Simulator `26.5`, Xcode `17F113`, and URL schemes `sidequestchess` plus `com.sidequestchess.app`.
- Fresh generated and built plists both report ATS arbitrary loads `false` and local networking `true`, resolving the read-only introspection concern without changing the archive/network gate. `ITSAppUsesNonExemptEncryption` remains absent because export classification is still pending.
- Fresh pod integration reproduced tracking `false`, no native collected-data declarations, and the previously recorded required-reason categories. This does not answer App Privacy or prove archive completeness.
- The app executable SHA-256 is `2e41a13074a3621822beafcd392137314df822b7f8e109957671055b1d82a04b`; the built Info.plist SHA-256 is `810f7b17e38d2dbd669314204b1d2561ac24e329bdd3abbfddca5776bac29670`.
- The 73 MB Debug Simulator app installed and launched on an iPhone 17 Pro / iOS 26.5 Simulator. Operational logs observed Metro bundling 973 modules and the app remaining alive after a synthetic `sidequestchess://sso-callback` URL was opened. Those Metro/deep-link logs were not retained in the result bundle, and the diagnostic screenshot was deleted after independent review found personal account and quest data in it. No screenshot asset is claimed. The synthetic callback does not verify handling, routing, any SSO provider, or an authenticated flow.
- Operational runtime logs also warned that Expo/React Native delegate methods exist without corresponding background-fetch/remote-notification modes and that the legacy UIKit lifecycle will become unsupported in a future SDK. These logs were not retained with the result bundle and must be reproduced during exact Release/archive review; adding unused background modes would be an unjustified capability change.
- This cached Debug Simulator build depends on Metro and is not a signed archive, IPA, TestFlight build, physical-device smoke, or App Store screenshot. It does not close signing, export, privacy, authentication, UGC, tablet, real-device, submission, or release gates.
...
| Apple agreements | Blocked | Crowdler AB account-holder readback of all required current agreements; any acceptance remains separately approval-gated |
...
| Signed candidate | Blocked | IPA hash, identity/version/build, Crowdler signature/team, entitlements and export-compliance inspection; processed upload readback must bind version/build, upload status, warnings and export compliance |
| Privacy labels | Draft only | Backend/provider/SDK/manifest/network reconciliation against exact IPA |
| Privacy policy safety data | Production disclosure read back; behavior blocked | Production signed-out readback contains report and block disclosures, including the current lack of an independent moderation queue; the underlying Guideline 1.2 implementation remains blocked |
...
| Support URL contact | Email/address read back; trader/local-law decision blocked | Production signed-out Support exposes Crowdler AB, `sam@crowdler.com`, and the legal address; a phone number is mandatory if Crowdler AB selects EU trader status, and owner/legal must separately confirm any other local-law requirement |
...
| App Review information/access | Blocked | Non-expiring owner-authorized demo account and required SSO demo access; review contact name, email and international-format phone; exact notes and backend/service availability read back |
...
TestFlight acceptance also requires: processed upload readback binding bundle, version/build, upload status, warnings and export compliance; no unresolved Missing Compliance status; correct name/icon/API; “What to Test”; internal group/build assignment; store-delivered installation; no debug endpoint; no secrets/tokens in logs; non-expiring review fixtures; public chess fixture; second-account multiplayer; background/foreground/forced relaunch; successful deletion and signed-out relaunch. External testing additionally requires Beta App Description, Feedback Email, Contact Information, and an internal group first. The first build added or submitted to an external testing group requires full TestFlight App Review; later builds for the same version may not. Read back each external-test field rather than assuming production review contact is reused. Record the build's 90-day expiry horizon. Internal testing, external TestFlight review, production App Review, approval, and public availability are separate states.
...
The implementation receipts below through the earlier 791-test result predate mobile-input baseline `3cf61fb1d5233a8899d5dfcd3a6caea4c2a8dc4a`. Later test and native-build statements are historical local observations unless a retained receipt explicitly identifies the source SHA and artifact.
...
- Signed-out Support contact TDD receipt: RED proved the Support screen omitted direct Crowdler contact information; GREEN renders Crowdler AB, `sam@crowdler.com`, and Kvarnängsvägen 15, 182 47 Enebyberg, Sweden without requiring sign-in. Production signed-out readback on 2026-08-25 confirmed those details.
...
- Historical unretained local Simulator observation: Xcode 26.6 with the iOS 26.5 SDK selected, CocoaPods 1.17.0 and runtime `23F77`; an unsigned Debug Simulator build was reported to compile, install, launch and render on iPhone 17 Pro / iOS 26.5. No logs, generated project, app hash, screenshot, `.xcresult`, signed archive, IPA, TestFlight build, physical-device result or release candidate were retained.
- Complete release Xcode build, Simulator test matrix, `.xcresult`, archive, signing, TestFlight and device smoke: NOT PASSED. Disk capacity remains narrow, and the separate Apple identity, legal account, duplicate, signing, approval, UGC, privacy, authentication, archive and real-device gates remain open.
- Historical full-suite receipt on mobile-input baseline `3cf61fb1d5233a8899d5dfcd3a6caea4c2a8dc4a`: PASS — 811 tests, 0 failures, 0 skipped/todo. It is not independently auditable and remains bound to that historical run.
- Fresh verification on the current reconciled packet tree (`5dce5b13b5d6c9c6bb2fb9a91faa827c5ff50121` plus this documentation/test-only TDD update) used the CI-pinned pnpm 11.12.0: `pnpm test` PASS — 816 tests, 0 failures, 0 skipped/todo; mobile TypeScript typecheck PASS; Expo Doctor PASS — 18/18; Next.js production build PASS; lint PASS with the same four pre-existing warnings and no errors; `git diff --check` PASS. The targeted packet contract first failed for the missing receipt, then passed after this update. Independent review of the exact two-file release-packet diff reported no blocking findings; its medium-strength concern is that the hard-coded source-baseline assertion must be refreshed whenever `origin/main` advances, and its low-strength concern is that production-readback contract tests lock prose rather than live state. These source and documentation checks do not make this branch a signed candidate, retained Simulator/Xcode result, IPA, TestFlight build, physical-device pass, review submission, approval, or public release.
