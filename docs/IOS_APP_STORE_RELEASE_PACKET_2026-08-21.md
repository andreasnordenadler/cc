...
**Prepared:** 2026-08-21; reconciled 2026-08-26<br>
**Source baseline:** `444539e1a0ffade891ff934e0615efe71d610395` (`origin/main`, fetched and merged 2026-08-26). The original packet landed in `5a5cbd45`; section 12 distinguishes historical receipts from exact-current evidence.<br>
**Status:** Drafts, verified source audit, and public-page readback only. This is not an App Store Connect record, Apple credential, IPA, TestFlight build, review submission, approval, or public release.
...
- No deliberate iOS build number is source-controlled. The EAS production profile has `autoIncrement: true` with local app-version sourcing, while a disposable clean prebuild currently defaults to build `1`; neither establishes the next available App Store Connect number or a reproducible first iOS candidate. No verified evidence of a final privacy manifest report, signed IPA, TestFlight build, or physical-iPhone callback proof is available to this lane.
- Production signed-out readback verified on 2026-08-25: `https://sidequestchess.com/support` returned HTTP 200 and exposed Crowdler AB, `sam@crowdler.com`, and Kvarnängsvägen 15, 182 47 Enebyberg, Sweden. Apple requires the Support URL to lead to actual contact information as required by local law; owner/legal must still confirm whether a telephone number is locally required.
...
- The iOS 26.5 Simulator runtime (`23F77`) is registered. On historical source-equivalent baseline `5aee7255`, a frozen dependency install, iOS prebuild, CocoaPods install, Xcode Simulator build, install, launch, and Metro bundle completed after only a stale 2.4 GiB Side Quest Chess DerivedData cache was removed. The iPhone 17e on iOS 26.5 build succeeded with zero errors and 483 dependency/toolchain warnings; no XCTest or Swift Testing targets executed. This supports source compatibility but is not an exact-current run, archive, Apple signing, TestFlight, physical-iPhone, authentication, deep-link, responsive, or release evidence.
- A 2026-08-26 host check found about 1.2 GiB of free disk space on the root volume. A fresh native build is blocked by inadequate disk headroom; the 2026-08-25 receipt remains historical source-equivalent evidence rather than a current-candidate rerun. Do not delete DerivedData or Simulator devices to clear space without a bounded ownership and evidence review.
...
- A historical unsigned generic Simulator build under Xcode 26.6 began compiling all 92 targets and reached app Swift/storyboard work, but failed while compiling `SplashScreen.storyboard` after the CoreSimulator service connection was interrupted. Runtime registration has since been restored, but no successful current-baseline compile, launch, archive, or test run has replaced that failed receipt. The historical build attempt's xcresult inspection recorded zero tests and result `unknown`; because the bundle is not retained at a verified durable path, it proves neither compilation nor launch and cannot be re-inspected.
...
| Public name legal clearance | Blocked | Owner/legal-approved trademark and name clearance for Side Quest Chess; App Store name availability is not legal clearance |
...
| Account deletion / retained safety identifiers | Blocked | Retained report and block identifiers are a source-level account-deletion blocker: purge or anonymize references held in other users' metadata using a concurrency-safe, atomic design rather than non-atomic replacement of Clerk metadata arrays, or document a legally required retention basis, scope, duration, access controls, and user disclosure |
...
| Public listing URLs | Blocked | Marketing, Support, Privacy, and Terms URLs all resolve to current, accurate, signed-out content in an exact-submission readback |
| Support URL contact | Email/address production readback verified | Production signed-out page exposes Crowdler AB, direct email and legal address; owner/legal must decide whether local law requires a telephone number |
...
| EU trader status | Blocked | Owner/legal self-assessment and app-specific selection; if Crowdler declares trader status, enter the organization phone number and email and verify both separately, upload business-name/address documentation, provide payment account details if absent, and complete the certification that offered products/services comply with applicable EU law. Each contact verification, document upload, payment-account action, certification/attestation, and app-specific selection requires separate approval before mutation |
...
| China mainland availability | Blocked | Owner/legal-approved inclusion only after verified NPPA game approval evidence and a determination of whether an MIIT ICP Filing Number is required; otherwise exclude China mainland |
| Vietnam availability | Blocked | Owner/legal-approved inclusion only after the required game-publishing license and legally sourced metadata instructions are verified; otherwise exclude Vietnam |
| Republic of Korea availability | Blocked | Complete the organization availability property, preserve the calculated Korean rating, verify whether final answers trigger an RCN, and exclude Korea if required information is unavailable |
...
11. Resolve retained safety-record identifiers during deletion. Purge or anonymize deleted users' reporter, target-owner, and blocked-user identifiers wherever they are stored in other users' metadata, or stop unless a legally required retention basis defines scope, duration, access controls, and user disclosure. The current per-user Clerk array model has no atomic compare-and-swap across account deletion and cross-account report/block arrays: scan-and-replace cleanup can skip or overwrite concurrent writes, while rollback can resurrect deleted-user references. Use a concurrency-safe, atomic data model/cleanup boundary rather than treating best-effort Clerk metadata scans as complete deletion. Internal owner/legal approval without legal necessity is insufficient. Current disclosure of the asymmetry does not by itself satisfy the deletion gate.
...
Before adoption, verify the live field limits, unique name/SKU, territories, content rights, EU trader status, tax category, version/build, review contact, exact User Access selection, Made for Kids answer, and copyright ownership and year. Accessibility Nutrition Labels are currently voluntary; if Crowdler publishes any support claim, require exact-candidate iPhone and iPad common-task evaluation (including first launch, login, password reset, and settings) for each claimed feature, otherwise preserve and read back the deliberate not indicated state. “What’s New” is unavailable for the first App Store version and required for each later version. A worldwide consumer App Store launch requires Public Distribution; Private Distribution is limited to specifically identified organizations through Apple Business Manager or Apple School Manager and is not a private consumer launch. Owner must explicitly approve the applicable method before submission. Public versus Private Distribution cannot be changed after approval without a new app record and binary; conversion from public to unlisted is the limited exception. Also decide whether “All Countries or Regions” may opt the app into storefronts Apple adds later or whether to use a specific approved territory list. For China mainland, obtain and verify the NPPA game approval number and supporting documents, and determine whether an MIIT ICP Filing Number is also required; if required, the ICP information and App Information metadata must match the Simplified Chinese localization, or the primary language if no Simplified Chinese localization exists. For Vietnam, obtain and verify the required game-publishing license and obtain owner/legal-approved metadata instructions from authoritative Vietnamese legal guidance before selecting that storefront; Apple's App Information reference establishes the licensing requirement but does not by itself prescribe where a license number or URL must appear. For the Republic of Korea, complete the organization-enrollment availability property, preserve the calculated Korean age rating, verify whether the final answers trigger a Rating Classification Number, and exclude Korea if required information is unavailable. Do not convert territory-specific compliance into an inferred worldwide exclusion: owner/legal must approve inclusion or exclusion per territory from verified provider and legal requirements. Submit each product-page field only after exact-candidate screenshot, privacy, age-rating and review-access reconciliation.
...
TestFlight acceptance also requires: processed build with no unresolved Missing Compliance status; processed-upload readback binding the exact version, build and export compliance state to the candidate; correct name/icon/API; “What to Test”; internal group/build assignment; store-delivered installation; no debug endpoint; no secrets/tokens in logs; non-expiring review fixtures; public chess fixture; second-account multiplayer; background/foreground/forced relaunch; successful deletion and signed-out relaunch. External testing additionally requires Beta App Description, Feedback Email, Contact Information, and an internal group first. For external testing, Apple's detailed workflow says the first build submitted for a version requires a full TestFlight App Review: select the external group and build, complete the required information, then use `Submit Review`; later builds for the same version may not require a full review. Apple's TestFlight overview separately describes the first build added to a group as sent to review, so preserve the exact live group, version, build, submission action, and review status rather than generalizing either formulation. Read back each external-test field rather than assuming production review contact is reused. Record the build's 90-day expiry horizon. Internal testing, external TestFlight review, production App Review, approval, and public availability are separate states.
...
Every native receipt below predates exact source baseline `444539e1` and is historical evidence, not proof that the command passed on current `origin/main`. The changes from `5aee7255` through `444539e1` are desktop-web-only and do not touch mobile or native build inputs, so the `5aee7255` native receipt is source-equivalent supporting evidence only; a fresh exact-commit run remains required when safe disk headroom is available.
...
- Historical branch receipt: `pnpm test`: PASS — 791 tests, 0 failures, 0 skipped/todo.
- Historical branch receipt: `pnpm build`: PASS — Next.js production build completed.
- Historical branch receipt: `pnpm --dir apps/mobile run typecheck`: PASS.
- Historical branch receipt: `pnpm --dir apps/mobile run doctor`: PASS — 18/18 checks before native generation.
- Historical branch receipt: `pnpm lint`: PASS with four pre-existing warnings and no errors (one unused variable and three `no-img-element` warnings).
...
- Signed-out Support contact TDD receipt: RED proved the Support screen omitted direct Crowdler contact information; GREEN renders Crowdler AB, `sam@crowdler.com`, and Kvarnängsvägen 15, 182 47 Enebyberg, Sweden without requiring sign-in. Production signed-out readback on 2026-08-25 verified the same identity, email and address; the local-law telephone decision remains open.
...
- Historical source-equivalent local tooling receipt (`5aee7255`, 2026-08-25): Xcode 26.6 with iOS 26.5 SDK selected; CocoaPods 1.17.0 installed; iOS 26.5 Simulator runtime `23F77` registered; zero valid local code-signing identities. The frozen dependency install used lockfile SHA-256 `5edd63864c5af4d5a85718e5af02bc769ae7634d8b8b60349a4f08c03d3317e9`; regenerated CocoaPods paths resolve inside the isolated `v26` worktree rather than another candidate checkout.
- Historical source-equivalent Xcode receipt: `SideQuestChess.xcworkspace`, scheme `SideQuestChess`, Debug, iPhone 17e UDID `25423278-D058-4FD7-95FE-7BA695DA23CE`, iOS 26.5 build `23F77`, arm64, bundle `com.sidequestchess.app`, plist version/build `0.1.349` / `1`, iOS deployment target `15.1`, device family `1,2`. `xcodebuild` status was `succeeded` with zero errors, zero analyzer warnings, and 483 dependency/toolchain warnings; readable result bundle: `/Users/sam/Projects/sqc-worktrees/ios-current-candidate-20260825-v26/.ios-build-20260825-v26-frozen.xcresult`.
- The built app installed and `simctl launch` returned process `40069`; Metro then bundled `apps/mobile/index.js` successfully with 974 modules. No XCTest or Swift Testing target was executed, and no screenshot or semantic UI journey was reviewed. This is not TestFlight evidence and not physical-iPhone, archive, signing, authentication, deep-link, responsive-layout, or release evidence.
- Simulator compilation and basic launch: PASSED on the declared historical source-equivalent destination. Exact-current Simulator rerun, Simulator tests, archive, Crowdler signing, IPA inspection, TestFlight and device smoke: NOT PASSED / BLOCKED by disk headroom plus the separate source, Apple identity, account, signing and approval gates.
- 2026-08-26 local capacity receipt: full Xcode and the iOS 26.5 runtime remain available, but the root volume had about 1.2 GiB free. No new prebuild, CocoaPods install, Xcode build, Simulator boot, archive, or account-authenticated EAS action was attempted under that unsafe headroom.
...
- South Korea compliance: https://developer.apple.com/help/app-store-connect/manage-compliance-information/manage-korea-compliance-information
- South Korea app-information properties: https://developer.apple.com/help/app-store-connect/reference/app-information/required-localizable-and-editable-properties
