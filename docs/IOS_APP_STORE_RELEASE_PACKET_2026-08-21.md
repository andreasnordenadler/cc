...
- The iOS 26.5 Simulator runtime (`23F77`) is now registered. This removes the earlier runtime-registration blocker but does not establish a successful build, simulator launch, archive, or test pass. Inspection of a historical build-attempt xcresult recorded result `unknown`, zero tests, and no device/configuration; that result bundle is not retained at a verified durable path, so the receipt is inconclusive rather than present build evidence. The 2026-08-25 prerequisite recheck found less than 0.5 GiB free on `/`; another native build is blocked until safe project-scoped disk headroom is available.
...
- Current-baseline local tooling receipt (`5aee7255`, 2026-08-25): Xcode 26.6 with iOS 26.5 SDK selected; CocoaPods 1.17.0 installed; iOS 26.5 Simulator runtime `23F77` registered; zero valid local code-signing identities. A historical build attempt's xcresult inspection recorded zero tests, no device/configuration, and result `unknown`; the bundle is not present under a verified durable path, so this is not a build or test pass.
- Complete Xcode build, Simulator launch/tests, readable passing `.xcresult`, archive, signing, TestFlight and device smoke: NOT PASSED / BLOCKED by constrained disk headroom plus the separate source, Apple identity, account, signing and approval gates.
