...
**Source baseline:** `3cdfc0cf90629b3dbd0a57b167ca6700be055159` (`origin/main`, fetched 2026-08-24); the earlier packet/source work is merged, while this reconciliation remains under PR review<br>
...
- The live Privacy, Support, and Terms pages returned HTTP 200 on 2026-08-24. The signed-out Support page exposes Crowdler AB, `sam@crowdler.com`, and the legal address; the live Privacy page includes report/block handling and deletion-asymmetry disclosures. This clears the earlier deployment/readback blockers only. Owner/legal must still confirm whether a telephone number is locally required.
...
- The iOS 26.5 simulator runtime is now registered and `simctl` lists available iPhone and iPad devices. A clean current-source Release Simulator build succeeded, but the built app crashes during launch, so no Simulator journey or `.xcresult` has passed.
...
- The historical unsigned generic Simulator build failed while compiling `SplashScreen.storyboard` after CoreSimulator interruption. The current exact-source Release Simulator build then succeeded for iPhone 17 Pro / iOS 26.5, producing `com.sidequestchess.app` version/build `0.1.349` / `1`, but two launches exited with SIGSEGV in React Native's `RCTKeyWindow` while UIKit accessed `UIScene` lifecycle state. A Debug build remained resident on the same simulator and reached the expected no-Metro error surface. This separates successful native compilation from a reproducible Release-only launch crash; it is not Simulator-journey evidence.
...
| Generated native audit | Build passed; Release launch blocked | Reproduce and resolve the iOS 26.5 Release SIGSEGV, then rerun clean prebuild/build/install/launch; preserve Info.plist, entitlements, URL types, deployment target, SDKs and privacy manifests |
...
| Privacy policy safety data | Live source-aligned readback passed | 2026-08-24 signed-out production readback contains report/block and deletion-asymmetry disclosures; recheck against the frozen candidate before submission |
...
| Support URL contact | Live readback passed; legal phone decision open | 2026-08-24 signed-out production page exposes Crowdler AB, legal address and direct email; owner/legal confirms whether a telephone number is locally required |
...
10. Remove production-facing QA/test copy from the offline fallback, verify the source-prepared password-reset path on the exact candidate (including email delivery, other-session revocation and MFA behavior), decide universal-link behavior for shared quest URLs, and reconcile Google/Facebook as privacy processors before source freeze. The current OAuth callback relies on the claimable custom scheme `sidequestchess://`; prefer a controlled universal-link callback if supported, or explicitly threat-test scheme collision, state/PKCE, replay, malformed state, and repeated callbacks before acceptance.
11. Pin or explicitly approve and record the production EAS iOS build image/Xcode/SDK. Node and pnpm are pinned, but the production profile currently pins only the Android image, so source freeze alone does not freeze the cloud iOS toolchain.
...
- `pnpm test`: PASS — 810 tests, 0 failures, 0 skipped/todo on the reconciled branch tree based on `3cdfc0cf90629b3dbd0a57b167ca6700be055159`.
...
- Current local native receipt: Xcode 26.6 with the iOS 26.5 SDK selected; CocoaPods 1.17.0 and `pod install` PASS; Release Simulator build PASS on iPhone 17 Pro / iOS 26.5 from source `3cdfc0cf90629b3dbd0a57b167ca6700be055159`; generated app identity `com.sidequestchess.app`, `0.1.349` / `1`, minimum iOS 15.1, device family `1,2`, Xcode `17F113`, SDK `26.5`. Install PASS, but Release launch FAIL reproduced twice with SIGSEGV in `RCTKeyWindow` / `UIScene`; Debug build remained resident on the same simulator and failed only because no Metro server was running. No candidate screenshot or journey is accepted from the crashed Release build.
- A strict TDD experiment set only `ios.newArchEnabled` false after a failing config contract, but the generated Legacy Architecture Release build failed compiling `fmt` 11.0.2 under Xcode 26.6 (`FMT_STRING` consteval expression). The experiment was reverted and is not proposed for merge. The launch crash therefore remains unresolved rather than hidden behind an unverified architecture downgrade.
- Complete Simulator journey, `.xcresult`, archive, signing, TestFlight and device smoke: NOT PASSED. Release launch is now the immediate local source/toolchain blocker; archive/signing/TestFlight remain separately blocked by Apple identity, account, signing and approval gates.
