...
**Prepared:** 2026-08-21; reconciled 2026-08-24<br>
**Source baseline:** `a2949393bdc18ba6ebda0067842fa71a520ba12f` (`origin/main`, fetched 2026-08-24). The iOS release-preparation lane merged in `5a5cbd456900f3e75d4a9cbc9e7769c157b1d83b`; subsequent mobile changes are copy-only “Coat of Arms” normalization and do not close binary, device, account, or release gates.<br>
...
- Simulator registration remains locally blocked. `xcodebuild -downloadPlatform iOS` downloaded the iOS 26.5 Simulator payload, then exited 70 while installing because CoreSimulator reported a duplicate disk image. After rerunning first-launch setup, `simctl list runtimes` still returned no registered runtimes and `xcodebuild` again reported that iOS 26.5 was not installed. Do not erase runtimes or reinstall Xcode without a separately reviewed remediation.
...
- An unsigned generic Simulator build under Xcode 26.6 began compiling all 92 targets and reached app Swift/storyboard work, but failed while compiling `SplashScreen.storyboard` after the CoreSimulator service connection was interrupted. A retry failed before compilation because the simulator runtime was not registered. Therefore Xcode build, archive inspection, signing and Simulator/device launch remain blocked; this is a concrete failed build receipt, not a pass.
...
- `pnpm test`: PASS — 809 tests, 0 failures, 0 skipped/todo on the reconciled 2026-08-24 tree.
...
- Complete Xcode build, Simulator tests, `.xcresult`, archive, signing, TestFlight and device smoke: NOT PASSED / BLOCKED by simulator registration plus the separate Apple identity, account, signing and approval gates.
