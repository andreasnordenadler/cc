...
- The iOS 26.5 simulator runtime is now registered and `simctl` lists available iPhone and iPad devices. This supersedes the earlier duplicate-disk-image/runtime-registration blocker, but no clean current-candidate Xcode build, Simulator journey, or `.xcresult` has yet been produced.
...
- An unsigned generic Simulator build under Xcode 26.6 began compiling all 92 targets and reached app Swift/storyboard work, but failed while compiling `SplashScreen.storyboard` after the CoreSimulator service connection was interrupted. A historical retry then failed before compilation during the now-resolved runtime-registration incident. Therefore the historical attempt remains a concrete failed build receipt, not a pass; rerun on the current clean candidate before claiming compilation or launch evidence.
...
- Current local native receipt: Xcode 26.6 with the iOS 26.5 SDK selected; first-launch setup PASS; CocoaPods 1.17.0 installation PASS; `pod install` PASS with 85 dependencies/84 pods; aggregate generated privacy manifest inspected; the earlier unsigned generic Simulator build FAILED after CoreSimulator interruption at `SplashScreen.storyboard`. The iOS 26.5 simulator runtime is now registered, superseding the retry blocker, but no current-candidate build pass, installable candidate, or `.xcresult` has yet been produced.
- Complete Xcode build, Simulator tests, `.xcresult`, archive, signing, TestFlight and device smoke: NOT PASSED. Simulator registration is no longer the blocker; clean native generation/build needs substantial free disk, and archive/signing/TestFlight remain separately blocked by Apple identity, account, signing and approval gates.
