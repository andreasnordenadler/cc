...
- The iOS 26.5 Simulator runtime (`23F77`) is now registered. This removes the earlier runtime-registration blocker but does not establish a successful build, simulator launch, archive, or test pass. Inspection of a historical build-attempt xcresult recorded result `unknown`, zero tests, and no device/configuration; that result bundle is not retained at a verified durable path, so the receipt is inconclusive rather than present build evidence. Disk headroom is constrained and must be rechecked before another bounded native build.
