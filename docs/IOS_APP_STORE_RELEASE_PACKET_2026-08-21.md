...
**Prepared:** 2026-08-21; local candidate evidence reconciled 2026-08-24<br>
**Source baseline:** `283ad0f0821a125376004ec1599f1fe01a54c302` (`origin/main`, freshly fetched 2026-08-24); this is observed candidate evidence, not an owner-approved release freeze<br>
...
- A fresh production readback on 2026-08-24 verified that the live signed-out Support page exposes Crowdler AB, `sam@crowdler.com`, and the legal address Kvarnängsvägen 15, 182 47 Enebyberg, Sweden. This closes the earlier deployment/readback gap for direct email and legal-address contact information. Owner/legal must still confirm whether a telephone number is locally required.
...
- Simulator registration is no longer locally blocked: `simctl` reported the iOS 26.5 runtime and exact available iPhone/iPad destinations on 2026-08-23. This removes only the former local runtime blocker; it does not provide signing, archive, TestFlight, physical-device, or store evidence.
- EAS is associated with a personal owner account; the verified authenticated identity is not the dedicated Crowdler operator. Do not use it for Apple access, credentials, build, or submission.
- Current exact-candidate receipt: [`IOS_CURRENT_CANDIDATE_RECEIPT_2026-08-24.md`](./IOS_CURRENT_CANDIDATE_RECEIPT_2026-08-24.md) records that the exact `origin/main` Release Simulator build, install, and launch passed on iPhone 17e and iPad Pro 13-inch; its callback probe proves scheme routing only, not provider authentication, signed archive, IPA, TestFlight, or physical-device behavior.
...
- The current exact-candidate receipt supersedes the earlier failed local build attempt: on source `283ad0f0821a125376004ec1599f1fe01a54c302`, an unsigned Release Simulator build succeeded under Xcode 26.6, and the same app installed and launched on exact iPhone and iPad destinations. This remains local Simulator evidence only; archive inspection, signing, TestFlight, provider authentication, and physical-device behavior remain blocked.
...
| Dedicated Crowdler operator | Blocked | Exact Apple Account, MFA/recovery custody, role and app scope; never a personal operational identity |
...
| Generated native audit | Unsigned current-candidate audit complete | Exact archive must still repeat and reconcile Info.plist, entitlements, URL types, deployment target, SDKs and privacy manifests |
...
| Privacy policy safety data | Live readback passed | The live Privacy Policy discloses report and block records, deletion behavior, and Crowdler AB contact details; final label and exact-candidate behavior reconciliation remain blocked |
...
| Screenshots | Blocked | Fresh same-candidate App Store listing captures at Apple's accepted iPhone/iPad dimensions, representative signed-in states, and complete source/build/device/locale/file/hash metadata; local signed-out QA captures do not satisfy this gate |
| Support URL contact | Live readback passed for email/address | Production signed-out page exposes the verified legal address and direct email; include a telephone number if owner/legal confirms it is locally required |
...
- logging in to Apple Developer or App Store Connect for read-only discovery, unless the approved dedicated operator, exact Crowdler AB team, read-only scope, and stop conditions are documented first;
...
- using any personal Apple identity or communicating externally;
...
- `pnpm test`: PASS — 804 tests, 0 failures, 0 skipped/todo on exact source `283ad0f0821a125376004ec1599f1fe01a54c302`.
...
- Signed-out Support contact TDD receipt: RED proved the Support screen omitted direct Crowdler contact information; GREEN renders Crowdler AB, `sam@crowdler.com`, and Kvarnängsvägen 15, 182 47 Enebyberg, Sweden without requiring sign-in. A signed-out production readback on 2026-08-24 verified the same email/address contact block live.
...
- Current local native receipt: [`IOS_CURRENT_CANDIDATE_RECEIPT_2026-08-24.md`](./IOS_CURRENT_CANDIDATE_RECEIPT_2026-08-24.md) binds the clean source SHA, Corepack package-manager install, repository gates, generated project/workspace, CocoaPods graph, Xcode Release Simulator build, inspected `.app`, exact iPhone/iPad launches, captures, deep-link routing probe, signing inventory, and shutdown to one candidate. The generated native tree was removed and is not proposed as source.
- Complete signed archive inspection, XCTest/XCUIAutomation `.xcresult`, TestFlight, provider-authentication callback, real-iPhone smoke, and App Store evidence: NOT PASSED / BLOCKED by the separate Apple identity, account, signing, approval, and exact-candidate QA gates.
