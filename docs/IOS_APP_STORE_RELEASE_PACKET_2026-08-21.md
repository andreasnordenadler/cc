...
**Source baseline:** `4f62e560ce2d14f82943c9fa0e2acd0f7669b470` (`origin/main`, fetched 2026-08-25). The original packet landed in `5a5cbd45`; section 12 distinguishes historical branch receipts from current-baseline evidence.<br>
...
- The iOS 26.5 Simulator runtime (`23F77`) is now registered. This removes the earlier runtime-registration blocker but does not establish a successful build, simulator launch, archive, or test pass. An existing `ios-build-4f62e560.xcresult` has result `unknown`, zero tests, and no device/configuration, so it is inconclusive rather than positive evidence. Disk headroom is constrained and must be rechecked before another bounded native build.
...
Unless a receipt below explicitly names baseline `4f62e560`, it is historical evidence preserved from the source-preparation branches merged by `5a5cbd45`, not proof that the command passed on current `origin/main`.
...
- Current-baseline local tooling receipt (`4f62e560`, 2026-08-25): Xcode 26.6 with iOS 26.5 SDK selected; CocoaPods 1.17.0 installed; iOS 26.5 Simulator runtime `23F77` registered; zero valid local code-signing identities. The pre-existing `ios-build-4f62e560.xcresult` reports zero tests, no device/configuration, and result `unknown`, so it is not a build or test pass.
