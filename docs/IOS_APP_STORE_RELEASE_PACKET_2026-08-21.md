...
**Source baseline:** `7a2c32cb77c296b484ba4ce71177f350a59c3a47` (`origin/main`, fetched 2026-08-23); branch audit and source-preparation changes remain under PR review<br>
...
- The iOS 26.5 Simulator runtime is now registered. A clean current-main Release simulator build, install, launch, and callback-open receipt is recorded below. This removes the former local runtime blocker, but simulator evidence is not a signed IPA, TestFlight receipt, or real-iPhone verification.
...
- The exact-current-main source commit `7a2c32cb77c296b484ba4ce71177f350a59c3a47` Release simulator build succeeded under Xcode 26.6 and the iOS 26.5 SDK for destination iPhone 17e (`25423278-D058-4FD7-95FE-7BA695DA23CE`) with `CODE_SIGNING_ALLOWED=NO`. The built app reported `Side Quest Chess`, bundle `com.sidequestchess.app`, version/build `0.1.349` / `1`, minimum iOS `15.1`, device families iPhone and iPad, embedded `main.jsbundle`, and the aggregated required-reason privacy manifest. The same built `.app` installed and launched on that iPhone 17e and on iPad mini (A17 Pro) (`2DE5C1CA-35C5-4792-A4FF-F3209AA06986`); OCR read back the signed-out Side Quest Chess screen on both. Opening `sidequestchess://sso-callback` reopened the app and preserved that screen. The callback was not a completed provider authentication, and simulator evidence is not TestFlight or real iPhone evidence. Build logs contained React Native bundle warnings plus UIKit `UIScene` and CoreUI simulator-runtime diagnostics; no test target or `.xcresult` exists in the generated workspace.
...
| Generated native audit | Current source inspected; archive pending | Clean prebuild and Release simulator build inspected; repeat against the frozen signed IPA for Info.plist, entitlements, URL types, deployment target, SDKs and privacy manifests |
...
- `pnpm test`: PASS — 794 tests, 0 failures, 0 skipped/todo on exact current-main commit `7a2c32cb77c296b484ba4ce71177f350a59c3a47` before this receipt-contract test was added.
...
- Current local native receipt: Xcode 26.6 with the iOS 26.5 SDK selected; CocoaPods 1.17.0 `pod install` PASS with 85 dependencies/84 pods; aggregate generated privacy manifest inspected; exact-destination unsigned Release simulator build PASS for iPhone 17e; the same `.app` installed and launched on iPhone 17e and iPad mini; the registered custom callback reopened the iPhone app. OCR receipts preserved the signed-out Side Quest Chess screen. This is an installable simulator artifact, not a signed archive or TestFlight candidate, and no `.xcresult` was produced because the generated workspace has no test target.
- Xcode source compilation and bounded Simulator launch: PASSED. Native tests/`.xcresult`: not run because the generated workspace has no test target. Unsigned archive inspection: not attempted. Signed archive, TestFlight and physical-device smoke: BLOCKED by the separate Apple identity, account, signing and approval gates.
