...
**Source baseline:** `7a2c32cb77c296b484ba4ce71177f350a59c3a47` (`origin/main`, fetched and reconciled 2026-08-23); branch source-preparation changes remain under PR review<br>
**Branch reconciliation point:** `cbd5a969bfb32e180313d0f612e154315a843114`; later working-tree changes and this packet are not a frozen release candidate<br>
**Native build receipt baseline:** `b98ad94c3bc00fca8377cf371882660d965692bc`; at reconciliation point `cbd5a969bfb32e180313d0f612e154315a843114`, `apps/mobile` and `pnpm-lock.yaml` remain byte-identical to `b98ad94c3bc00fca8377cf371882660d965692bc`, but the unsigned Simulator receipt below is not an exact-HEAD build receipt<br>
...
- Publisher/controller target: Crowdler AB. Contractual minimum age 13; storefront ratings vary by region. No ads, IAP, subscriptions, or real-money prizes.
...
- Signed-out production readback on 2026-08-23 confirms that the live Support page exposes Crowdler AB, `sam@crowdler.com`, and Kvarnängsvägen 15, 182 47 Enebyberg, Sweden, plus an authenticated-message path. The direct-contact source/deployment gate is therefore passed; owner/legal must still confirm whether a telephone number is locally required.
...
- The iOS 26.5 Simulator runtime is now registered and available. Exact named destinations include iPhone 17 Pro and iPad Pro 13-inch (M5); machine-local simulator identifiers are intentionally omitted. This removes the former local runtime blocker but does not provide signing, archive, TestFlight, or physical-device evidence.
...
- At native receipt baseline `b98ad94c3bc00fca8377cf371882660d965692bc`, frozen `pnpm` dependencies, generated iOS source, and CocoaPods 1.17.0 completed. Current reconciliation point `cbd5a969bfb32e180313d0f612e154315a843114` has byte-identical native inputs (`apps/mobile` and `pnpm-lock.yaml`), but no exact-HEAD native build was run. At the receipt baseline, Debug and Release Simulator builds both passed under Xcode 26.6 for the exact iPhone 17 Pro destination; the Release build bundled JavaScript and produced `SideQuestChess.app` with bundle ID `com.sidequestchess.app`, version/build `0.1.349` / `1`, minimum iOS `15.1`, device families `1,2`, and the expected URL schemes. The same Release app installed and launched on iPhone 17 Pro and iPad Pro 13-inch (M5); synthetic callback dispatch succeeded and the iPhone process did not terminate. This does not prove app routing, callback handling, or authentication. Simulator receipts are unsigned and are not an IPA, TestFlight build, authenticated callback proof, visually accepted QA result, or real-device result.
...
| Generated native audit | Source receipt passed; exact IPA blocked | Clean prebuild diff and Simulator build inspected; repeat Info.plist, entitlements, URL types, deployment target, SDK and privacy-manifest inspection against the signed IPA |
...
| Privacy policy | Partial live readback; full compliance reconciliation blocked | Verify signed-out and in-app policy access plus every collected-data category, collection method, use, third-party protection, retention/deletion rule, and consent withdrawal/deletion request path against the exact provider/backend inventory; this is separate from App Privacy label reconciliation |
...
| Support URL contact | Live email/address readback passed; legal phone decision blocked | Production signed-out page exposes the verified legal address and direct email; include a telephone number if owner/legal confirms it is locally required |
...
Before adoption, verify the live field limits, unique name/SKU, territories, content rights, EU trader status, tax category, version/build, review contact, exact User Access selection, Made for Kids answer, and copyright ownership and year. Accessibility Nutrition Labels are voluntary as of 2026-08-23; publish only claims verified against Apple's common-task criteria, continue accessibility QA independently, and re-check whether Apple has announced a mandatory date before submission. “What’s New” is unavailable for the first App Store version and required for each later version. A worldwide consumer App Store launch requires Public Distribution; Private Distribution is limited to specifically identified organizations through Apple Business Manager or Apple School Manager and is not a private consumer launch. Owner must explicitly approve the applicable method before submission. Public versus Private Distribution cannot be changed after approval without a new app record and binary; conversion from public to unlisted is the limited exception. Also decide whether “All Countries or Regions” may opt the app into storefronts Apple adds later or whether to use a specific approved territory list. Treat Republic of Korea availability as a separate organization account gate: read back the live eligibility and rating state, and if Apple or GRAC requires one, preserve the Korea Rating Classification Number (Korea RCN) or approved override before including that storefront. For China mainland, obtain and verify the NPPA game approval number and supporting documents, and determine whether an MIIT ICP Filing Number is also required; if required, its metadata must match the App Store localization or primary language. For Vietnam, verify the required game-publishing license. Explicitly exclude any storefront whose required documentation is unavailable before submission.
...
- **What Andreas must do:** have the Crowdler AB Account Holder or Admin use account-wide visibility to search Apple Developer and App Store Connect for `com.sidequestchess.app`, Side Quest Chess, App IDs, app records and SKUs, and return a redacted duplicate/bundle/SKU receipt plus the highest existing iOS build number. Separately, confirm in writing the dedicated Crowdler/Sam Apple Account address and read back that already-provisioned identity's current role and app scope without changing it. An app-scoped App Manager cannot prove an account-wide absence of records it cannot view and must not be used as the sole duplicate-search authority.
...
- **Return receipt:** legal entity, Team ID, membership status/expiry; Account Holder/Admin account-wide duplicate-search result and role used for that readback; operator address and current role/app scope; App ID/app-record/SKU/bundle association; highest build number. Redact security and personal data.
...
- Contract-test receipts now lock the required-reason API upload gate, immutable distribution choice, China mainland/Vietnam/Republic of Korea territory requirements, account-wide duplicate-discovery authority, provider-specific SSO review-access rule, and Android/web predecessor launch-order gate after independent Apple-primary-source review.
- A historical direct local execution of the exact iOS prebuild command on the reconciled branch state preceding `cbd5a969bfb32e180313d0f612e154315a843114` generated `com.sidequestchess.app` with version/build `0.1.349` / `1`, deployment target `15.1`, device family `1,2`, URL schemes `sidequestchess` and `com.sidequestchess.app`, ATS arbitrary loads disabled, and the expected Apple sign-in entitlement. Its exact working-tree source SHA was not recorded, so it is not an exact-HEAD receipt; `apps/mobile` and `pnpm-lock.yaml` at reconciliation point `cbd5a969bfb32e180313d0f612e154315a843114` were byte-identical to the independently bound native receipt baseline above, while later source-preparation edits are not covered by that build. No `.xcprivacy` file was generated before dependency installation. The generated tree was inspected and removed with no generated tracked change retained.
...
- Signed-out Support contact TDD receipt: RED proved the Support screen omitted direct Crowdler contact information; GREEN renders Crowdler AB, `sam@crowdler.com`, and Kvarnängsvägen 15, 182 47 Enebyberg, Sweden without requiring sign-in. Signed-out production readback on 2026-08-23 confirmed those values; any telephone-number requirement remains an owner/legal gate.
...
- Google and Facebook cancellation TDD receipt: RED reproduced canceled or dismissed browser sessions being shown as sign-in failures and raw provider SDK details being exposed; GREEN treats both outcomes as a non-error and uses stable user-safe failure copy, so provider SDK details are not exposed. Exact-candidate Google and Facebook callback, cancellation, and error behavior remain not verified.
...
- Public-proof fixture TDD receipt: RED proved that the production decoder accepted any unsigned `preview-<challengeId>` route and rendered fixed sample data; GREEN removes that bypass and requires the normal signed-token format. The targeted test and full 800-test suite pass.
- `git diff --check`: PASS.
...
- Current local native receipt at `b98ad94c3bc00fca8377cf371882660d965692bc`: Xcode 26.6 with the iOS 26.5 SDK and iOS 26.5 Simulator runtime registered; CocoaPods 1.17.0; `pod install` PASS with 85 dependencies/84 pods; aggregate generated privacy manifest inspected; Debug Simulator build PASS; Release Simulator build PASS with bundled JavaScript. Release app executable SHA-256: `52f56f248a64ea15006aa50ebc78ef4f0f6dac9b88c9a8dcf31760bbe23af37d`; `main.jsbundle` SHA-256: `50b8ce23ddb84078f62aa897361de99913f1d7288441b2f1df9ed729558f2d7b`.
- Simulator launch receipt: iPhone 17 Pro on iOS 26.5 launch PASS; synthetic callback dispatch succeeded and the iPhone process did not terminate; iPad Pro 13-inch (M5) on iOS 26.5 launch PASS. Machine-local simulator identifiers are intentionally omitted. Captures were produced at `1206×2622` and `2064×2752`, but they were not visually accepted and are not listing assets. Simulator logs included a UIKit scene-lifecycle future-requirement warning and CoreUI theme-store errors; investigate them before treating launch as clean runtime evidence.
- Complete signed archive, IPA inspection, TestFlight, authenticated callback, `.xcresult` test evidence and real-device smoke: NOT PASSED / BLOCKED by the separate Apple legal identity, dedicated operator, account, signing and approval gates. Simulator receipts are unsigned and are not an IPA, TestFlight build, authenticated callback proof, visually accepted QA result, or real-device result.
...
- App Review Guidelines 5.1.1 privacy-policy requirements: https://developer.apple.com/app-store/review/guidelines/#data-collection-and-storage
