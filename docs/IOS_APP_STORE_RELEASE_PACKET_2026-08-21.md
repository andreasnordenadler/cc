...
- The live signed-out Support page now exposes Crowdler AB, `sam@crowdler.com`, and the legal address. Apple requires the Support URL to lead to actual contact information as required by local law, so owner/legal must still confirm whether a telephone number is locally required. The native signed-out support modal no longer presents an unusable authenticated message composer on this branch; it directs users to the public Support page instead.
...
- The iOS 26.5 Simulator runtime is now registered and available. Exact destinations include iPhone 17 Pro (`94D16E18-197E-43FD-A133-572FF0A7FBE4`) and iPad Pro 13-inch (M5) (`02189F8B-B2ED-49AF-83B5-E630C8059EB1`). This supersedes the earlier duplicate-runtime blocker without any destructive reset.
...
- On 2026-08-23, exact-destination Debug and Release Simulator builds passed under Xcode 26.6 for the iPhone 17 Pro destination. The Release build bundled JavaScript, validated the app, and produced a local Simulator `.app`; this is compilation evidence only, not a signed archive, IPA, TestFlight build, or device result.
...
| Privacy policy safety data | Production readback passed; label reconciliation blocked | Live signed-out policy includes report/block/deletion disclosures; exact-candidate processor, retention, SDK and label reconciliation remains blocked |
...
| Support URL contact | Production email/address verified; legal check pending | Production signed-out page exposes the legal address and direct email; owner/legal must determine whether a telephone number is locally required |
...
>
> To delete an account: My Account → Delete account → type `DELETE MY ACCOUNT` → Permanently delete account. After deletion succeeds, the app attempts local Clerk sign-out; if that cleanup fails, it confirms that deletion already completed and directs the user to close and reopen the app. [Retain only after verifying deletion of the entire account, associated personal data and shared user-generated content; provider-token revocation, any legally required retention, completion timing and confirmation must be disclosed and tested.]
>
...
### Exact owner action required now — account-wide discovery receipt

- **What Andreas must do:** have the Crowdler AB Account Holder/Admin perform the account-wide read-only search for `com.sidequestchess.app`, Side Quest Chess, app records and SKUs, report the highest existing iOS build number, and provide the legal entity, Team ID, and membership status/expiry. Separately confirm in writing the dedicated Crowdler/Sam Apple Account address and its current role/app scope if it is already provisioned on that exact team. Do not grant Certificates, Identifiers & Profiles access merely to perform this discovery: Apple states that access cannot be app-limited and exposes all app information.
...
- **Return receipt:** Account Holder/Admin identity by role only, legal entity, Team ID, membership status/expiry, duplicate-search results, App ID/app-record/SKU/bundle association and highest build number; separately, dedicated operator address plus current role/app scope. Redact security and unrelated personal data.
...
- **What it unblocks:** selecting a non-colliding build number and preparing a later app-scoped operator invitation/access approval. It does not authorize invitation, capability, certificate/profile, record creation, build, upload, tester, submission, or release actions.
...
- Signed-out Support contact TDD receipt: RED proved the Support screen omitted direct Crowdler contact information; GREEN renders Crowdler AB, `sam@crowdler.com`, and Kvarnängsvägen 15, 182 47 Enebyberg, Sweden without requiring sign-in. Production signed-out readback now confirms those details; the local-law telephone determination remains open.
...
- Current local native receipt: Xcode 26.6 with the iOS 26.5 SDK selected; CocoaPods 1.17.0 `pod install` PASS with 85 dependencies/84 pods; generated workspace `SideQuestChess.xcworkspace`; scheme `SideQuestChess`; Debug and Release Simulator build PASS at exact iPhone 17 Pro / iOS 26.5 destination. Generated Release identity readback: Side Quest Chess, `com.sidequestchess.app`, `0.1.349 (1)`. The same Release `.app` installed successfully; iPhone 17 Pro and iPad Pro 13-inch (M5), both iOS 26.5, launched it as a running process. OCR of settled captures found public “Side Quest Chess” and signed-out sign-in copy; captures were not visually reviewed and are not App Store screenshots. These and the other local execution receipts in this section are transient local observations, not durable release evidence: their raw logs, generated `.app`, captures, and `.xcresult` were not committed or otherwise preserved at a referenced evidence path. A signed archive was not produced.
- Complete Xcode compile: PASSED for the unsigned local Simulator candidate. XCTest/Swift Testing `.xcresult`, archive, Crowdler signing, TestFlight and real-device smoke: NOT PASSED / BLOCKED by the separate Apple identity, account, signing and approval gates.
