...
**Prepared:** 2026-08-21; reconciled 2026-08-24

**Current source:** `a2949393bdc18ba6ebda0067842fa71a520ba12f` (`origin/main`, fetched 2026-08-24); the iOS preparation lane was merged in `5a5cbd45`

**Status:** Release-preparation evidence only. This is not an App Store Connect record, Apple credential, IPA, TestFlight build, review submission, approval, or public release.
...
- No deliberate iOS build number is source-controlled. The EAS production profile has `autoIncrement: true` with local app-version sourcing, while a disposable clean prebuild currently defaults to build `1`; neither establishes the next available App Store Connect number or a reproducible first iOS candidate. No final privacy manifest report exists. There is no signed IPA, TestFlight build, or physical-iPhone callback proof.
- Support production readback passed on 2026-08-24: the signed-out `https://sidequestchess.com/support` page returned HTTP 200 and exposed Crowdler AB, `sam@crowdler.com`, and Kvarnängsvägen 15, 182 47 Enebyberg, Sweden. The retained web readback is `/Users/sam/Projects/sqc-evidence/ios-20260824-eda06968/logs/21-production-web-readback.log`; its timestamped response body has support body SHA-256 `3195258199e5864da4c5adb36e61302dd3b9fa6815c106eb35c08ebd06c5784a`. Apple requires the Support URL to lead to actual contact information as required by local law; owner/legal must still confirm whether a telephone number is required in any selected territory.
...
- Public-proof integrity and deletion are newly confirmed source blockers. `decodePublicProof` accepts unsigned `preview-` proof tokens for valid challenge IDs and renders fixed accepted proof data, so these non-production fixtures must not remain reachable as trusted public proof. Separately, signed shared proof URLs embed runner/game evidence without expiry or revocation lookup; shared proof URLs can remain readable after account deletion even though the app's deletion copy includes proofs. Before release, require cryptographically authenticated production proof only and implement a deletion-compatible proof revocation or expiry policy, with regression tests and truthful retention/deletion copy.
...
- The iOS 26.5 simulator runtime is now available and registered. On 2026-08-24, `simctl list runtimes` reported `com.apple.CoreSimulator.SimRuntime.iOS-26-5` available, and available devices included iPhone 17 Pro, iPhone 17 Pro Max, iPhone 17e, iPad Pro 13-inch (M5), and iPad mini (A17 Pro). This supersedes the earlier duplicate-disk-image blocker; it does not by itself prove the exact current candidate builds or launches.
...
- Historical receipt: an unsigned generic Simulator build under Xcode 26.6 reached app Swift/storyboard work but failed after a CoreSimulator interruption. The exact-current-source receipt below supersedes that local build blocker.
...
| Guideline 4.8 login equivalence | Source prepared; provider integration blocked | Crowdler App ID/provider configuration under separate approval; equivalent-login characteristics, account linking, cancellation, relay email, and physical-iPhone tests |
| Guideline 5.1.1(v) account deletion and provider revocation | Blocked | Apple authorization-token revocation implemented for account deletion; Google/Facebook data-access revocation mechanism; deletion and credential-revocation tests on the exact candidate |
...
| Privacy policy safety data | Public-page readback passed; labels blocked | Production readback passed on 2026-08-24: report and block disclosures are published. App Privacy answers and exact-binary/provider reconciliation remain blocked |
...
| Support URL contact | Public-page readback passed | Production signed-out page exposes the verified Crowdler AB legal address and direct email; owner/legal must decide whether a telephone number is locally required for selected territories |
...
1. Complete the prepared Sign in with Apple integration for Guideline 4.8: after explicit approval, reconcile the Crowdler App ID capability and Clerk/provider configuration and verify account linking, cancellation/error behavior, relay-email handling, and the equivalent-login characteristics on the exact TestFlight candidate. Separately satisfy Guideline 5.1.1(v): implement Apple authorization-token revocation in the account-deletion path and an in-app mechanism to revoke Google and Facebook credentials and disable their data access, then verify credential revocation and deletion on the exact candidate. Deleting only the Clerk user is not evidence that Apple authorization was revoked. Do not create capabilities or provider credentials without approval.
...
Field-limit receipt: Name 16/30 characters; Subtitle 28/30; Promotional Text 84/170; Description at most 4,000 characters and plain text only; Keywords 62/100 bytes with every keyword longer than two characters; Review Notes at most 4,000 bytes. The App Review phone must use international `+country-code` format and cannot be digits-only. Recheck all limits in the live form before adoption.
...
- `pnpm test`: PASS — 812 tests, 0 failures, 0 skipped/todo.
...
- Signed-out Support contact TDD receipt: RED proved the Support screen omitted direct Crowdler contact information; GREEN renders Crowdler AB, `sam@crowdler.com`, and Kvarnängsvägen 15, 182 47 Enebyberg, Sweden without requiring sign-in. Production signed-out readback passed on 2026-08-24.
...
- Exact-current-source native receipt (2026-08-24): the build ran from a clean isolated worktree at fetched `origin/main` `a2949393bdc18ba6ebda0067842fa71a520ba12f`. Frozen dependency installation passed, iOS prebuild passed, and CocoaPods 1.17.0 installed 85 dependencies/84 pods. The generated workspace is `SideQuestChess.xcworkspace`, scheme `SideQuestChess`, bundle ID `com.sidequestchess.app`, version/build `0.1.349` / `1`, deployment target iOS 15.1, SDK iOS 26.5, device family `1,2`, and configuration Release. The current retained local receipt is `/Users/sam/Projects/sqc-evidence/ios-20260824-a2949393/README.md`; the adjacent `.xcresult` and captures preserve the build and rendered-state evidence.
- Release simulator build on iPhone 17 Pro destination `94D16E18-197E-43FD-A133-572FF0A7FBE4`: **BUILD SUCCEEDED** with signing disabled, 0 errors, 0 analyzer warnings, and 535 warnings. The warning count is not a pristine gate: the result primarily reports generated Expo/React Native/Hermes/dependency and JavaScript-bundler warnings, which remain visible for remediation/review. The result bundle is `/Users/sam/Projects/sqc-evidence/ios-20260824-a2949393/artifacts/sqc-ios-a2949393-release.xcresult`. The Release artifact receipt records executable SHA-256 `4eac725767265816472b810f8f3b9ec2faca750003b8cc45f800fb21abf31c56` and `main.jsbundle` SHA-256 `7363ba1711f2e13d6ef3c054deebf53067bde9ca71c3c69d605449589b8e4d5f`.
- Bounded simulator launch smoke: the retained unsigned Release artifact `SideQuestChess-0.1.349-1-unsigned-simulator.zip` has SHA-256 `6ce9200ff137b0b81390fd6d1d0edab21fa0e80678fdb11ceb39a67bd75b0f0a`. A machine-generated linkage receipt at `/Users/sam/Projects/sqc-evidence/ios-20260824-a2949393/artifacts/install-launch-linkage.json` records source-app and installed-container hashes for the executable, `main.jsbundle`, Info.plist, and PrivacyInfo.xcprivacy; all four matched before launch on each device. The exact app installed and launched as `com.sidequestchess.app` on iPhone 17 Pro (`94D16E18-197E-43FD-A133-572FF0A7FBE4`, iOS 26.5; PID 5615) and iPad Pro 13-inch (M5) (`02189F8B-B2ED-49AF-83B5-E630C8059EB1`, iOS 26.5; PID 5908). Bounded OCR polling found `Side Quest Chess`, `Sign in to continue.`, `Browse Solo Side Quests`, and `Browse Multiplayer Side Quests` on both linked fresh captures. The iPhone capture was 1206×2622 and the iPad capture 2064×2752; these are QA evidence only, not App Store listing assets. Local captures are preserved under `/Users/sam/Projects/sqc-evidence/ios-20260824-a2949393/captures/`: iPhone SHA-256 `eec98b10954b118378af2995958f1b3ecc0ffb38ed9d4b0097789e491cbb12f2`; iPad SHA-256 `28a8296192697c8135e37f8ff670ce040657ada85a4b58c1f8a492f67bdf03de`.
- A prior simulator log capture contained a future `UIScene` lifecycle warning plus CoreUI theme-store and launch-measurement simulator faults. No process crash was observed in the bounded exact-current launch, but the earlier warnings need exact archive/device review and are not waived by a successful render.
- This is local unsigned Simulator support only: it is not TestFlight evidence and not physical-device evidence. Simulator interaction coverage, signed archive and signed/archive result evidence, App Store signing, TestFlight, authentication/deep-link/deletion smoke, and real-iPhone acceptance remain blocked or not performed. Signing, TestFlight, and physical-device work remain behind the separate Apple identity, account, signing, and approval gates.
...
- In-app account deletion and Sign in with Apple token revocation: https://developer.apple.com/support/offering-account-deletion-in-your-app/
