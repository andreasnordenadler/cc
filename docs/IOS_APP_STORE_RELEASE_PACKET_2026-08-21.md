...
**Source baseline:** source commit `7a2c32cb77c296b484ba4ce71177f350a59c3a47` (`origin/main`, fetched 2026-08-23); the original iOS source-preparation work merged in PR #285<br>
...
- The live Support page was re-read signed out on 2026-08-23 and exposes Crowdler AB, `sam@crowdler.com`, and the Enebyberg legal address. Apple requires the Support URL to lead to actual contact information as required by local law; owner/legal must still confirm whether a telephone number is locally required.
...
- The iOS 26.5 Simulator runtime is now registered and available. Exact iPhone 17 Pro and iPad Pro 13-inch (M5) simulators booted, installed, and launched the current unsigned Release Simulator app; this supersedes the earlier duplicate-runtime blocker without any simulator erase or Xcode reinstall.
...
- On 2026-08-23, unsigned Debug and Release Simulator builds succeeded for all 92 targets from source commit `7a2c32cb77c296b484ba4ce71177f350a59c3a47`, Xcode 26.6, iOS 26.5 SDK, and CocoaPods 1.17.0. The Release build bundled JavaScript, passed Xcode's simulator `-validate-for-store` step, installed and launched on iPhone 17 Pro and iPad Pro 13-inch (M5), and rendered the signed-out account surface on both. The callback scheme was accepted by Simulator and the app process remained running. This is local unsigned Simulator evidence only: there is no signed archive, IPA, TestFlight build, or physical-device evidence.
...
| Generated native audit | Source-generated and Simulator-compiled | Repeat on frozen candidate; inspect exact archive Info.plist, entitlements, URL types, deployment target, SDKs, signatures and privacy manifests |
...
| Privacy policy safety data | Deployed and signed-out read back | Provider/processor and exact-label reconciliation remain blocked |
...
| Support URL contact | Email/address deployed and read back | Confirm whether a telephone number is locally required; keep production contact reachable through review |
...
| Age Categories and Override | Not Applicable — provisional for a calculated 13+ result; “Made for Kids” is not the applicable selection for that result. Read back the live calculated rating before adoption |
| User Access | Limited Access — provisional; select only approved dedicated users for Side Quest Chess and read back the exact app-record selection |
| Content Rights | Yes — provisional because public chess records, player names, provider APIs and community content are used; owner/legal must confirm the app holds or is permitted to use every applicable right before adoption |
| Republic of Korea availability | Provisional worldwide intent; organization-account availability declaration and any game-rating/business evidence remain owner/legal gates before Korea is selected |
...
- Age Categories and Override: Not Applicable as the working draft for a calculated 13+ result; read back the live result and any override. Keep all screenshots and metadata suitable for the resulting audience.
- Advertising: No
...
- Profanity or Crude Humor; Mature or Suggestive Themes; Alcohol, Tobacco, or Drug Use or References; Sexual Content or Nudity; Horror/Fear Themes; Medical or Treatment Information; Health or Wellness Topics; Graphic Sexual Content and Nudity; Cartoon or Fantasy Violence; Realistic Violence; Prolonged Graphic or Sadistic Realistic Violence; Guns or Other Weapons: publisher-authored content is intended None for each descriptor, but reachable UGC must be moderated and audited before selecting the final frequency for every row.
...
- `pnpm test`: PASS — 795 tests, 0 failures, 0 skipped/todo on the final packet/test tree.
...
- Signed-out Support contact TDD receipt: RED proved the Support screen omitted direct Crowdler contact information; GREEN renders Crowdler AB, `sam@crowdler.com`, and Kvarnängsvägen 15, 182 47 Enebyberg, Sweden without requiring sign-in. Production deployment and signed-out readback were verified on 2026-08-23; telephone requirements remain an owner/legal gate.
...
- Current local native receipt: Xcode 26.6 with the iOS 26.5 SDK selected; CocoaPods 1.17.0 `pod install` PASS with 85 dependencies/84 pods; aggregate generated privacy manifest inspected; exact-destination unsigned Debug and Release Simulator builds PASS. The Release app installed and launched on iPhone 17 Pro (`94D16E18-197E-43FD-A133-572FF0A7FBE4`) and iPad Pro 13-inch (M5) (`02189F8B-B2ED-49AF-83B5-E630C8059EB1`) under iOS 26.5. OCR confirmed the rendered Side Quest Chess signed-out surface. Simulator screenshots were `1206×2622` and `2064×2752`; they are execution evidence, not approved App Store assets.
- Xcode compilation and bounded Simulator launch: PASSED. There are no native XCTest targets or test plans in the generated project, so no `.xcresult` was produced. Signed archive, IPA inspection, TestFlight and physical-device smoke remain NOT PASSED / BLOCKED by the separate Apple identity, account, signing and approval gates. Runtime logs also contain a future-facing `UIScene` lifecycle warning and CoreUI simulator faults; the app processes remained alive, but these must be retested on the frozen candidate and current Expo toolchain.
