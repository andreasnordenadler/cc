...
**Source baseline:** `c5ff2b038aafed06a8a4c2b6400a301c974e300b` (`origin/main`, fetched 2026-08-23); branch audit and source-preparation changes remain under PR review<br>
...
- Simulator registration is no longer locally blocked: `simctl` now reports the iOS 26.5 runtime and exact available iPhone/iPad destinations. This removes the former local runtime blocker only; it does not provide signing, archive, TestFlight, physical-device, or store evidence.
...
- On the exact current source baseline, an unsigned Debug Simulator build for iPhone 17e / iOS 26.5 compiled all 92 targets and succeeded under Xcode 26.6. The generated app installed and launched as `com.sidequestchess.app`; Metro bundled 974 modules. For `sidequestchess://sso-callback`, OS dispatch succeeded and the app remained running; route handling and provider authentication were not verified. This proves local source compilation, installation, JavaScript bundling, launch, and scheme registration only—not provider authentication, a signed archive, a release bundle, TestFlight, or physical-device behavior. Launch logs also warn that Expo's app delegate implements background-fetch/remote-notification handlers without corresponding `UIBackgroundModes`, and that the legacy non-`UIScene` lifecycle will become mandatory in a future SDK; both warnings require dependency/toolchain reconciliation before source freeze rather than speculative entitlement changes.
...
- Exact-current-source native receipt (`c5ff2b038aafed06a8a4c2b6400a301c974e300b`): frozen pnpm 11.12.0 install PASS with lockfile SHA-256 `5edd63864c5af4d5a85718e5af02bc769ae7634d8b8b60349a4f08c03d3317e9`; Xcode 26.6 / iOS 26.5 runtime PASS; CocoaPods 1.17.0 `pod install` PASS with 85 dependencies/84 pods; aggregate privacy manifest inspected; unsigned exact-destination Simulator build SUCCEEDED across 92 targets for iPhone 17e / iOS 26.5, UDID `25423278-D058-4FD7-95FE-7BA695DA23CE`; install and launch PASS as `com.sidequestchess.app`; Metro bundled 974 modules; for `sidequestchess://sso-callback`, OS dispatch succeeded and the app remained running, but route handling and provider authentication were not verified. The current generated app is `Side Quest Chess` `0.1.349 (1)`, minimum iOS 15.1, device family `1,2`, with the expected URL schemes and Apple sign-in entitlement. No valid local Apple signing identity exists, and no signed archive, IPA, `.xcresult`, TestFlight build, physical-device proof, or release screenshot was produced.
- Source tests on this exact baseline: `pnpm test` PASS — 797 tests, 0 failures, 0 skipped/todo; mobile typecheck PASS; Expo Doctor PASS — 18/18 checks.
- Simulator screenshots confirmed rendering after Metro bundling but contained persisted non-fixture account state; they were deleted and must not be committed or used as release evidence.
- Complete signed archive inspection, XCTest/XCUIAutomation `.xcresult`, TestFlight, real-iPhone smoke, iPad responsive matrix, provider callback authentication, and store evidence: NOT PASSED / BLOCKED by the separate Apple identity, account, signing and approval gates plus uncompleted candidate QA.
