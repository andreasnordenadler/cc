...
Before adoption, verify the live field limits, unique name/SKU, territories, content rights, EU trader status, tax category, version/build, review contact, accessibility declarations, and whether “What’s New” is required.
...
| Diagnostics / Other Diagnostic Data | App Functionality | Optional support bundle: application ID, app version/build, OS/platform and API destination |
...
The contractual minimum age is 13; Apple’s calculated regional rating is separate. Preserve the exact live questionnaire version, every answer, calculated global/regional ratings, and any override. A storefront override does not enforce account eligibility.
...
- Contests: unresolved frequency; rankings, goals, podiums and trophies exist, with no money or prizes
...
- iPad: use Apple's current required 13-inch iPad dimensions while `supportsTablet` remains enabled; equivalent truthful tablet frames. Re-read the specification at capture time because the extracted current reference was truncated before the iPad table.
...
TestFlight acceptance also requires: store-delivered installation; correct name/icon/version/build/API; no debug endpoint; no secrets/tokens in logs; non-expiring review fixtures; public chess fixture; second-account multiplayer; background/foreground/forced relaunch; successful deletion and signed-out relaunch. Internal testing, external TestFlight review, production App Review, approval, and public availability are separate states.
...
## 12. Verification receipt for this docs-only change

- `pnpm test`: PASS — 754 tests, 0 failures, 0 skipped/todo.
...
- App Store field structural check: PASS — name 16/30, subtitle 28/30, promotional text 84/170, keywords 62/100 characters; public listing draft contains no “SQC”.
- `git diff --check`: PASS.
