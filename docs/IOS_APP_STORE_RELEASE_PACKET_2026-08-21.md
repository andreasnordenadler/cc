...
**Source baseline:** `757698da73182520e4418e68165c5a5c4cb0e23a` (`origin/main`, fetched 2026-08-23); the source-preparation changes merged through PR #285 and this receipt was rerun from a new clean worktree at that exact commit<br>
...
- The live signed-out Support page was re-read from production on 2026-08-23 and exposes Crowdler AB, `sam@crowdler.com`, and the legal address. Production signed-out readback verified the direct email and address. Owner/legal must still confirm whether a telephone number is locally required; that legal question, not deployment, remains the Support URL gate.
...
- The iOS 26.5 Simulator runtime is now registered and available. Exact available destinations include iPhone 17 Pro Max, iPhone 17e, iPad Pro 13-inch (M5), and iPad mini (A17 Pro). This supersedes the earlier duplicate-disk-image registration failure without any destructive Simulator reset.
...
- At exact current main `757698da73182520e4418e68165c5a5c4cb0e23a`, an unsigned exact-destination Debug Simulator build under Xcode 26.6 succeeded across all 92 targets for iPhone 17e (`25423278-D058-4FD7-95FE-7BA695DA23CE`) on iOS 26.5. A standalone Release Simulator build with bundled JavaScript also succeeded for that device; its processed plist reports `Side Quest Chess`, `com.sidequestchess.app`, `0.1.349` / `1`, iOS `15.1`, device family `1,2`, and both expected URL schemes. The same Release app installed and launched on iPhone 17e and iPad Pro 13-inch (M5) (`02189F8B-B2ED-49AF-83B5-E630C8059EB1`). OCR-reviewed local captures exposed the signed-out entry on iPhone, iPad portrait, and iPad with `accessibility-extra-extra-extra-large` text under light system appearance. Opening `sidequestchess://sso-callback` displayed iOS's “Open in Side Quest Chess?” confirmation, proving scheme association but not an accepted handoff, Clerk/provider authentication, or callback completion. The artifact is unsigned Simulator evidence, not signed IPA, TestFlight, App Store screenshot, or real-device evidence.
...
| Generated native audit | Source receipt passed; archive blocked | Current-main clean prebuild, pods, Info.plist, entitlements, URL types, deployment target, SDKs and aggregate privacy manifest inspected; exact signed archive still required |
...
| Privacy policy safety data | Production readback verified | Live signed-out policy exposes the report, block and deletion disclosures; exact-candidate/provider data reconciliation remains blocked |
...
| App Store screenshot set | Blocked | Fresh same-candidate, state-rich captures in Apple's required iPhone and iPad sizes; local Simulator QA captures are not listing assets |
| Support URL contact | Email/address verified; legal phone decision blocked | Production signed-out page exposes Crowdler AB, `sam@crowdler.com`, and the legal address; owner/legal must decide whether a telephone number is locally required |
...
- `pnpm test`: PASS — 797 tests, 0 failures, 0 skipped/todo on the final branch (795 on the unmodified current-main baseline plus the packet contracts, including the machine-readable current-candidate native receipt).
...
- Signed-out Support contact TDD receipt: RED proved the Support screen omitted direct Crowdler contact information; GREEN renders Crowdler AB, `sam@crowdler.com`, and Kvarnängsvägen 15, 182 47 Enebyberg, Sweden without requiring sign-in. Production signed-out readback verified those fields on 2026-08-23; the telephone requirement remains an owner/legal gate.
...
- Current local native receipt at `757698da73182520e4418e68165c5a5c4cb0e23a`: Xcode 26.6 with iOS 26.5 SDK/runtime selected; CocoaPods 1.17.0; `pod install` PASS with 85 dependencies/84 pods; aggregate generated privacy manifest inspected; exact iPhone Debug and Release Simulator builds PASS; bundled JavaScript SHA-256 `272bb37105a1bb6d45ec09a2be3f95a5ababa62c824be4bfc79bc20655c3bd92`; the Release app installed and launched on iPhone 17e and iPad Pro 13-inch (M5). Three local OCR-reviewed captures cover iPhone signed-out entry, iPad portrait, and iPad at the largest accessibility text setting; their hashes are in `docs/evidence/ios-simulator-757698da.json`. The callback probe reached iOS's “Open in Side Quest Chess?” confirmation and was not accepted, so full URL handoff and authentication remain unverified. The receipt binds baseline Git/mobile tree identities and records source-file hashes, command exit states, destinations, inspected app identity, and local capture hashes. Multi-megabyte logs, Simulator app, and captures remain at machine-local paths, not as durable repository or release artifacts; remote review can verify the source binding and receipt schema but cannot independently replay those local files. No signed archive, IPA, `.xcresult`, provider-auth completion, TestFlight build, App Store screenshot set, or physical-device proof was produced.
- Complete Simulator source compilation, bundled Release build, bounded process launch, and limited signed-out iPhone/iPad rendering: PASSED. Swift Testing/XCTest/XCUIAutomation were not present in the generated app scheme, so no `.xcresult` is claimed. Landscape, split-view widths, authenticated journeys, accepted callback dispatch, provider authentication, archive, signing, TestFlight and real-device smoke remain BLOCKED or unverified; Simulator rotation was unavailable in the installed Xcode 26.6 `simctl io` interface and no landscape claim is made.
