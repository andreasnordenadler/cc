...
**Source baseline:** `3cf61fb1d5233a8899d5dfcd3a6caea4c2a8dc4a` (`origin/main`, fetched and reconciled 2026-08-25)<br>
**Status:** Drafts and current exact-commit verification only. This is not an App Store Connect record, Apple credential, IPA, TestFlight build, review submission, approval, or public release.
...
- The iOS 26.5 Simulator runtime (`23F77`) is registered and available. On the exact reconciled mobile source, an unsigned Debug Simulator build of all 92 targets succeeded, installed on the iPhone 17 Pro simulator, launched, bundled through Metro, and rendered the state-rich authenticated home. This is local source/build evidence only—not a signed IPA, TestFlight build, physical-device result, responsive-matrix pass, or release candidate.
...
- The superseding exact-current receipt used generated workspace `SideQuestChess.xcworkspace`, scheme `SideQuestChess`, Debug configuration, iOS Simulator SDK 26.5, and destination iPhone 17 Pro / iOS 26.5 / UDID `94D16E18-197E-43FD-A133-572FF0A7FBE4`. With `CODE_SIGNING_ALLOWED=NO` and a worktree-bounded DerivedData path, Xcode completed the 92-target dependency graph with `** BUILD SUCCEEDED **`. The app read back as bundle `com.sidequestchess.app`, short version `0.1.349`, build `1`; `simctl install` completed and `simctl launch` returned PID `84337`; Metro bundled `974 modules`; local OCR confirmed the application home rendered, after which the screenshot was discarded because the reused simulator contained non-fixture account data. This is not a signed IPA, TestFlight build, physical-device smoke, screenshot asset, or release-readiness receipt.
...
The implementation receipts below through the earlier 791-test result predate exact source baseline `3cf61fb1d5233a8899d5dfcd3a6caea4c2a8dc4a`; the later 811-test and native-build receipts are fresh exact-current evidence.
...
- Current local native receipt: Xcode 26.6 with the iOS 26.5 SDK selected; CocoaPods 1.17.0; runtime `23F77`; exact-current unsigned Debug Simulator build succeeded for iPhone 17 Pro / iOS 26.5 / UDID `94D16E18-197E-43FD-A133-572FF0A7FBE4`; the installed app launched, Metro bundled `974 modules`, and the application home rendered. The earlier CoreSimulator-interrupted build is superseded as a compile/launch receipt but retained as historical debugging context. No signed archive, IPA, TestFlight build, physical-device result, UI test `.xcresult`, or release candidate was produced.
- Complete release Xcode build, Simulator test matrix, `.xcresult`, archive, signing, TestFlight and device smoke: NOT PASSED. Exact-current local source compilation and one Simulator launch now pass; disk capacity remains narrow, and the separate Apple identity, legal account, duplicate, signing, approval, UGC, privacy, authentication, archive and real-device gates remain open.
- Current exact-commit documentation contract on 2026-08-25: PASS — 9 tests, after a RED failure proved the packet did not yet record the new build. Current full suite: PASS — 811 tests, 0 failures, 0 skipped/todo. Mobile TypeScript check: PASS. Expo Doctor after deleting the disposable generated iOS tree: PASS — 18/18; the expected intermediate 17/18 result while that tree existed correctly warned that managed Expo config would not be synced in a non-CNG project. The generated iOS project, bounded DerivedData, cloned dependency links, and non-fixture screenshot were not retained as source/evidence artifacts.
