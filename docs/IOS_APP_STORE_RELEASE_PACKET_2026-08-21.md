...
**Source baseline:** `3cdfc0cf90629b3dbd0a57b167ca6700be055159` (`origin/main`, fetched 2026-08-24); the current isolated branch adds release-safety fixes and remains unmerged<br>
**Status:** Drafts, verified source audit, and unsigned Simulator evidence only. This is not an App Store Connect record, Apple credential, signed IPA, TestFlight build, real-device smoke, review submission, approval, or public release.
...
- The iOS 26.5 runtime is now available and exact iPhone/iPad destinations are registered. This supersedes the earlier duplicate-disk-image registration blocker. No destructive runtime reset or Xcode reinstall was performed.
...
- Historical receipt: an earlier unsigned generic Simulator build compiled 92 targets but failed at `SplashScreen.storyboard` after a CoreSimulator interruption; a retry failed while the simulator runtime was not registered. That historical failure is superseded by the current exact-destination build below, but remains recorded rather than rewritten as a pass.
- Current receipt provenance: the build came from an uncommitted isolated working tree based on `3cdfc0cf90629b3dbd0a57b167ca6700be055159`; the built `apps/mobile/App.tsx` SHA-256 was `13e439a912b7c3a4f35ed57b4704aebd9341a3d9986e284c4464d0cb0ddc8488`. The retained local receipt and captures are documented at `/Users/sam/Projects/sqc-evidence/ios-20260824-v20/README.md`. This is not a frozen source commit.
- Current receipt: an unsigned **Release Simulator build** for iOS 26.5 and exact destination iPhone 17 Pro bundled a `3,724,407`-byte `main.jsbundle` (SHA-256 `7b1b86cf1f34f6372191270f8e484459b08ba1168e84a684790f2eaf54aa6bb4`), produced a 97 MB `.app`, passed Xcode's simulator validation, and succeeded under Xcode 26.6. This is unsigned Simulator evidence only. No signed archive, IPA, TestFlight build, or real-device evidence exists.
- The built app installed and launched on iPhone 17 Pro (`94D16E18-197E-43FD-A133-572FF0A7FBE4`) and iPad Pro 13-inch (M5) (`02189F8B-B2ED-49AF-83B5-E630C8059EB1`), both on iOS 26.5. OCR read the signed-out Side Quest Chess screen and sign-in controls on both captures. The 13-inch iPad capture was `2064×2752`; these are QA receipts, not approved App Store screenshots.
- Opening `sidequestchess://sso-callback` caused iOS to display its system confirmation sheet to open Side Quest Chess. This is scheme recognition only. The sheet was not accepted, so app delivery, callback completion, and authentication were not proved; provider redirect, session activation, cancellation, and cold-start callback behavior remain blocked pending approved provider configuration and real-iPhone TestFlight smoke.
- Simulator logs emitted a future-required `UIScene` lifecycle fault and CoreUI theme-registration faults. Their release impact is unresolved and they remain a pre-freeze reconciliation gate; this receipt does not relabel them as harmless merely because the build and launch completed.
...
- `pnpm test`: PASS — 814 tests, 0 failures, 0 skipped/todo on the current branch.
...
- Operational support mailbox TDD receipt: RED proved the signed-out web email composer still targeted Andreas's personal mailbox even though the visible Crowdler contact card used `sam@crowdler.com`; GREEN now sends the mailto handoff to the Crowdler operational mailbox and rejects the personal address in that component.
- Non-personal review-preview TDD receipt: RED proved the mobile preview fixture and a public display-name placeholder used a real team member's name, email, and chess username; GREEN uses clearly fictional preview data (`Review Knight`, `review-knight@example.com`, `reviewknight`) and a neutral placeholder. This prepares safe previews but does not create reviewer credentials or approve screenshots.
...
- Current local native receipt: Xcode 26.6 with the iOS 26.5 SDK selected; CocoaPods 1.17.0 installation PASS; `pod install` PASS with 85 dependencies/84 pods; aggregate generated privacy manifest inspected; unsigned bundled Release Simulator build PASS on exact iPhone 17 Pro destination; the same `.app` installed/launched on exact iPhone 17 Pro and iPad Pro 13-inch destinations. The historical CoreSimulator interruption is superseded. The exact baseline, mobile-source hash, bundle hash, capture hashes and limitations are retained in `/Users/sam/Projects/sqc-evidence/ios-20260824-v20/README.md`.
- Simulator compile/install/launch is now passed at the declared iOS 26.5 destinations. No Xcode test target or test plan exists, so no Swift/XCTest test-result `.xcresult` was produced in the v20 run; historical build-result bundles from other source revisions are separate evidence. Signed archive, IPA, Apple capability/account reconciliation, TestFlight, provider auth, real-device smoke, submission and release remain NOT PASSED / approval-blocked.
