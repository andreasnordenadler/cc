...
**Source baseline:** `a161663a041f91306f4970b735525c403f3d48d1` (`origin/main`, fetched 2026-08-23); the source-preparation changes merged through PR #285 and this receipt was rerun from a new clean worktree at that exact commit<br>
...
- Exact-destination unsigned Debug Simulator builds under Xcode 26.6 succeeded across all 92 targets for iPhone 17e (`25423278-D058-4FD7-95FE-7BA695DA23CE`) and iPad Pro 13-inch (M5) (`02189F8B-B2ED-49AF-83B5-E630C8059EB1`) on iOS 26.5. A standalone Release Simulator build with bundled JavaScript also succeeded for iPhone 17 Pro Max (`3511296F-A745-4B56-8913-EA454DA8420E`); its processed plist reports `Side Quest Chess`, `com.sidequestchess.app`, `0.1.349` / `1`, iOS `15.1`, device family `1,2`, and both expected URL schemes. Installation and launch succeeded, and an `xcrun simctl openurl` handoff to `sidequestchess://sso-callback?test=bounded` succeeded. This proves compile, install, process launch, and OS URL dispatch only—not Clerk/provider authentication or callback completion. The artifact is unsigned Simulator evidence, not signed IPA evidence, TestFlight evidence, screenshots, or real-device evidence.
...
- `pnpm test`: PASS — 796 tests, 0 failures, 0 skipped/todo on the final branch (794 on the unmodified current-main baseline plus the packet contracts, including the machine-readable current-candidate native receipt).
...
- Current local native receipt at `a161663a041f91306f4970b735525c403f3d48d1`: Xcode 26.6 with iOS 26.5 SDK/runtime selected; CocoaPods 1.17.0; `pod install` PASS with 85 dependencies/84 pods; aggregate generated privacy manifest inspected; Debug Simulator builds PASS on exact iPhone and 13-inch iPad destinations; standalone Release Simulator build PASS with bundled JavaScript SHA-256 `a0024e6da7e50417b27e27428fb84fc9d3e673bcf9beead67783889552a8a7d9`; iPhone 17 Pro Max install, launch, and `sidequestchess://sso-callback` OS handoff PASS. `docs/evidence/ios-simulator-a161663a.json` binds the baseline Git/mobile tree identities and records source-file hashes, local command exit states/log hashes, destinations, and inspected app identity. The multi-megabyte logs and Simulator app remain at the receipt's machine-local paths, not as durable repository or release artifacts; remote review can verify the source binding and receipt schema but cannot independently replay those local files. No signed archive, IPA, `.xcresult`, provider-auth completion, screenshots, TestFlight build, or physical-device proof was produced.
- Complete Simulator source compilation and bounded process launch: PASSED. Swift Testing/XCTest/XCUIAutomation were not present in the generated app scheme, so no `.xcresult` is claimed. Archive, signing, TestFlight and real-device smoke remain BLOCKED by the separate Apple legal team, dedicated identity, account, signing and approval gates—not by local Simulator registration.
