...
- Historical attempt: an unsigned generic Simulator build under Xcode 26.6 failed at `SplashScreen.storyboard` after the CoreSimulator service connection was interrupted, and its immediate retry could not see the downloaded runtime. After runtime registration recovered, the exact current branch passed unsigned Debug and Release Simulator builds for the `SideQuestChess` scheme on iPhone 17 Pro / iOS 26.5. The self-contained Release app was installed and launched on iPhone 17 Pro, iPad mini (A17 Pro), and iPad Pro 13-inch (M5); OCR of each signed-out capture found the expected Side Quest Chess sign-in screen. This is local unsigned Simulator evidence only. No signed archive, TestFlight build, or physical-iPhone evidence exists.
...
- Current local native receipt at branch commit `f628e8324f1b5bdab0d441430bb9b352e2067987`: Xcode 26.6 with the iOS 26.5 SDK and registered iOS 26.5 (`23F77`) runtime; CocoaPods 1.17.0; `pod install` PASS with 85 dependencies/84 pods; aggregate generated privacy manifest inspected; exact `SideQuestChess` scheme, Release configuration, bundle `com.sidequestchess.app`, version/build `0.1.349` / `1`, deployment target `15.1`, device family `1,2`, URL schemes `sidequestchess` and `com.sidequestchess.app`, and simulator destinations discovered rather than guessed. Exact-destination unsigned Release Simulator build PASS on iPhone 17 Pro / iOS 26.5 with a bundled `main.jsbundle`. No code-signing identities were installed, so this is not a signed candidate or archive.
- The commit-`f628e8324f1b5bdab0d441430bb9b352e2067987` unsigned Release Simulator app installed and launched as PID `69152` on iPhone 17 Pro (`94D16E18-197E-43FD-A133-572FF0A7FBE4`) / iOS 26.5. Readback from the built app returned display name `Side Quest Chess`, bundle `com.sidequestchess.app`, and version/build `0.1.349` / `1`; the fresh launch capture was `1206×2622`. Earlier branch evidence also launched on iPad mini (A17 Pro) and iPad Pro 13-inch (M5), but those iPad launches were not repeated after the main reconciliation because the merged change affected only web CSS and its contract test. These captures are local launch evidence, not approved App Store screenshots or responsive interaction acceptance.
