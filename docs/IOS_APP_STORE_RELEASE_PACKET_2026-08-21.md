...
**Source baseline:** `3cf61fb1d5233a8899d5dfcd3a6caea4c2a8dc4a` (`origin/main`, fetched and merged into this packet branch 2026-08-25). The iOS release-preparation lane merged in `5a5cbd456900f3e75d4a9cbc9e7769c157b1d83b`; later current-main changes do not close binary, device, account, or release gates.<br>
...
- The live signed-out Support page now exposes Crowdler AB, `sam@crowdler.com`, and the legal address, and the live Privacy page contains the prepared report/block and deletion disclosures. Production deployment and signed-out readback passed on 2026-08-24. Provider-specific processing wording still unresolved: the policy does not clearly name direct Apple, Google, and Facebook sign-in processing. Apple requires the Support URL to lead to actual contact information as required by local law. A telephone number remains an owner/legal gate for every intended storefront.
...
- Full Xcode 26.6 (build `17F113`) is now selected at `/Applications/Xcode.app/Contents/Developer`, its first-launch components completed installation, and the iOS 26.5 SDK is visible. This satisfies the version floor for local source compilation under Apple's upload-toolchain requirement in force since 2026-04-28, but it is not archive or upload evidence. CocoaPods 1.17.0 is now installed. No valid local Apple signing identity is verified.
- The iOS 26.5 simulator runtime is now registered, with available iPhone and iPad devices. This clears only the former local runtime-registration blocker. It does not establish an exact-current launch pass, signed archive, TestFlight delivery, or physical-device behavior.
...
- The historical unsigned generic Simulator build failed at `SplashScreen.storyboard` after a CoreSimulator interruption. A later Release Simulator build from the materially same mobile source compiled and installed, but reproduced a launch crash; that remains historical failure evidence rather than a current-main acceptance receipt.
- Unmerged PR #337 at `736bef39f92f14f3753a0b1c74180220a93ff630` adds a fail-closed Hermes artifact checker and a macOS CI job. Its unsigned Release Simulator build passed under Xcode 26.6 and verified arm64/x86_64 Hermes slices, but the job does not install or launch the app. It therefore does not prove that the launch crash is resolved. A signed archive is not verified, and PR #337 must remain distinct from current `origin/main` until separately reviewed and merged under the repository authorization boundary.
...
| Generated native audit | Build-only CI candidate under review | Merge/reconcile the generated-settings gate and PR #337; then rerun clean prebuild/build/install/launch on the exact frozen source and inspect Info.plist, entitlements, URL types, deployment target, SDKs and privacy manifests |
...
| Privacy policy safety data | Production disclosure deployed; provider wording blocked | Production deployment and signed-out readback passed for report/block and deletion disclosures; provider-specific processing wording still unresolved |
...
| Support URL contact | Email/address readback passed; legal gate open | Production signed-out page exposes Crowdler AB, `sam@crowdler.com`, and the verified legal address; owner/legal must determine whether a telephone number is required for intended storefronts |
| Accessibility Nutrition Labels | Draft only | Do not claim VoiceOver, Voice Control, Larger Text, Reduced Motion, or another label until every common task passes source and exact-candidate device verification |
...
1. Complete the prepared Sign in with Apple integration: implement Apple authorization-token revocation in the account-deletion path, then, after explicit approval, reconcile the Crowdler App ID capability and Clerk/provider configuration and verify account linking, cancellation/error behavior, relay-email handling (including the unified `private.icloud.com` relay domain), credential revocation, and deletion on the exact TestFlight candidate. Deleting only the Clerk user is not evidence that Apple authorization was revoked. Verify whether Clerk or Side Quest Chess retains usable Google/Facebook authorization after deletion; revoke or disable that access where retained and reconcile provider terms. Treat this as a privacy/integration gate, while Apple's explicit provider-token requirement in its account-deletion guidance applies to Sign in with Apple. Do not create capabilities or provider credentials without approval.
...
11. Preserve the PR #337 Hermes Release-artifact gate if it is approved, but extend final evidence beyond compilation: install and launch the exact Release app, exercise bounded relaunch/background cycles, and run the same artifact check against the eventual archive. Do not infer runtime stability from an unsigned build-only CI pass.
...
| Privacy Choices URL | `https://sidequestchess.com/privacy#choices` — optional draft; adopt only after App Store Connect review confirms the live anchor and owner approves |
...
Beginning with September 2026 submissions, Apple requires the social-media questionnaire for new apps and updates. Before any September 2026-or-later submission, complete it and preserve the live receipt. Side Quest Chess's Community discovery, likes, profiles, and public posting support the working answer **Yes**; “disabled for users under 13” remains **No** unless the Declared Age Range API and age-appropriate delivery are implemented and verified. Recheck Korea before an October 2026-or-later submission: Apple has announced that infrequent profanity/crude humor and infrequent mature/suggestive themes move from `All` to `12+` there, and reachable UGC frequencies must be based on the moderated candidate rather than publisher intent.
...
- App icon: the configured source `app-icon-light-blue.png` passes the 1024×1024 icon dimension and no alpha checks. Visual approval and proof that the exact archive contains this asset remain blocked; inspect the rendered artwork for stale names or abbreviations and preserve an archive identity/hash receipt.
...
Accessibility Nutrition Labels are voluntary at this baseline, but any adopted claim must work across every common task, including first launch, login/reset, primary gameplay, settings, support, and deletion. Source labels are not sufficient evidence. In particular, do not claim Reduced Motion until continuously looping animations and all other motion are verified against the system preference; preserve source and exact-candidate device receipts for every claimed feature.
...
- Current-receipt contract TDD: RED reproduced the stale `a2949393` baseline, resolved simulator-registration blocker, and absent build-versus-launch distinction; GREEN binds the packet to current `origin/main`, records the registered runtime, and keeps PR #337's unsigned build-only evidence separate from launch, archive, TestFlight, and device evidence.
...
- `pnpm test`: PASS — 814 tests, 0 failures, 0 skipped/todo on the reconciled 2026-08-25 tree.
...
- Signed-out Support contact TDD receipt: RED proved the Support screen omitted direct Crowdler contact information; GREEN renders Crowdler AB, `sam@crowdler.com`, and Kvarnängsvägen 15, 182 47 Enebyberg, Sweden without requiring sign-in. Production deployment and signed-out readback passed on 2026-08-24; the storefront-specific telephone requirement remains an owner/legal gate.
...
- Current local tooling receipt: Xcode 26.6 with the iOS 26.5 SDK selected; CocoaPods 1.17.0 installed; the iOS 26.5 simulator runtime registered; available iPhone and iPad destinations visible; no valid local code-signing identity.
- PR #337 build-only receipt: macOS CI at `736bef39f92f14f3753a0b1c74180220a93ff630` passed an unsigned Release Simulator build and Hermes architecture/UUID/symbol checks. The job does not install or launch the app, and it is not current-main, archive, signing, TestFlight, or device evidence.
- Complete exact-current Simulator journey, `.xcresult`, signed archive, TestFlight and device smoke: NOT PASSED / BLOCKED by unmerged source gates plus the separate Apple identity, account, signing and approval gates.
...
- Upload toolchain requirement in force since 2026-04-28: https://developer.apple.com/news/upcoming-requirements/?id=04282026a
