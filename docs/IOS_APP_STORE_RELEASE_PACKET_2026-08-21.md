...
**Source baseline:** `5aee72552afd8c33496eb536a8bc190032cf7e69` (`origin/main`, fetched 2026-08-25). The original packet landed in `5a5cbd45`; section 12 distinguishes historical branch receipts from current-baseline evidence.<br>
...
- The iOS 26.5 Simulator runtime (`23F77`) is registered. After removing only a stale 2.4 GiB Side Quest Chess DerivedData cache, the exact current baseline completed a frozen dependency install, iOS prebuild, CocoaPods install, Xcode Simulator build, install, launch, and Metro bundle. The iPhone 17e on iOS 26.5 build succeeded with zero errors and 483 dependency/toolchain warnings; no XCTest or Swift Testing targets executed. This is current compilation and launch evidence, not archive, Apple signing, TestFlight, physical-iPhone, authentication, deep-link, responsive, or release evidence.
- A 2026-08-26 host check found less than 1 GiB of free disk space. A fresh native build is blocked by inadequate disk headroom; the 2026-08-25 baseline receipt remains historical evidence rather than a current-candidate rerun. Do not delete DerivedData or Simulator devices to clear space without a bounded ownership and evidence review.
...
Unless a receipt below explicitly names baseline `5aee7255`, it is historical evidence preserved from earlier source-preparation branches, not proof that the command passed on current `origin/main`.
...
- Simulator compilation and basic launch: PASSED on the declared current-baseline destination. Simulator tests, archive, Crowdler signing, IPA inspection, TestFlight and device smoke: NOT PASSED / BLOCKED by the separate source, Apple identity, account, signing and approval gates.
- 2026-08-26 local capacity receipt: full Xcode and the iOS 26.5 runtime remain available, but the root volume had less than 1 GiB free. No new prebuild, CocoaPods install, Xcode build, Simulator boot, archive, or account-authenticated EAS action was attempted under that unsafe headroom.
