...
**Source baseline:** `5aee72552afd8c33496eb536a8bc190032cf7e69` (`origin/main`, fetched 2026-08-25); this exact-current candidate is the source-audit baseline, while any branch changes and source freeze remain under PR review<br>
...
- Signed-out production readback on 2026-08-25 verified that the live Support page exposes Crowdler AB, `sam@crowdler.com`, and the legal address, and that the live Privacy page discloses report and block records plus the current deletion asymmetry. This closes only the prior deployment/readback gaps; owner/legal must still confirm whether a telephone number is locally required, and exact-candidate in-app link behavior remains a QA gate.
...
- Simulator registration is no longer blocked: on 2026-08-25, `simctl` reported the iOS 26.5 runtime registered and available, with exact available destinations including iPhone 17 Pro Max and iPad Pro 13-inch (M5). The exact-current unsigned Release Simulator build now passes and launches on both destinations as recorded below. This remains local Simulator evidence—not signing, TestFlight, real-device, responsive-matrix, or store evidence.
...
- The 2026-08-22 unsigned generic Simulator attempt failed after a CoreSimulator interruption and remains only historical failure evidence. On 2026-08-25, the exact-current source baseline completed clean prebuild, CocoaPods installation, and both Debug and Release Simulator builds after the frozen dependency tree was materialized inside the isolated worktree. The Release product embeds `main.jsbundle`; no Metro server is required to launch it.
...
| Generated native audit | Exact-current Simulator source/build audit passed; archive audit blocked | Clean prebuild and pod diff inspected; generated Info.plist, entitlements, URL types, deployment target, SDK and aggregate privacy manifest inspected. Repeat against exact signed archive |
...
| Privacy policy safety data | Production readback verified; candidate QA blocked | Signed-out production exposes the report/block and deletion-asymmetry disclosures; verify exact-candidate in-app navigation and reconcile final labels |
...
| Support URL contact | Production email/address readback verified; legal/QA pending | Production signed-out page exposes Crowdler AB, the verified legal address and direct email; confirm telephone requirement and exact-candidate navigation |
...
| EU trader status | Blocked | Owner/legal self-assessment and trader-status declaration at the app-record level even if the app is not distributed in the EU. If Crowdler AB declares trader status, approval-gate verification of the organization's phone number and email, payment-account details if absent, EU-law compliance certification, and business/address evidence before any submission |
...
- PR #349 merge-ref CI: PASS — 814 tests, 0 failures, 0 skipped/todo; `pnpm build`: PASS. This receipt applies to GitHub's synthetic merge commit `d88eb962afacdfd03e9e45ad0115add9bd157671`, not to an unmodified checkout of the exact `5aee72552afd8c33496eb536a8bc190032cf7e69` baseline.
...
- Signed-out Support contact TDD receipt: RED proved the Support screen omitted direct Crowdler contact information; GREEN renders Crowdler AB, `sam@crowdler.com`, and Kvarnängsvägen 15, 182 47 Enebyberg, Sweden without requiring sign-in. The 2026-08-25 production readback recorded above verifies that contact information live; exact-candidate in-app navigation remains a separate QA gate.
...
- Prior local native receipt: Xcode 26.6 with the iOS 26.5 SDK selected; first-launch setup PASS; CocoaPods 1.17.0 installation PASS; `pod install` PASS with 85 dependencies/84 pods; aggregate generated privacy manifest inspected. That older-source unsigned build failed after a CoreSimulator interruption and is not evidence for the exact-current candidate.
- Exact-current candidate `5aee72552afd8c33496eb536a8bc190032cf7e69`: clean `expo prebuild --platform ios --no-install` PASS; `pod install` PASS with 85 dependencies/84 pods; generated app identity `Side Quest Chess`, `com.sidequestchess.app`, version/build `0.1.349` / `1`, iOS minimum `15.1`, device family `1,2`, URL schemes `sidequestchess` and `com.sidequestchess.app`, Apple sign-in entitlement, ATS arbitrary loads disabled, and aggregate `PrivacyInfo.xcprivacy` inspected.
- Local-only Simulator narrative receipt: the supporting captures and build artifacts were not retained in this repository or PR, so the following hashes, OCR observations, destinations, and build result are non-reproducible local evidence rather than durable CI, archive, TestFlight, or store receipts.
- Exact-current unsigned Release Simulator build: PASS under Xcode 26.6 / iOS 26.5 for destination `3511296F-A745-4B56-8913-EA454DA8420E` (iPhone 17 Pro Max). The Release product embeds `main.jsbundle`; Debug also compiled but correctly required Metro at launch. Two initial Release attempts failed only because the temporary cross-worktree dependency symlink layout was outside Metro's resolver root; recreating the frozen lockfile-identical dependency tree inside the isolated worktree resolved the environment issue without source or lockfile changes.
- iPhone 17 Pro Max, iOS 26.5: install PASS; launch PASS; process remained alive; OCR verified the signed-out `Side Quest Chess` screen and both browse actions. Fresh local capture `1320×2868`, SHA-256 `a6f91d57b542ad73c7775b7d7a9ea7ea6fe77769080d3e381696b68e19cc0833`.
- iPad Pro 13-inch (M5), iOS 26.5: install PASS; launch PASS; process remained alive; a ready-state OCR readback verified the signed-out Side Quest Chess screen and both browse actions. Fresh portrait capture `2064×2752`, SHA-256 `7590820f9546449037f008098c1149cd2a545f75a4bb762f417358e16df4bfb6`. Command-line rotation was unavailable in this `simctl`, so landscape and multitasking widths remain blocked rather than inferred.
- Both Simulator launches logged the runtime warning that `UIScene` lifecycle will become required and CoreUI theme-store diagnostics. Neither process exited during the bounded launch check, but those warnings require exact-archive/current-toolchain review. This unsigned local product is not an IPA, TestFlight build, real-iPhone smoke, screenshot set, or App Store candidate. Simulator tests, `.xcresult`, archive, signing, TestFlight and device smoke remain NOT PASSED; account, signing, upload and release steps remain separately approval-gated.
