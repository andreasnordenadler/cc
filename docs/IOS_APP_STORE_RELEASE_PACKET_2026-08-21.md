...
**Source baseline:** `7a2c32cb77c296b484ba4ce71177f350a59c3a47` (`origin/main`, fetched and verified unchanged immediately before native generation on 2026-08-23); this is a current source-readiness baseline, not a frozen submission candidate<br>
...
- The iOS 26.5 Simulator runtime is now registered and available. Current-main Debug and Release Simulator builds completed on an exact iPhone 17e destination, and the bundled Release app launched without Metro on iPhone 17e, iPad mini (A17 Pro), and iPad Pro 13-inch (M5). This resolves the former local runtime blocker only; Simulator evidence does not prove signing, archive validity, TestFlight delivery, physical-device authentication, or release readiness.
...
On 2026-08-23, a new isolated worktree at exact current `origin/main` commit `7a2c32cb77c296b484ba4ce71177f350a59c3a47` completed the repository-pinned pnpm 11.12.0 frozen install, 794/794 source tests, mobile TypeScript, Expo Doctor 18/18, unsigned iOS prebuild, and CocoaPods 1.17.0 installation. Xcode 26.6 then completed Debug and bundled Release Simulator builds for iPhone 17e / iOS 26.5. Readable result bundles are `/tmp/sqc-ios-current-build.xcresult` and `/tmp/sqc-ios-current-release-build.xcresult`: both report `succeeded`, zero errors, zero analyzer warnings, and respectively 483 and 517 build warnings, predominantly from generated bundles, dependencies, and the toolchain. The Release `.app` contains `main.jsbundle`, launched as `com.sidequestchess.app`, and reached the signed-out sign-in surface without Metro on iPhone 17e, iPad mini (A17 Pro), and iPad Pro 13-inch (M5); OCR-checked captures are under `/tmp/sqc-ios-current-*-settled.png`. These temporary local paths are supporting evidence for this run, not durable CI artifacts or App Store screenshots. Lint completed with zero errors and four pre-existing warnings. No Apple account, signing identity, archive, upload, or external record was used.
...
- The former transient storyboard/runtime failures are superseded for source compilation by the 2026-08-23 current-main Debug and Release Simulator passes. The current generated source still defaults to version/build `0.1.349` / `1`, links the Sign in with Apple entitlements file into both target configurations, declares deployment target iOS 15.1 and device families iPhone/iPad, and exposes URL schemes `sidequestchess` and `com.sidequestchess.app`. Simulator ad-hoc signatures expose no effective Apple Sign in entitlement, as expected without an Apple team; only an approved Crowdler-signed device/archive build can prove that capability. The aggregated app privacy manifest still reports tracking `false`, no native collected-data rows, and the required-reason API categories already listed above. Archive inspection, signing, physical-device behavior, and TestFlight remain blocked.
...
| Generated native audit | Current-main source pass; frozen-candidate audit pending | Repeat on the frozen candidate; preserve clean prebuild diff, Info.plist, entitlements, URL types, deployment target, SDKs, privacy manifests and readable build result |
...
“13+” is the contractual/in-app minimum, not a promise of one uniform storefront rating. Under Apple's OS 26-and-later rating system, Social Media answered Yes indicates higher regional ratings including Australia 16+, Brazil A16, Vietnam 16+, and Korea 15+. Earlier operating systems can display different ratings, so preserve both sets of live calculated regional receipts before territory approval.
...
Capture fresh images from the exact selected build with fictional, non-personal account data that truthfully represents the candidate. Initial locale: English (U.S.). Add localizations only with reviewed field copy and asset inheritance. The screenshot set must accurately show the app in use and must not consist merely of title art, splash, or login screens; keep every frame suitable for a 4+ metadata audience regardless of the calculated app age rating.
...
- `pnpm test`: PASS — 794 tests, 0 failures, 0 skipped/todo.
...
- Current local native receipt: Xcode 26.6 with the iOS 26.5 SDK/runtime selected; first-launch setup PASS; CocoaPods 1.17.0 installation PASS; `pod install` PASS with 85 dependencies/84 pods; aggregate generated privacy manifest inspected; exact-destination Debug and bundled Release Simulator builds PASS on iPhone 17e / iOS 26.5. The readable Debug and Release `.xcresult` bundles report zero errors and zero analyzer warnings. The bundled Release app installed and reached the signed-out sign-in surface on iPhone 17e, iPad mini (A17 Pro), and iPad Pro 13-inch (M5). No Simulator test suite, signed archive, or store-installable candidate was produced.
- Source compilation, `.xcresult` creation, and bounded signed-out Simulator launches: PASSED for this current-main baseline. Frozen-candidate native audit, full Simulator journeys, archive, Crowdler signing, TestFlight, physical-device authentication, and real-device smoke: NOT PASSED / BLOCKED by the separate source, UGC, provider-revocation, Apple identity, account, signing, and approval gates.
