# CC Next Verifier-Backed Challenge Slice
...
Define the smallest post-catalog challenge expansion now that all three currently shipped challenges have real automated Lichess verdicts.

## Exact next challenge to add

Add `finish-as-white` as the next verifier-backed challenge.

## Why this is the smallest reviewable next step

- it grows the catalog by exactly one challenge
- it reuses the existing finished-game verification path instead of introducing a new result type
- it reuses player-color evidence we already read for `win-as-white` and `win-as-black`
- it adds one useful intermediate step between `finish-any-game` and the stricter win challenges
...
For a submitted `finish-as-white` attempt:
...
6. persist one of these verdicts:
   - `passed`: username is White in a finished game
   - `failed`: username appears in a finished game, but the user was not White
...
- add `finish-as-white` to `src/lib/challenges.ts`
- extend `src/lib/lichess.ts` only as needed to verify a side-specific finished-game challenge
- wire `src/app/actions.ts` to persist the real verdict for `finish-as-white`
- leave the rest of the product loop unchanged

## Acceptance for the next code slice

A signed-in user with a saved Lichess username can submit a finished game for `finish-as-white` and then see a persisted automated verdict of `passed`, `failed`, or `pending` after refresh.
...
```bash
test -f docs/NEXT_VERIFIER_BACKED_CHALLENGE_SLICE_2026-04-10.md
```
