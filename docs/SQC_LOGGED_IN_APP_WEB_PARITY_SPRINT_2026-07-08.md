...
- Web evidence before edit: signed-in Sam production `/settings` snapshot and `artifacts/sqc-parity-settings-editor-2026-07-09/web-settings-before.png` showed the old generic `Account sync` simple screen with `Progress sync`, `Chess username`, and `Display name` rows.
