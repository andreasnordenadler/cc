...
- Fresh targeted iOS/auth/release contracts: **60 passing**. They cover generated-project identity, Apple sign-in availability/completion/recovery, encryption declaration, EAS toolchain, release provenance, review packet, and release workflow controls.
- Fresh reconciled-source gates (2026-09-20): full suite **2,294 passing**; root lint and production web build passed; mobile TypeScript passed. The frozen build-3 `apps/mobile` tree remains byte-identical to current `origin/main`. Expo Doctor reported **17/18**: its non-CNG advisory detects tracked native project folders alongside `app.json` native configuration. This is a release-configuration warning to reconcile before a rebuild, not evidence about the already-exported IPA. The isolated candidate checkout was clean before and after the gates.
