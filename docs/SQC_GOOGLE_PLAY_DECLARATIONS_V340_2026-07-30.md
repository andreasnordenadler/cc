...
Current checked-in release baseline: `0.1.348 (348)`; Current inspected Google Play upload candidate: `0.1.348 (349)`
...
- Current canonical source: `origin/main` at `6702cf288d2b0e7f10d005d6e3e0866c71b953f7`; its latest mobile baseline is `4925cd13b6a39a8be1658ac46c0bea396260dbd2` (`0.1.348`, checked-in code `348`). That immutable commit is the EAS build source. The nine later `origin/main` commits change only web source/tests and this candidate's merged upload-approval packet; mobile runtime and release inputs remain byte-identical to the build source.
- The current inspected AAB is `0.1.348 (349)`, AAB SHA-256: `c416609b1240114612f888c8a0fff205fafe0a8821ee4065cb833b395f7cbf71`, size 86,159,538 bytes. EAS build: `dd277377-25fb-4923-a1ec-10b930c25563`. Immutable EAS source: `4925cd13b6a39a8be1658ac46c0bea396260dbd2`. It is the only current upload candidate; codes 342 through 348 are superseded.
- Current AAB inspection confirms package `com.sidequestchess.app`, version `0.1.348` (349), min SDK 24, target SDK 36, no enabled debuggable attribute, `allowBackup=false`, four ABIs, ZIP and bundletool validity, production API `https://sidequestchess.com`, bounded runtime permissions, and continuity with the established upload certificate. The Play app-signing certificate remains a separate console/read-back and Play-delivered-install gate.
...
- The repository now contains the reviewed listing-ready 512 × 512 store icon and 1,024 × 500 feature graphic. Current-candidate phone screenshots remain blocked on the authorized Play-delivered code-349 install and must not be substituted with responsive engineering captures.
...
- upload candidate: `0.1.348` / version code `349`, AAB SHA-256 `c416609b1240114612f888c8a0fff205fafe0a8821ee4065cb833b395f7cbf71`;
...
2. Confirm Internal testing still belongs to this exact package and that no code higher than `349` makes the candidate stale.
...
4. Upload only the bound code-349 AAB; require Play to parse package `com.sidequestchess.app`, version `0.1.348`, and code `349`. An APK or sideload is not an acceptable substitute.
...
Tester assignment, AAB upload, Console declaration entry, declaration submission, internal-track rollout, Play-delivered installation, and publication each remain an explicit owner gate. Until these read-backs pass, code 349 is a provenance-verified Play-format candidate, not a Play-accepted internal-test release.
...
The current code-349 AAB was inspected after the permission-hardening merge. Its manifest is limited to Internet, network state, vibration, and the app-scoped dynamic-receiver permission; storage, overlay, biometric, fingerprint, location, camera, microphone, contacts, SMS, phone, and Play install-referrer permissions are absent. Manifest presence or absence alone is not permission to answer a data-safety question inaccurately.
...
- current code-349 AAB provenance: `apps/mobile/artifacts/android/mobile-v348-code349/evidence/AAB_PROVENANCE.md` in the mobile release lane
- current upload-approval and Play-delivered acceptance packet: `docs/SQC_GOOGLE_PLAY_INTERNAL_UPLOAD_REQUEST_CODE349_2026-08-10.md`
...
- legacy listing copy and asset evidence only: `docs/SQC_GOOGLE_PLAY_LISTING_V340_2026-07-30.md`. Its code-348 candidate and screenshot instructions are superseded; the code-349 upload-approval packet controls current candidate operations.
