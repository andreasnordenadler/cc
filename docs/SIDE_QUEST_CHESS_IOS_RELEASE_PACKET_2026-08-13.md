...
Use this in review context if requested; do not assume App Store Connect exposes release notes for the first App Store version. Refresh Android status immediately before use and do not submit while Android remains private. First iOS release. Side Quest Chess follows the public web and Android products with Solo, Custom, Community, and Multiplayer Side Quests; public-game proof checks; account sync; Trophy Cabinet; support; Community Multiplayer reporting/blocking; a Community Solo support-report handoff; and in-app account deletion.

## Age-rating draft

Owner must adopt the answers in App Store Connect after checking the current questionnaire wording. Product-fact draft:

- Gambling, simulated gambling, sweepstakes, loot boxes, and real-money prizes: none. **Contests: Frequent** under Apple's broad current definition because recurring rankings, goals, podiums, and trophy outcomes are reachable, even though there are no prizes. Re-evaluate the exact frequency against the frozen candidate and live questionnaire before adoption.
...
- Product distribution posture: minimum intended audience 13+. This is not a promise of a uniform worldwide storefront rating: Apple-calculated regional ratings may be higher. With Social Media enabled, preserve the live questionnaire receipt and verify the expected current AU 16+, VN 16+, and KR 15+ outcomes (and any other regional variation) before territory adoption. Apply a minimum 13+ override if Apple calculates lower so the store rating does not contradict the Terms minimum age.

Capability-answer draft to map field-by-field onto the live dated questionnaire: user-generated content **Yes**; social media **Yes**; messaging/chat **Yes** under the public-posting definition; contests **Frequent**; unrestricted web access **No**; advertising **No**; in-app purchases **No**; loot boxes **No**; gambling/simulated gambling/real-money gaming **No**; sweepstakes **No**; location sharing **No**; parental controls **No**; social media disabled for users under 13 **No**; age-assurance mechanism inside the app **No**. Do not adopt reporting/filtering/blocking answers more broadly than the exact tested surfaces. Publisher-authored mature content is not enough to answer whole-app descriptor frequencies: profanity, sexual/suggestive themes, drugs, violence, and every other UGC-capable descriptor remain unadoptable until filtering and an exact-candidate reachable-content inventory are verified. Record the exact questionnaire version/date, every displayed question and selected frequency, Apple-calculated rating, 13+ minimum override if needed, and every regional outcome/override before adoption.
...
| --- | --- | --- |
| Contact Info / Name | App Functionality | First/last name received from Clerk or a selected sign-in provider; runner display names and handles belong under User ID |
...
| Identifiers / User ID | App Functionality | Clerk user/session identity, runner display names, Clerk usernames, public chess handles, participant/host IDs, report-target account IDs, and application records |
...
| User Content / Other User Content | App Functionality | Profile bio, custom/community and Multiplayer free text, invite copy, and user-authored report reasons |
| Usage Data / Product Interaction | App Functionality | Persisted likes, reports, blocks, quest selections/actions, proof checks/submissions, Multiplayer creation/joins/leaves, and related action timestamps; linked to the signed-in account |
| Diagnostics / Other Diagnostic Data | App Functionality | Optional support bundle's app build/version, bundle/package ID, platform and OS version, and API destination; default off and previewed before send. Display name/usernames remain User ID, while active-quest and Multiplayer counts remain Gameplay Content/Product Interaction |

Mobile-origin Product Interaction is collected for App Functionality because signed-in product actions and their resulting records are transmitted and persisted. Do not add Analytics as a purpose unless Crowdler verifies those records are also used to evaluate user behavior or product features. Display names, usernames, provider profile images, standings, Community content, and proof details may be publicly visible by product design. Clerk and the selected Apple, Google, Facebook, or email/password sign-in path process authentication data; hosting and chess-record providers process the data described by the public policy. Current reports and blocks are stored in the reporter's Clerk private metadata and are therefore account-linked; source commit `f615820c368bf70a44038ab99a3ccab1dfcda0ac` adds an explicit policy disclosure, but that disclosure is not public until the branch lands and the web policy is deployed. Account deletion removes account-attached identity/profile/progress after required cleanup, subject to security, legal, fraud-prevention, backup, and de-identified/aggregate exceptions stated in the adopted policy. Before adoption, obtain a dated Clerk/Vercel/hosting inventory for retained IP addresses, user agents, session/device identifiers, retention, linkage, and purpose; IPA inspection alone cannot close provider-side collection. Map any retained provider data to Apple's then-current Coarse Location, Device ID, Diagnostics, or other applicable type instead of assuming it is exempt. Do not declare crash data, performance data, device ID, location, contacts, purchases, browsing/search history, or sensitive information until the exact IPA, provider inventory, Expo, React Native, and embedded privacy manifests have been reconciled. The optional support bundle is collected when sent despite being opt-in. Adopted answers must match `https://sidequestchess.com/privacy`, provider contracts, retention behavior, and the production binary. Supply `https://sidequestchess.com/privacy#choices` as the optional User Privacy Choices URL only after the candidate policy is deployed and that anchor plus a signed-out monitored Crowdler support contact are live.
...
- Fresh full canonical suite after reconciliation and the deletion/support, packet-contract, and compact-touch-target corrections: 763/763 passed.
...
- A reviewed inventory of compact native menu, close, tab, selection, filter, authentication-mode, duration, and condition-action controls now retains at least 44-point source-defined heights, with explicit 44×44 dimensions for icon-only controls and their fixed close-button wrapper. A regression contract covers the enumerated styles. This narrows but does not close the physical-device VoiceOver, Dynamic Type, contrast, effective-layout, or touch-target matrix. The mobile README now matches the current native moderation controls and approval-gated EAS profiles.
